...
public class TestDriverManagerJdbcConnectionFactory {
  ...
  @Test
  public void testCreateConnectionThrowsWithInvalidDriverClass()
      throws Exception {
    String invalidDriverClass = "this_is_an_invalid_driver_class";
    connectionFactory = new DriverManagerJdbcConnectionFactory(
        invalidDriverClass, HsqldbTestServer.getUrl(), DB_USERNAME,
        DB_PASSWORD);

    expectedException.expect(RuntimeException.class);
    expectedException.expectMessage(
        "Could not load db driver class: this_is_an_invalid_driver_class");
    connectionFactory.createConnection();
  }

  @Test
  public void testCreateConnectionThrowsWithoutRunningDatabase()
      throws Exception {
    String notRunningDb = "jdbc:postgresql://myhost:1234/database";
    connectionFactory = new DriverManagerJdbcConnectionFactory(
        POSTGRESQL_DRIVER_CLASS, notRunningDb, DB_USERNAME, DB_PASSWORD);
  ...
  public void testCreateConnectionThrowsWithInvalidUsername() throws Exception {
    ...
    connectionFactory = new DriverManagerJdbcConnectionFactory(
        HSQLDB_DRIVER_CLASS, HsqldbTestServer.getUrl(), invalidUsername,
        DB_PASSWORD);
  ...
  public void testCreateConnectionThrowsWithInvalidPassword() throws Exception {
    ...
    connectionFactory = new DriverManagerJdbcConnectionFactory(
        HSQLDB_DRIVER_CLASS, HsqldbTestServer.getUrl(), DB_USERNAME,
        invalidPassword);
  ...
  @Test
  public void testCreateConnectionSucceedsWithValidParameters()
      throws Exception {
    connectionFactory = new DriverManagerJdbcConnectionFactory(
        HSQLDB_DRIVER_CLASS, HsqldbTestServer.getUrl(), DB_USERNAME,
        DB_PASSWORD);
