...
public class HBaseImportAddRowKeyTest extends HBaseTestCase {
  ...
  public void testAddRowKeyDefault() throws IOException {
    String[] argv =
        getImportArguments(false, hbaseTableName, hbaseColumnFamily);
  ...
  public void testAddCompositeKey() throws IOException {
    ...
    String[] argv =
        getImportArguments(true, hbaseTableName, hbaseColumnFamily, rowKey);

    runImport(argv);

    // Row key should have been added
    verifyHBaseCell(hbaseTableName, join(columnValues, '_'), hbaseColumnFamily,
                    getColName(0), columnValues[0]);
    verifyHBaseCell(hbaseTableName, join(columnValues, '_'), hbaseColumnFamily,
                    getColName(1), columnValues[1]);
  }

  private String[] getImportArguments(boolean addRowKey, String hbaseTableName,
  ...
  private String[] getImportArguments(boolean addRowKey, String hbaseTableName,
                                      String hbaseColumnFamily, String rowKey) {
    ...
    result.addAll(
        asList(getArgv(true, hbaseTableName, hbaseColumnFamily, true, null)));
