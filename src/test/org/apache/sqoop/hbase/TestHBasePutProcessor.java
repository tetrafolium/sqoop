...
@Rule public ExpectedLogMessage expectedLogMessage = new ExpectedLogMessage();
...
public void before() {
	...
	hBasePutProcessor = new HBasePutProcessor(configuration, putTransformer,
	                                          hbaseConnection, bufferedMutator);
}
...
@Test
public void testNoMutationIsDoneWhenListContainingNullsIsReceived()
throws Exception {
...
@Test
public void testNoMutationIsDoneWhenListContainingUnknownMutationIsReceived()
throws Exception {
...
@Test
public void testWarningIsLoggedWhenListContainingEmptyPutIsReceived()
throws Exception {
	...
	when(putTransformer.getMutationCommand(anyMap())).thenReturn(inputList);
	verifyNoMoreInteractions(bufferedMutator);
	expectedLogMessage.expectWarn(
		"Could not insert row with no columns for row-key column: emptyPutMutation");

	hBasePutProcessor.accept(fieldMappable);
}

@Test
public void testWarningIsLoggedWhenListContainingEmptyDeleteIsReceived()
throws Exception {
	Mutation emptyDeleteMutation = mock(Delete.class);
	when(emptyDeleteMutation.getRow())
	.thenReturn("emptyDeleteMutation".getBytes());
	when(emptyDeleteMutation.isEmpty()).thenReturn(true);
	List<Mutation> inputList = singletonList(emptyDeleteMutation);
	when(putTransformer.getMutationCommand(anyMap())).thenReturn(inputList);
	verifyNoMoreInteractions(bufferedMutator);
	expectedLogMessage.expectWarn(
		"Could not delete row with no columns for row-key column: emptyDeleteMutation");
...
@Test
public void
testMutationIsDoneForAllElementsWhenListContainingValidMutationsIsReceived()
throws Exception {
	Mutation aPutMutation = mock(Put.class);
	Mutation aDeleteMutation = mock(Delete.class);
	Mutation anotherPutMutation = mock(Put.class);
	List<Mutation> inputList =
		Arrays.asList(aPutMutation, aDeleteMutation, anotherPutMutation);
	when(putTransformer.getMutationCommand(anyMap())).thenReturn(inputList);
