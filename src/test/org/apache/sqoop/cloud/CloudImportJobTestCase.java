...
public abstract class CloudImportJobTestCase extends ImportJobTestCase {
  ...
  public static final String CUSTOM_MAP_OUTPUT_FILE_00001 =
      MAPREDUCE_OUTPUT_BASENAME + "-m-00001";
  public static final String CUSTOM_REDUCE_OUTPUT_FILE_00000 =
      MAPREDUCE_OUTPUT_BASENAME + "-r-00000";
  ...
  @Rule public CloudFileSystemRule fileSystemRule;
  ...
  protected ArgumentArrayBuilder
  getArgumentArrayBuilderForUnitTests(String targetDir) {
  ...
  protected ArgumentArrayBuilder
  getArgumentArrayBuilderForHadoopCredProviderUnitTests(String targetDir) {
  ...
  protected ArgumentArrayBuilder
  getArgumentArrayBuilderForUnitTestsWithFileFormatOption(String targetDir,
                                                          String fileFormat) {
    ArgumentArrayBuilder builder =
        getArgumentArrayBuilderForUnitTests(targetDir);
    builder.withOption(fileFormat);
    useParquetHadoopAPIImplementationIfAsParquet(builder, fileFormat);
    return builder;
  }

  protected String[] getArgsForUnitTestsWithFileFormatOption(
      String targetDir, String fileFormat) {
    ArgumentArrayBuilder builder =
        getArgumentArrayBuilderForUnitTests(targetDir);
    builder.withOption(fileFormat);
    useParquetHadoopAPIImplementationIfAsParquet(builder, fileFormat);
  ...
  private void
  useParquetHadoopAPIImplementationIfAsParquet(ArgumentArrayBuilder builder,
                                               String fileFormat) {
    // Parquet import to Cloud storages is supported only with the Parquet
    // Hadoop API implementation.
  ...
  protected ArgumentArrayBuilder
  addExternalHiveTableImportArgs(ArgumentArrayBuilder builder, String hs2Url,
                                 String externalTableDir) {
    return builder.withOption("hive-import")
        .withOption("hs2-url", hs2Url)
        .withOption("external-table-dir", externalTableDir);
  }

  protected ArgumentArrayBuilder
  addCreateHiveTableArgs(ArgumentArrayBuilder builder,
                         String hiveExternalTableDirName) {
    return builder.withOption("create-hive-table")
        .withOption("hive-table", hiveExternalTableDirName);
  }

  protected ArgumentArrayBuilder
  addIncrementalAppendImportArgs(ArgumentArrayBuilder builder,
                                 String temporaryRootDir) {
    return builder.withOption("incremental", "append")
  ...
  protected ArgumentArrayBuilder
  addIncrementalMergeImportArgs(ArgumentArrayBuilder builder,
                                String temporaryRootDir) {
    return builder.withOption("incremental", "lastmodified")
  ...
  protected void createTestTableFromInputData() {
    ...
    createTableWithColTypesAndNames(dataSet.getColumnNames(),
                                    dataSet.getColumnTypes(), new String[0]);
  ...
  protected void insertInputDataIntoTable(String[] inputData) {
    insertIntoTable(dataSet.getColumnNames(), dataSet.getColumnTypes(),
                    inputData);
  }

  protected void createTestTableFromInitialInputDataForMerge() {
    createTableWithRecordsWithColTypesAndNames(
        dataSet.getColumnNamesForMerge(), dataSet.getColumnTypesForMerge(),
        dataSet.getInitialInputDataForMerge());
  }

  protected void
  insertInputDataIntoTableForMerge(List<List<Object>> inputData) {
    insertRecordsIntoTableWithColTypesAndNames(dataSet.getColumnNamesForMerge(),
                                               dataSet.getColumnTypesForMerge(),
                                               inputData);
  }

  protected void failIfOutputFilePathContainingPatternExists(
      FileSystem cloudFileSystem, Path targetDirPath, String pattern)
      throws IOException {
    List<Path> outputFilesWithPathContainingPattern =
        FileSystemUtil.findFilesWithPathContainingPattern(
            targetDirPath, cloudFileSystem.getConf(), pattern);
    if (outputFilesWithPathContainingPattern.size() != 0) {
      fail("No output file was expected with pattern" + pattern);
    }
  }

  protected void failIfOutputFilePathContainingPatternDoesNotExists(
      FileSystem cloudFileSystem, Path targetDirPath, String pattern)
      throws IOException {
    List<Path> outputFilesWithPathContainingPattern =
        FileSystemUtil.findFilesWithPathContainingPattern(
  ...
  protected CloudTestDataSet getDataSet() { return dataSet; }

  protected HiveMiniCluster createCloudBasedHiveMiniCluster() {
    HiveMiniCluster hiveMiniCluster =
        new HiveMiniCluster(new NoAuthenticationConfiguration());
