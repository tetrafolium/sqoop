...
public final class HCatalogTestUtils {
  ...
  public void setStorageInfo(String info) { storageInfo = info; }
  ...
  private static String getHCatCreateTableCmd(String dbName, String tableName,
                                              List<HCatFieldSchema> tableCols,
   ...
   * memory list.
   */
  public void createHCatTableUsingSchema(String dbName, String tableName,
                                         List<HCatFieldSchema> tableCols,
                                         List<HCatFieldSchema> partKeys)
      throws Exception {

    String databaseName =
        dbName == null ? SqoopHCatUtilities.DEFHCATDB : dbName;
    dropHCatTableIfExists(tableName, databaseName);
    LOG.info("Successfully dropped HCatalog table if it existed previously " +
             databaseName + '.' + tableName);
    String createCmd =
        getHCatCreateTableCmd(databaseName, tableName, tableCols, partKeys);
  ...
  public void dropHCatTableIfExists(String tableName, String databaseName) {
    ...
    try {
      ...
      LOG.info("Unable to drop table." + databaseName + "." + tableName +
               ".   Assuming it did not exist");
    }
  }
  ...
  public static class HCatWriterMapper
    ...
    public static int getWrittenRecordCount() { return writtenRecordCount; }
  ...
  public static class HCatReaderMapper
    ...
    public static int getReadRecordCount() { return readRecordCount; }
  ...
  loadHCatTable(String dbName, String tableName, Map<String, String> partKeyMap,
    ...
    Path path =
        new Path(fs.getWorkingDirectory(), "mapreduce/HCatTableIndexInput");

    job.getConfiguration().setInt(ConfigurationConstants.PROP_MAPRED_MAP_TASKS,
                                  1);
    ...
    OutputJobInfo outputJobInfo =
        OutputJobInfo.create(dbName, tableName, partKeyMap);
   ...
   * @throws Exception
   */
  public List<HCatRecord> readHCatRecords(String dbName, String tableName,
    ...
    Path path =
        new Path(fs.getWorkingDirectory(), "mapreduce/HCatTableIndexOutput");
    if (fs.exists(path)) {
      fs.delete(path, true);
    }
   ...
   * defined columns.
   */
  public enum KeyType { NOT_A_KEY, STATIC_KEY, DYNAMIC_KEY }
  ;
   ...
   */
  public static String forIdx(int idx) { return "COL" + idx; }

  public static ColumnGenerator
  colGenerator(final String name, final String dbType, final int sqlType,
    ...
    if (expectedVal != null && expectedVal instanceof byte[]) {
      ...
      if (expectedVal instanceof Float) {
        if (actualVal instanceof Double) {
          ...
          Assert.assertEquals("Got unexpected column value", expectedVal,
                              actualVal);
        }
   ...
   */
  public void assertSqlColValForRowId(Connection conn, String table, int id,
                                      String colName, boolean escapeId,
    ...
    PreparedStatement statement = conn.prepareStatement(
        "SELECT \"" + colName + "\" FROM " + table + " WHERE " + escapeStr +
            "id" + escapeStr + " = " + id,
   ...
   */
  public static void
  assertHCatColValForRowId(List<HCatRecord> recs, HCatSchema schema, int id,
                           String fieldName, Object expectedVal)
      throws IOException {
  ...
  getSqlCreateTableStatement(String tableName, boolean escapeIdMsgCol,
                             ColumnGenerator... extraCols) {
    StringBuilder sb = new StringBuilder();
    sb.append("CREATE TABLE ");
    sb.append(tableName);
    String escapeStr = escapeIdMsgCol ? "\"" : "";
    sb.append(" (" + escapeStr + "id" + escapeStr +
              " INT NOT NULL PRIMARY KEY, " + escapeStr + "msg" + escapeStr +
              " VARCHAR(64)");
    int colNum = 0;
  ...
  public static String
  getSqlInsertTableStatement(String tableName, ColumnGenerator... extraCols) {
    StringBuilder sb = new StringBuilder();
  ...
  public void createSqlTable(Connection conn, boolean generateOnly, int count,
                             String table, ColumnGenerator... extraCols)
      throws Exception {
    createSqlTable(conn, generateOnly, count, table, false, extraCols);
  }
  public void createSqlTable(Connection conn, boolean generateOnly, int count,
                             String table, boolean escapeIdMsgCols,
                             ColumnGenerator... extraCols) throws Exception {
    PreparedStatement statement = conn.prepareStatement(
        getSqlDropTableStatement(table), ResultSet.TYPE_FORWARD_ONLY,
  ...
  public HCatSchema createHCatTable(CreateMode mode, int count, String table,
                                    ColumnGenerator... extraCols)
      throws Exception {
    ...
    if (mode != CreateMode.NO_CREATION) {
      ...
      createHCatTableUsingSchema(null, table, hCatTblSchema.getFields(),
                                 hCatPartSchema.getFields());
  ...
  HCatSchema createHCatExternalTable(String table, ColumnGenerator... extraCols)
    ...
    String createCmd =
        getHCatCreateTableCmd(databaseName, table, hCatTblSchema.getFields(),
                              hCatPartSchema.getFields())
            .replaceFirst("create table", "create external table");
    utils.launchHCatCli(createCmd);
    LOG.info("Created HCatalog table " + databaseName + "." + table);
    return hCatFullSchema;
  }

  private void loadHCatTable(HCatSchema hCatSchema, String table, int count,
                             ColumnGenerator... extraCols) throws Exception {
    ...
    loadHCatTable(null, table, staticKeyMap, hCatSchema,
  ...
  private HCatSchema generateHCatTableSchema(ColumnGenerator... extraCols)
    ...
    for (ColumnGenerator gen : extraCols) {
      if (gen.getKeyType() == KeyType.NOT_A_KEY) {
        ...
        case DECIMAL:
          tInfo =
              new DecimalTypeInfo(gen.getHCatPrecision(), gen.getHCatScale());
          break;
        default:
          tInfo = new PrimitiveTypeInfo();
        ...
        hCatTblCols.add(
            new HCatFieldSchema(gen.getName().toLowerCase(), tInfo, ""));
      }
    }
  ...
  private HCatSchema
  generateHCatDynamicPartitionSchema(ColumnGenerator... extraCols)
      throws Exception {
    List<HCatFieldSchema> hCatPartCols = new ArrayList<HCatFieldSchema>();
    PrimitiveTypeInfo tInfo;
  ...
  private HCatSchema
  generateHCatStaticPartitionSchema(ColumnGenerator... extraCols)
      throws Exception {
    List<HCatFieldSchema> hCatPartCols = new ArrayList<HCatFieldSchema>();
    PrimitiveTypeInfo tInfo;
  ...
  public String hCatRecordDump(List<HCatRecord> recs, HCatSchema schema)
      throws Exception {
