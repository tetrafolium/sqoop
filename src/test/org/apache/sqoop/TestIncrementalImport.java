...
public class TestIncrementalImport {
  ...
  public static final Log LOG =
      LogFactory.getLog(TestIncrementalImport.class.getName());
  ...
  public static Configuration newConf() {
    ...
    conf.set(AutoGenericJobStorage.AUTO_STORAGE_USER_KEY,
             AUTO_STORAGE_USERNAME);
    conf.set(AutoGenericJobStorage.AUTO_STORAGE_PASS_KEY,
             AUTO_STORAGE_PASSWORD);
  ...
  private void assertRowCount(String table, int numRows) throws SQLException {
    ...
    ResultSet rs = null;
    try {
      s = c.prepareStatement("SELECT COUNT(*) FROM " +
                             manager.escapeTableName(table));
      rs = s.executeQuery();
      if (!rs.next()) {
        fail("No resultset");
      }
  ...
  private void insertIdRows(String tableName, int low, int hi)
    ...
    try {
      s = c.prepareStatement("INSERT INTO " +
                             manager.escapeTableName(tableName) + " VALUES(?)");
  ...
  private void insertIdTimestampRows(String tableName, int low, int hi,
    ...
    try {
      s = c.prepareStatement(
          "INSERT INTO " + manager.escapeTableName(tableName) + " VALUES(?,?)");
      for (int i = low; i < hi; i++) {
        s.setInt(1, i);
        s.setTimestamp(2, ts);
        s.executeUpdate();
  ...
  private void createIdTable(String tableName, int insertRows)
    ...
    PreparedStatement s = null;
    try {
      s = c.prepareStatement("CREATE TABLE " +
   ...
   */
  private void createIdVarcharTable(String tableName, int insertRows)
      throws SQLException {
    SqoopOptions options = new SqoopOptions();
    options.setConnectString(SOURCE_DB_URL);
    HsqldbManager manager = new HsqldbManager(options);
  ...
  /**
   * Look at a directory that should contain files full of an imported 'id'
   * column and 'last_modified' column. Assert that all numbers in [0,
   * expectedNums) are present in order.
   */
  public void assertDirOfNumbersAndTimestamps(String tableName,
                                              int expectedNums) {
    try {
      FileSystem fs = FileSystem.getLocal(new Configuration());
      Path warehouse = new Path(BaseSqoopTestCase.getLocalWarehouseDir());
      ...
      for (int i = 0; i < expectedNums; i++) {
        assertEquals((int)i,
                     (int)Integer.valueOf(receivedNums.get(i).split(",")[0]));
      }
    } catch (Exception e) {
      fail("Got unexpected exception: " + StringUtils.stringifyException(e));
    }
  ...
  private List<String> getArgListForQuery(String query, String directoryName,
                                          boolean commonArgs, boolean isAppend,
                                          boolean appendTimestamp) {
    List<String> args = new ArrayList<String>();
    if (commonArgs) {
      CommonArgs.addHadoopFlags(args);
   ...
   */
  private void runJob(String jobName) { runJob(jobName, newConf()); }
  ...
  public void testEmptyThenFullJobAppendWithQuery() throws Exception {
    ...
    List<String> args =
        getArgListForQuery(QUERY, TABLE_NAME, false, true, false);
    createJob(TABLE_NAME, args);
    runJob(TABLE_NAME);
    assertDirOfNumbers(TABLE_NAME, 0);
  ...
  @Test
  public void testEmptyLastModifiedWithNonExistingParentDirectory()
      throws Exception {
    final String TABLE_NAME = "emptyLastModifiedNoParent";
    final String QUERY = "SELECT id, last_modified FROM \"" + TABLE_NAME +
                         "\" WHERE $CONDITIONS";
    final String DIRECTORY = "non-existing/parents/" + TABLE_NAME;
    createTimestampTable(TABLE_NAME, 0, null);
    List<String> args =
        getArgListForQuery(QUERY, DIRECTORY, true, false, false);
  ...
  @Test
  public void testLastModifiedImportWithExistingOutputDirectoryFails()
      throws Exception {
  ...
  public void testAppendWithString() throws Exception {
    ...
    thrown.reportMissingExceptionWithMessage(
        "Expected incremental import on varchar column to fail");
    runJob(TABLE_NAME);
  }
  ...
  public void testModifyWithTimestamp() throws Exception {
    ...
    PreparedStatement s = null;
    try {
      s = c.prepareStatement("UPDATE " + manager.escapeTableName(TABLE_NAME) +
  ...
  public void testUpdateModifyWithTimestampWithQuery() throws Exception {
    ...
    final String QUERY =
        "SELECT id, last_modified FROM \"UpdateModifyWithTimestampWithQuery\" WHERE $CONDITIONS";

    List<String> args =
        getArgListForQuery(QUERY, TABLE_NAME, true, false, false);
  ...
  public static class InstrumentHsqldbManager extends HsqldbManager {
    ...
    public InstrumentHsqldbManager(SqoopOptions options) { super(options); }
  ...
  public void testIncrementalHiveAppendEmptyThenFull() throws Exception {
    ...
    String testDataPath =
        new Path(new Path(hiveHome), "scripts/"
