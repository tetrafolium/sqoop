...
public static final Log LOG =
	LogFactory.getLog(TestAppendUtils.class.getName());
...
protected ArrayList getOutputlessArgv(boolean includeHadoopFlags,
                                      boolean queryBased, String[] colNames,
                                      Configuration conf) {
	if (null == colNames) {
...
private FileStatus[] listFiles(FileSystem fs, Path path, String prefix)
throws IOException {
...
throws IOException {
	...
	try {
		...
		String previousImage =
			getFileCreationTimeImage(fs, outputPath, previousFileCount);
...
public void testAppend() throws IOException {
	ArrayList args = getOutputlessArgv(
		false, false, HsqldbTestServer.getFieldNames(), getConf());
...
public void testAppendWithQuery() throws IOException {
	ArrayList args = getOutputlessArgv(
		false, true, HsqldbTestServer.getFieldNames(), getConf());
	String targetDir = getWarehouseDir() + "/tempTargetDir";
	args.add("--target-dir");
	args.add(targetDir);
...
public void testAppendSrcDoesNotExist() throws IOException {
	...
	ImportJobContext importContext =
		new ImportJobContext("meep", null, options, missingPath);
	AppendUtils utils = new AppendUtils(importContext);
	utils.append();
}

/**
 * Test that when we pass in -Dmapreduce.output.basename=prefix, file should
 * also got appended
 *
 * @throws IOException
 */
...
public void testAppendWithMapreduceOutputBasename() throws IOException {
	...
	args.addAll(getOutputlessArgv(false, true, HsqldbTestServer.getFieldNames(),
	                              getConf()));
