...
public class SystemImportTest extends OraOopTestCase {
  ...
  static {
    try {
      ...
      methSetBinaryDouble = preparedStatementClass.getMethod(
          "setBinaryDouble", int.class, double.class);
      methSetBinaryFloat = preparedStatementClass.getMethod(
          "setBinaryFloat", int.class, float.class);
    } catch (Exception e) {
  ...
  public void importTest() throws Exception {
    ...
    try {
      ...
      try {
        s.executeUpdate(
            "CREATE TABLE " + OracleUtils.SYSTEMTEST_TABLE_NAME +
        ...
        PreparedStatement ps = conn.prepareStatement(
            "INSERT INTO " + OracleUtils.SYSTEMTEST_TABLE_NAME +
        ...
        try {
          ...
          int retCode = runImport(OracleUtils.SYSTEMTEST_TABLE_NAME,
                                  getSqoopConf(), true);
          ...
          for (FileStatus status : statuses) {
            if (status.getPath().getName().startsWith("part-m-")) {
              ...
              SqoopRecord value = ((Class<SqoopRecord>)reader.getValueClass())
                                      .getConstructor()
                                      .newInstance();
              ps = conn.prepareStatement(
                  "SELECT bd, bf, b, c, cl, d, f, l, nc, "
              ...
              while (reader.next(key, value)) {
                ...
                assertTrue("Did not find row with id " + id + " in oracle",
                           rs.next());
                assertEquals("BinaryDouble did not match for row " + id,
                             fields.get("BD"), rs.getDouble(1));
                assertEquals("BinaryFloat did not match for row " + id,
                             fields.get("BF"), rs.getFloat(2));
                // LONG column needs to be read before BLOB column
                assertEquals("Long did not match for row " + id,
                             fields.get("L"), rs.getString(8));
                ...
                assertEquals("Char did not match for row " + id,
                             fields.get("C"), rs.getString(4));
                ClobRef hadoopClob = (ClobRef)fields.get("CL");
                Clob oraClob = rs.getClob(5);
                assertEquals("Clob did not match for row " + id,
                             hadoopClob.getData(),
                             oraClob.getSubString(1, (int)oraClob.length()));
                assertEquals("Date did not match for row " + id,
                             fields.get("D"), rs.getString(6));
                ...
                assertEquals("Float did not match for row " + id, hadoopFloat,
                             oraFloat);
                assertEquals("NChar did not match for row " + id,
                             fields.get("NC"), rs.getString(9));
                assertEquals("NClob did not match for row " + id,
                             fields.get("NCL"), rs.getString(10));
                assertEquals("NVarChar did not match for row " + id,
                             fields.get("NVC"), rs.getString(11));
                assertEquals("RowId did not match for row " + id,
                             fields.get("R"),
                ...
                assertEquals("UriType did not match for row " + id,
                             fields.get("U"), urlString);
                ...
                assertEquals("Interval Day to Second did not match for row " +
                                 id,
                             ids, rs.getString(15));
                assertEquals("Timestamp did not match for row " + id,
                             fields.get("T"), rs.getString(16));
          ...
          assertEquals("Number of records in Hadoop does not match number of "
                           + "records in oracle",
