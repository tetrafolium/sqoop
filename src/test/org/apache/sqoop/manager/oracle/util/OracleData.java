...
private static String getColumnList(OracleTableDefinition tableDefinition) {
	...
	for (OracleDataDefinition column : tableDefinition.getColumnList()) {
		result.append(delim)
		.append(OracleUtils.escapeIdentifier(column.getColumnName()))
		.append(" ")
...
getDataExpression(List<OracleDataDefinition> columnList) {
	...
	for (OracleDataDefinition column : columnList) {
		result.append("l_ret_rec.")
		.append(OracleUtils.escapeIdentifier(column.getColumnName()))
		.append(" := ")
		.append(column.getDataExpression())
		.append(";\n");
	}
	return result.toString();
}
...
throws Exception {
	String pkgSql = IOUtils.toString(
		classLoader.getResource("oraoop/pkg_tst_product_gen.psk").openStream());
	pkgSql =
...
throws Exception {
	String pkgSql = IOUtils.toString(
		classLoader.getResource("oraoop/pkg_tst_product_gen.pbk").openStream());
	...
	pkgSql = pkgSql.replaceAll("\\$COLUMN_LIST", columnList);
	pkgSql = pkgSql.replaceAll("\\$TABLE_NAME", tableDefinition.getTableName());
	pkgSql =
		pkgSql.replaceAll("\\$DATA_EXPRESSION_LIST",
		                  getDataExpression(tableDefinition.getColumnList()));

	pkgSql = pkgSql.replaceAll("\\$TABLE_ORGANIZATION_CLAUSE",
	                           tableDefinition.isIndexOrganizedTable()
	                           ? "ORGANIZATION INDEX OVERFLOW NOLOGGING"
	                           : "");

	pkgSql = pkgSql.replaceAll("\\$PARTITION_CLAUSE",
	                           tableDefinition.getPartitionClause());
	PreparedStatement stmt = conn.prepareStatement(pkgSql);
	stmt.execute();
}
...
private static String getKeyColumns(KeyType keyType,
	...
	if (columns != null && columns.size() > 0) {
		...
		for (String column : columns) {
			keyColumnList.append(delim).append(
				OracleUtils.escapeIdentifier(column));
			delim = ",";
		}
...
private static void dropTableIfExists(Connection conn, String tableName)
throws Exception {
