...
public class OracleConnectionFactoryTest extends OraOopTestCase {
  ...
  private void setAndCheckJdbcFetchSize(int jdbcFetchSize) {
    ...
    try {
      ...
      if (!sqlFound) {
        Assert.fail("Unable to find the performance metrics for the SQL "
  ...
  public void testCreateOracleJdbcConnectionBadUserName() {
    ...
    try {
      ...
      Assert.fail("OracleConnectionFactory should have thrown an exception in "
  ...
  public void testExecuteOraOopSessionInitializationStatements() {
    ...
    checkLogContainsText(
        ...
        "Initializing Oracle session with SQL : alter session disable "
            + "parallel query");
    checkLogContainsText(
        oraoopLog, "Initializing Oracle session with SQL : alter session set "
    ...
    try {
      ...
      if (!doesTableExist(conn, OracleUtils.SYSTEMTEST_TABLE_NAME)) {
        Assert.fail(
            "The session-initialization statement to create the table " +
      ...
      checkExecuteOraOopSessionInitializationStatements(
          "create table " + OracleUtils.SYSTEMTEST_TABLE_NAME +
          " (col1 number);insert into " + OracleUtils.SYSTEMTEST_TABLE_NAME +
          " values (1) ; --update " + OracleUtils.SYSTEMTEST_TABLE_NAME +
          " set col1 = col1 + 1; update " + OracleUtils.SYSTEMTEST_TABLE_NAME +
  ...
  public void testParseOraOopSessionInitializationStatements() {
    ...
    try {
      statements =
          OracleConnectionFactory.parseOraOopSessionInitializationStatements(
              null);
  ...
  private void
  checkExecuteOraOopSessionInitializationStatements(String statements) {
  ...
  public void testSetSessionClientInfo() {
    ...
    try {
      ...
      if (resultSet.next()) {
        Assert.fail(
            "There should be no Oracle sessions with an action name of " +
            actionName);
      }
      ...
      if (sessionFoundCount < 1) {
        Assert.fail(
            "Unable to locate an Oracle session with the expected module "
            + "and action.");
      }

      if (sessionFoundCount > 1) {
        Assert.fail("Multiple sessions were found with the expected module and "
