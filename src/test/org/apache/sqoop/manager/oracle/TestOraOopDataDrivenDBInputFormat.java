...
import org.apache.sqoop.manager.oracle.OraOopConstants.OraOopOracleBlockToSplitAllocationMethod;
...
public class TestOraOopDataDrivenDBInputFormat extends OraOopTestCase {
  ...
  public class Exposer<T extends SqoopRecord>
    ...
    @Override
    public List<InputSplit> groupTableDataChunksIntoSplits(
      ...
      return super.groupTableDataChunksIntoSplits(
          dataChunks, desiredNumberOfSplits, blockAllocationMethod);
    }
  }
  ...
  public void testgroupTableDataChunksIntoSplits() {
    ...
    for (int idx = 0; idx < 241; idx++) {
      OraOopOracleDataChunk dataChunk = new OraOopOracleDataChunkExtent(
          "23480", 666, 1, startBlockNumber, startBlockNumber + 8);
      startBlockNumber += 8;
    ...
    List<InputSplit> splits = e.groupTableDataChunksIntoSplits(
        dataChunks, 32, OraOopOracleBlockToSplitAllocationMethod.SEQUENTIAL);
    ...
    if (lowestNumberOfDataChunksAllocatedToASplit == 0) {
      Assert.fail("There is a split that has not had any "
