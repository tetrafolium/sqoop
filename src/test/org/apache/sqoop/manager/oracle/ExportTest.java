...
public static void setUpHdfsData() throws Exception {
	...
	int retCode = TEST_CASE.runImport("TST_PRODUCT", TEST_CASE.getSqoopConf(),
	                                  false, DISABLE_ORACLE_ESCAPING_FLAG);
	assertEquals("Return code should be 0", 0, retCode);
}

@Test
public void testProductExport() throws Exception {
	int retCode = TEST_CASE.runExportFromTemplateTable(
		"TST_PRODUCT", "TST_PRODUCT_EXP", false, DISABLE_ORACLE_ESCAPING_FLAG);
	assertEquals("Return code should be 0", 0, retCode);
}
...
public void testProductExportMixedCaseTableName() throws Exception {
	int retCode = TEST_CASE.runExportFromTemplateTable(
		"TST_PRODUCT", "\"\"T5+_Pr#duct_Exp\"\"", false,
