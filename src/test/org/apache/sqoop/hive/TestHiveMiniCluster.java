...
public static Iterable<? extends Object> authenticationParameters() {
	return Arrays.asList(
		new NoAuthenticationConfiguration(),
...
public void before() throws SQLException {
	...
	connectionFactory = authenticationConfiguration.decorateConnectionFactory(
		new HiveServer2ConnectionFactory(hiveMiniCluster.getUrl(),
...
private int getDataFromTestTable() throws SQLException {
	try (Connection conn = connectionFactory.createConnection();
	     PreparedStatement stmnt = conn.prepareStatement(SELECT_SQL)) {
...
private void createTestTable() throws SQLException {
	try (Connection conn = connectionFactory.createConnection();
	     PreparedStatement stmnt = conn.prepareStatement(CREATE_TABLE_SQL)) {
		stmnt.executeUpdate();
	}
}
