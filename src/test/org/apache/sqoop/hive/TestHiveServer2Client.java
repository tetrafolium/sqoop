...
private static final List<String> TEST_COMMANDS =
	asList("command1", "command2", "command3");
...
public void before() throws Exception {
	...
	when(hs2Connection.prepareStatement(anyString()))
	.thenReturn(preparedStatement);

	when(tableDefWriter.getCreateTableStmt())
	.thenReturn(CREATE_TABLE_STATEMENT);
	when(tableDefWriter.getLoadDataStmt()).thenReturn(LOAD_DATA_STATEMENT);
	when(tableDefWriter.getFinalPath()).thenReturn(finalPath);

	hs2Client = new HiveServer2Client(sqoopOptions, tableDefWriter,
	                                  hs2ConnectionFactory, hiveClientCommon);
	hs2ClientSpy = spy(hs2Client);
}

@Test
public void
testImportTableExecutesHiveImportWithCreateTableAndLoadDataCommands()
throws Exception {
	doNothing().when(hs2ClientSpy).executeHiveImport(anyList());

	hs2ClientSpy.importTable();

	verify(hs2ClientSpy, times(1))
	.executeHiveImport(asList(CREATE_TABLE_STATEMENT, LOAD_DATA_STATEMENT));
}

@Test
public void testCreateTableExecutesHiveImportWithCreateTableCommandOnly()
throws Exception {
	doNothing().when(hs2ClientSpy).executeHiveImport(anyList());
	...
	verify(hs2ClientSpy, times(1))
	.executeHiveImport(asList(CREATE_TABLE_STATEMENT));
}

@Test
public void testExecuteHiveImportInvokesMethodsInCorrectSequence()
throws Exception {
...
@Test
public void testExecuteCommandsCreatesExactlyOneConnection()
throws Exception {
...
@Test
public void
testExecuteCommandsClosesConnectionWhenStatementExecutionIsSuccessful()
throws Exception {
	hs2Client.executeCommands(TEST_COMMANDS);
...
@Test
public void testExecuteCommandsClosesConnectionWhenStatementExecutionThrows()
throws Exception {
	when(hs2Connection.prepareStatement(anyString()))
...
@Test
public void
testExecuteCommandsClosesPreparedStatementsWhenStatementExecutionIsSuccessful()
throws Exception {
	hs2Client.executeCommands(TEST_COMMANDS);
...
@Test
public void
testExecuteCommandsClosesPreparedStatementWhenStatementExecutionThrows()
throws Exception {
...
@Test
public void testExecuteCommandsThrowsWhenCreateConnectionThrows()
throws Exception {
...
@Test
public void testExecuteCommandsThrowsWhenPrepareStatementThrows()
throws Exception {
...
@Test
public void testExecuteCommandsThrowsWhenExecuteStatementThrows()
throws Exception {
	SQLException expected = mock(SQLException.class);
