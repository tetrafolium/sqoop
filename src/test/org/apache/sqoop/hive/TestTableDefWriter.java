...
private static final String TEST_AVRO_SCHEMA =
	"{\"type\":\"record\",\"name\":\"IMPORT_TABLE_1\",\"fields\":[{\"name\":\"C1_VARCHAR\",\"type\":[\"null\",\"string\"]},{\"name\":\"C2_INTEGER\",\"type\":[\"null\",\"int\"]},{\"name\":\"_3C_CHAR\",\"type\":[\"null\",\"string\"]}]}";

private static final String EXPECTED_CREATE_PARQUET_TABLE_STMNT =
	"CREATE TABLE IF NOT EXISTS `outputTable` ( `C1_VARCHAR` STRING, `C2_INTEGER` INT, `_3C_CHAR` STRING) STORED AS PARQUET";

public static final Log LOG =
	LogFactory.getLog(TestTableDefWriter.class.getName());
...
public void before() {
	...
	when(connManager.getColumnTypes(anyString()))
	.thenReturn(new SqlTypeMap<String, Integer>());
	when(connManager.getColumnNames(anyString())).thenReturn(new String[] {});

	writer = new TableDefWriter(options, connManager, inputTable, outputTable,
	                            conf, false);
}
...
private void expectExceptionInCharCode(int charCode) {
	...
	thrown.reportMissingExceptionWithMessage(
		"Expected IllegalArgumentException with out-of-range Hive delimiter");
...
public void testDifferentTableNames() throws Exception {
	...
	assertTrue(
		createTable.indexOf("CREATE TABLE IF NOT EXISTS `outputTable`") != -1);
...
public void testDifferentTargetDirs() throws Exception {
	...
	// Assert that the statements generated have the form we expect.
	assertTrue(createTable.indexOf("CREATE TABLE IF NOT EXISTS `" +
	                               outputTable + "`") != -1);
	assertTrue(loadData.indexOf("INTO TABLE `" + outputTable + "`") != -1);
...
public void testUserMappingWithDecimal() throws Exception {
	options.setMapColumnHive(
		"id=STRING,value2=DECIMAL(13,5),value1=INTEGER,value3=DECIMAL(4,5),value4=VARCHAR(255)");
...
public void testUserMappingFailWhenCantBeApplied() throws Exception {
	...
	thrown.expect(IllegalArgumentException.class);
	thrown.reportMissingExceptionWithMessage(
		"Expected IllegalArgumentException on non applied Hive type mapping");
...
public void testGetCreateTableStmtWithAvroSchema() throws Exception {
	...
	assertEquals(EXPECTED_CREATE_PARQUET_TABLE_STMNT,
	             writer.getCreateTableStmt());
}

private void setUpMockConnManager(String tableName,
                                  Map<String, Integer> typeMap) {
	when(connManager.getColumnTypes(tableName)).thenReturn(typeMap);
	when(connManager.getColumnNames(tableName))
	.thenReturn(typeMap.keySet().toArray(new String[] {}));
}
}
