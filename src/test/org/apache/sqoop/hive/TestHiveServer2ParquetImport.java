...
public class TestHiveServer2ParquetImport {
  ...
  private static final String[] TEST_COLUMN_NAMES = {"C1_VARCHAR", "C2#INTEGER",
                                                     "3C_CHAR"};

  private static final String[] TEST_COLUMN_TYPES = {"VARCHAR(32)", "INTEGER",
                                                     "CHAR(64)"};

  private static final String[] TEST_COLUMN_ALL_TYPES = {
      "INTEGER",   "BIGINT", "DOUBLE",       "DECIMAL(10, 2)", "BOOLEAN",
      "TIMESTAMP", "BINARY", "VARCHAR(100)", "CHAR(100)"};

  private static final List<Object> TEST_COLUMN_ALL_TYPES_VALUES =
      Arrays.<Object>asList(10, 12345678910123L, 12.34, 456842.45, "TRUE",
                            "2018-06-14 15:00:00.000", "abcdef", "testVarchar",
                            "testChar");

  private static final Object[] EXPECTED_TEST_COLUMN_ALL_TYPES_VALUES = {
      10,
      12345678910123L,
      12.34,
      "456842.45",
      true,
      timeFromString("2018-06-14 15:00:00.000"),
      decodeHex("abcdef"),
      "testVarchar",
      "testChar"};

  private static final List<Object> TEST_COLUMN_VALUES =
      Arrays.<Object>asList("test", 42, "somestring");

  private static final List<Object> TEST_COLUMN_VALUES_MAPPED =
      Arrays.<Object>asList("test", "42", "somestring");

  private static final List<Object> TEST_COLUMN_VALUES_LINE2 =
      Arrays.<Object>asList("test2", 4242, "somestring2");
  ...
  public static class ParquetCompressionCodecTestCase
    ...
    public void setUp() {
      ...
      createTableWithColTypesAndNames(TEST_COLUMN_NAMES, TEST_COLUMN_TYPES,
                                      TEST_COLUMN_VALUES);
    }

    @Test
    public void testHiveImportAsParquetWithCompressionCodecCanBeLoaded()
        throws Exception {
    ...
    public void testImportedFilesHaveCorrectCodec() throws Exception {
      Path tablePath = new Path(hiveMiniCluster.getTempFolderPath() + "/" +
  ...
  public static class GeneralParquetTestCase extends ImportJobTestCase {
    ...
    @Test
    public void
    testHiveImportAsParquetWithMapColumnJavaAndOriginalColumnNameSucceeds()
        throws Exception {
      String[] args = commonArgs(getConnectString(), getTableName())
     ...
     * The reason is that
     * org.apache.sqoop.orm.AvroSchemaGenerator#toAvroType(java.lang.String,
     * int) which maps the Avro schema type uses the original column name and
     * not the Avro identifier but
     * org.apache.sqoop.orm.ClassWriter#toJavaType(java.lang.String, int) can
     * map the DAO class field types based on the Avro identifier too so there
     * will be a discrepancy between the generated Avro schema types and the DAO
     * class field types.
     */
    @Test
    public void testHiveImportAsParquetWithMapColumnJavaAndAvroIdentifierFails()
        throws Exception {
      String[] args = commonArgs(getConnectString(), getTableName())
    ...
    /**
     * This test case documents that a mapping with the Avro
     * identifier(C2_INTEGER) of a special column name(C2#INTEGER) is ignored in
     * map-column-hive. The reason is that the column type of the Avro schema
     * and the Hive table must be equal and if we would be able to override the
     * Hive column type using map-column-hive the inconsistency would cause a
     * Hive error during reading.
     */
    @Test
    public void
    testHiveImportAsParquetWithMapColumnHiveAndAvroIdentifierIgnoresMapping()
        throws Exception {
      String[] args = commonArgs(getConnectString(), getTableName())
     ...
     * The reason is that Sqoop uses the Avro identifier(C2_INTEGER) as Hive
     * column name and there is a check in
     * org.apache.sqoop.hive.TableDefWriter#getCreateTableStmt() which verifies
     * that all the columns in map-column-hive are actually valid column names.
     * Since C2_INTEGER is used instead of C2#INTEGER the check will fail on the
     * latter.
     */
    @Test
    public void
    testHiveImportAsParquetWithMapColumnHiveAndOriginalColumnNameFails()
        throws Exception {
      String[] args = commonArgs(getConnectString(), getTableName())
      ...
      expectedException.expectMessage(
          "No column by the name C2#INTEGERfound while importing data");
    ...
    public void testAllDataTypesHiveImportAsParquet() throws Exception {
      ...
      createTableWithColTypes(TEST_COLUMN_ALL_TYPES,
                              TEST_COLUMN_ALL_TYPES_VALUES);
      String[] args = commonArgs(getConnectString(), getTableName()).build();
    ...
    public void testAppendHiveImportAsParquet() throws Exception {
      ...
      insertIntoTable(TEST_COLUMN_NAMES, TEST_COLUMN_TYPES,
                      TEST_COLUMN_VALUES_LINE2);
    ...
    /**
     * --create-hive-table option is now supported with the Hadoop Parquet
    ...
    /**
     * This scenario works fine since the Hadoop Parquet writer implementation
     * does not check the Parquet schema of the existing files. The exception
     * will be thrown by Hive when it tries to read the files with different
     * schema.
     */
    @Test
    public void testHiveImportAsParquetWhenTableExistsWithIncompatibleSchema()
      ...
      String hiveTableName =
          "hiveImportAsParquetWhenTableExistsWithIncompatibleSchema";
      String[] incompatibleSchemaTableTypes = {"INTEGER", "INTEGER", "INTEGER"};
      List<Object> incompatibleSchemaTableData =
          Arrays.<Object>asList(100, 200, 300);
      ...
      incrementTableNum();
      createTableWithColTypes(incompatibleSchemaTableTypes,
                              incompatibleSchemaTableData);
  ...
  private static ArgumentArrayBuilder commonArgs(String connectString,
                                                 String tableName) {
