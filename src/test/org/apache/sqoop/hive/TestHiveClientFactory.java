...
public class TestHiveClientFactory {
  ...
  public void before() {
    connectionFactoryInitializer =
        mock(HiveServer2ConnectionFactoryInitializer.class);
  ...
  @Test
  public void testCreateHiveClientCreatesHiveImportWhenHs2UrlIsNotProvided()
      throws Exception {
    HiveClient hiveClient =
        hiveClientFactory.createHiveClient(sqoopOptions, connectionManager);
    assertThat(hiveClient, instanceOf(HiveImport.class));
  }

  @Test
  public void testCreateHiveClientInitializesHiveImportProperly()
      throws Exception {
    HiveImport hiveImport = (HiveImport)hiveClientFactory.createHiveClient(
        sqoopOptions, connectionManager);
  ...
  @Test
  public void testCreateHiveClientCreatesHiveServer2ClientWhenHs2UrlIsProvided()
      throws Exception {
  ...
  @Test
  public void testCreateHiveClientInitializesHiveServer2ClientProperly()
      throws Exception {
    when(sqoopOptions.getHs2Url()).thenReturn(TEST_HS2_URL);
    when(connectionFactoryInitializer.createJdbcConnectionFactory(sqoopOptions))
        .thenReturn(jdbcConnectionFactory);

    HiveServer2Client hs2Client =
        (HiveServer2Client)hiveClientFactory.createHiveClient(
            sqoopOptions, connectionManager);

    softly.assertThat(hs2Client.getSqoopOptions()).isSameAs(sqoopOptions);
    softly.assertThat(hs2Client.getHs2ConnectionFactory())
        .isSameAs(jdbcConnectionFactory);
    softly.assertThat(hs2Client.getTableDefWriter().getOptions())
        .isSameAs(sqoopOptions);
    softly.assertThat(hs2Client.getTableDefWriter().getConnManager())
        .isSameAs(connectionManager);
    softly.assertThat(hs2Client.getTableDefWriter().getInputTableName())
        .isEqualTo(TEST_TABLE_NAME);
    softly.assertThat(hs2Client.getTableDefWriter().getOutputTableName())
        .isEqualTo(TEST_HIVE_TABLE_NAME);
    softly.assertThat(hs2Client.getTableDefWriter().getConfiguration())
        .isSameAs(configuration);
    softly.assertThat(hs2Client.getTableDefWriter().isCommentsEnabled())
        .isFalse();
