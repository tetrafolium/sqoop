...
public class TestExportUpdate extends ExportJobTestCase {
  ...
  private void populateDatabase(int numRows) throws SQLException {
    ...
    PreparedStatement statement =
        conn.prepareStatement("CREATE TABLE " + getTableName() +
   ...
   */
  private void createUpdateFiles(int numFiles, int updatesPerFile, int keyCol,
                                 int... startOffsets) throws IOException {
    Configuration conf = getConf();
    if (!isOnPhysicalCluster()) {
      conf.set(CommonArgs.FS_DEFAULT_NAME, CommonArgs.LOCAL_FS);
    }
  ...
  private void runUpdate(int numMappers, String updateCol) throws IOException {
    runExport(
        getArgv(true, 2, 2, "-m", "" + numMappers, "--update-key", updateCol));
  }
  ...
  public void testUpdateColumnSubset() throws Exception {
    ...
    runExport(getArgv(true, 2, 2, "-m", "1", "--update-key", "A", "--columns",
                      "A,B"));
  ...
  public void testUpdateColumnNotInColumns() throws Exception {
    ...
    thrown.reportMissingExceptionWithMessage(
        "Expected IOException as --columns is not a superset of --update-key");
    runExport(
        getArgv(true, 2, 2, "-m", "1", "--update-key", "A", "--columns", "B"));
  }
}
