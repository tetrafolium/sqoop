...
@Rule public ExpectedLogMessage logMessage = new ExpectedLogMessage();
...
public void before() {
	...
	importTool =
		new ImportTool("import", codeGenTool, false, hiveClientFactory);
	importToolSpy = spy(importTool);
}

@Theory
public void
esnureTransactionIsolationLevelsAreMappedToTheRightValues(Object[] values)
throws Exception {
	ImportTool importTool = new ImportTool();
	String[] args = {"--" + BaseSqoopTool.METADATA_TRANSACTION_ISOLATION_LEVEL,
		         values[0].toString()};
	SqoopOptions options = importTool.parseArguments(args, null, null, true);
	assertThat(options.getMetadataTransactionIsolationLevel(),
	           is(equalTo(values[1])));
}

@Test
public void testImportToolHandlesAvroSchemaMismatchExceptionProperly()
throws Exception {
	...
	AvroSchemaMismatchException expectedException =
		new AvroSchemaMismatchException(errorMessage, writtenWithSchema,
		                                actualSchema);
	doThrow(expectedException)
	.when(importToolSpy)
	.importTable(any(SqoopOptions.class));
...
public void testExternalTableNoHiveImportThrowsException()
throws InvalidOptionsException {
	String hdfsTableDir = "/data/movielens/genre";
	SqoopOptions options =
		new SqoopOptions("jdbc:postgresql://localhost/movielens", "genres");
	...
	Assert.fail(
		"testExternalTableNoHiveImportThrowsException unit test failed!");
}

@Test
public void
testShouldCheckExistingOutputDirectoryReturnsFalseForHBaseImport() {
	SqoopOptions sqoopOptions = mock(SqoopOptions.class);
...
@Test
public void
testShouldCheckExistingOutputDirectoryReturnsTrueForNonHBaseImport() {
	SqoopOptions sqoopOptions = mock(SqoopOptions.class);
