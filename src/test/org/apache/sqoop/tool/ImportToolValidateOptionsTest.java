...
public class ImportToolValidateOptionsTest {
  ...
  fileLayoutAndValidationMessageParameters() {
  ...
  public ImportToolValidateOptionsTest(SqoopOptions.FileLayout fileLayout,
  ...
  @Test
  public void testValidationFailsWithHiveImportAndIncrementalLastmodified()
      throws Exception {
    ...
    thrown.expectMessage(
        BaseSqoopTool.HIVE_IMPORT_WITH_LASTMODIFIED_NOT_SUPPORTED);

    importTool.validateOptions(options);
  }

  /**
   * Note that append mode (--append) is designed to be used with HDFS import
   * and not Hive import. However this test case is added to make sure that the
   * error message generated is correct even if --append is used.
   *
   */
  @Test
  public void
  testValidationFailsWithHiveImportAndAppendModeIncrementalLastmodified()
      throws Exception {
    SqoopOptions options = buildBaseSqoopOptions();
    options.setHiveImport(true);
    options.setIncrementalTestColumn(CHECK_COLUMN_NAME);
  ...
  @Test
  public void testValidationSucceedsWithHiveImportAndIncrementalAppendRows()
      throws Exception {
    SqoopOptions options = buildBaseSqoopOptions();
    options.setHiveImport(true);
    options.setIncrementalTestColumn(CHECK_COLUMN_NAME);
  ...
  /**
   * Note that append mode (--append) is designed to be used with HDFS import
   * and not Hive import. However this test case is added to make sure that
   * SQOOP-2986 does not break the already existing validation.
   *
   */
  @Test
  public void
  testValidationSucceedsWithHiveImportAndAppendModeAndIncrementalAppendRows()
      throws Exception {
    SqoopOptions options = buildBaseSqoopOptions();
    options.setHiveImport(true);
    options.setIncrementalTestColumn(CHECK_COLUMN_NAME);
  ...
  @Test
  public void
  testValidationFailsWithHBaseImportAndFileLayoutDifferentFromTexFile()
      throws Exception {
    SqoopOptions options = buildBaseSqoopOptions();
  ...
  @Test
  public void testValidationSucceedsWithHBaseImportAndAsTextFile()
      throws Exception {
    SqoopOptions options = buildBaseSqoopOptions();
    options.setHBaseTable(HBASE_TABLE_NAME);
    options.setHBaseColFamily(HBASE_COL_FAMILY);
