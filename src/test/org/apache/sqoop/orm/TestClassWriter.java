...
static final String CODE_GEN_DIR =
	BaseSqoopTestCase.getTempBaseDir() + "sqoop/test/codegen";
static final String JAR_GEN_DIR =
	BaseSqoopTestCase.getTempBaseDir() + "sqoop/test/jargen";

private File runGenerationTest(String[] argv, String classNameToCheck) {
	return runGenerationTest(argv, classNameToCheck,
...
private File runGenerationTest(String[] argv, String classNameToCheck,
	...
	ClassWriter writer =
		new ClassWriter(options, manager, tableName, compileMgr);
	...
	String classFileNameToCheck =
		classNameToCheck.replace('.', File.separatorChar);
	...
	File tableClassFile =
		new File(classGenDirFile, classFileNameToCheck + ".class");
...
public void testSetClassName() {
	...
	// Set the option strings in an "argv" to redirect our srcdir and bindir
	String[] argv = {
		"--bindir",   JAR_GEN_DIR,    "--outdir",
		CODE_GEN_DIR, "--class-name", OVERRIDE_CLASS_NAME,
	};
...
public void testSetClassAndPackageName() {
	...
	String[] argv = {
		"--bindir",   JAR_GEN_DIR,    "--outdir",
		CODE_GEN_DIR, "--class-name", OVERRIDE_CLASS_AND_PACKAGE_NAME,
	};
...
public void testSetPackageName() {
	...
	String[] argv = {
		"--bindir",   JAR_GEN_DIR,      "--outdir",
		CODE_GEN_DIR, "--package-name", OVERRIDE_PACKAGE_NAME,
	};
...
public void testJavaIdentifierConversion() {
	...
	assertEquals("_int", ClassWriter.toJavaIdentifier("int"));
	assertEquals("thisismanywords",
	             ClassWriter.toJavaIdentifier("this is many words"));
	assertEquals("_9isLegalInSql",
	             ClassWriter.toJavaIdentifier("9isLegalInSql"));
	...
	assertEquals("_PROTOCOL_VERSION",
	             ClassWriter.toJavaIdentifier("PROTOCOL_VERSION"));
}
...
public void testColumnNameAsProtocolVersion() throws SQLException {
	...
	try {
		st.executeUpdate("DROP TABLE " + tableName + " IF EXISTS");
		st.executeUpdate("CREATE TABLE " + tableName + " (PROTOCOL_VERSION INT)");
...
@Test
public void testCloningTableWithVarbinaryDoesNotThrowNPE()
...
InvocationTargetException {
	...
	String className =
		OVERRIDE_PACKAGE_NAME + "." + HsqldbTestServer.getTableName();
	File ormJarFile = runGenerationTest(argv, className);

	ClassLoader prevClassLoader =
		ClassLoaderStack.addJarFile(ormJarFile.getCanonicalPath(), className);
	...
	assertTrue(cloneImplementation.invoke(instance)
	           .getClass()
	           .getCanonicalName()
	           .equals(className));
...
InvocationTargetException {
	...
	ClassLoader prevClassLoader = ClassLoaderStack.addJarFile(
		ormJarFile.getCanonicalPath(), OVERRIDE_CLASS_AND_PACKAGE_NAME);
	Class tableClass =
		Class.forName(OVERRIDE_CLASS_AND_PACKAGE_NAME, true,
...
InvocationTargetException {
	...
	ClassLoader prevClassLoader = ClassLoaderStack.addJarFile(
		ormJarFile.getCanonicalPath(), USERMAPPING_CLASS_AND_PACKAGE_NAME);
	Class tableClass =
		Class.forName(USERMAPPING_CLASS_AND_PACKAGE_NAME, true,
		              Thread.currentThread().getContextClassLoader());
...
public void testNoClassGeneration() throws Exception {
	...
	ClassWriter writer = new ClassWriter(
		options, manager, HsqldbTestServer.getTableName(), compileMgr);
...
public void testWideTableClassGeneration() throws Exception {
	...
	ClassLoader prevClassLoader = ClassLoaderStack.addJarFile(
		ormJarFile.getCanonicalPath(), WIDE_TABLE_NAME);
	Class tableClass = Class.forName(
		WIDE_TABLE_NAME, true, Thread.currentThread().getContextClassLoader());

	Object instance = tableClass.newInstance();
	Method setterMethod =
		tableClass.getMethod("setField", String.class, Object.class);
...
private void createWideTable() throws Exception {
	try (Connection conn = testServer.getConnection();
	     Statement stmt = conn.createStatement(); ) {
		stmt.executeUpdate("DROP TABLE \"" + WIDE_TABLE_NAME + "\" IF EXISTS");
		StringBuilder sb =
			new StringBuilder("CREATE TABLE \"" + WIDE_TABLE_NAME + "\" (");
		for (int i = 0; i < WIDE_TABLE_COLUMN_COUNT; ++i) {
