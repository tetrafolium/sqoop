...
public void testEmptyString() throws Exception {
	...
	thrown.reportMissingExceptionWithMessage(
		"Expected InvalidOptionsException on empty string");
	SqoopOptions.toChar("");
}
...
public void testNullString() throws Exception {
	thrown.expect(SqoopOptions.InvalidOptionsException.class);
	thrown.reportMissingExceptionWithMessage(
		"Expected InvalidOptionsException on null string");
...
public void testUnknownEscape1() throws Exception {
	thrown.expect(SqoopOptions.InvalidOptionsException.class);
	thrown.reportMissingExceptionWithMessage(
		"Expected InvalidOptionsException on unknown escaping");
...
public void testErrOctalChar() throws Exception {
	...
	thrown.reportMissingExceptionWithMessage(
		"Expected NumberFormatException on erroneous octal char");
...
public void testErrHexChar() throws Exception {
	thrown.expect(NumberFormatException.class);
	thrown.reportMissingExceptionWithMessage(
		"Expected NumberFormatException on erroneous hex char");
...
public void testBadNumMappers1() throws Exception {
	...
	thrown.expect(SqoopOptions.InvalidOptionsException.class);
	thrown.reportMissingExceptionWithMessage(
		"Expected InvalidOptionsException on invalid --num-mappers argument");
	parse(args);
}
...
public void testBadNumMappers2() throws Exception {
	...
	thrown.expect(SqoopOptions.InvalidOptionsException.class);
	thrown.reportMissingExceptionWithMessage(
		"Expected InvalidOptionsException on invalid -m argument");
	parse(args);
}
...
public void testPropertySerialization1() {
	...
	assertEquals("connection params don't match", connParams,
	             out.getConnectionParams());
	assertEquals("connection params don't match", connParams,
	             in.getConnectionParams());
}
...
private void validateImportOptions(String[] extraArgs) throws Exception {
	String[] args = {
		"--connect", HsqldbTestServer.getUrl(), "--table", "test", "-m", "1",
	};
	ImportTool importTool = new ImportTool();
...
public void testDeleteTargetDirWithAppend() throws Exception {
	...
	thrown.expect(SqoopOptions.InvalidOptionsException.class);
	thrown.reportMissingExceptionWithMessage(
		"Expected InvalidOptionsException on incompatibility of "
...
public void testHBaseBulkLoad() throws Exception {
	String[] extraArgs = {
		...
		longArgument(BaseSqoopTool.TARGET_DIR_ARG),
		"./test",
		longArgument(BaseSqoopTool.HBASE_TABLE_ARG),
		"test_table",
		longArgument(BaseSqoopTool.HBASE_COL_FAM_ARG),
		"d"
	};
...
public void testHBaseBulkLoadMissingHbaseTable() throws Exception {
	...
	thrown.expect(SqoopOptions.InvalidOptionsException.class);
	thrown.reportMissingExceptionWithMessage(
		"Expected InvalidOptionsException because of missing --hbase-table");
	validateImportOptions(extraArgs);
}
...
public void testResetToOneMapperAndSplitBy() throws Exception {
	...
	thrown.expect(SqoopOptions.InvalidOptionsException.class);
	thrown.reportMissingExceptionWithMessage(
		"Expected Exception on incompatibility of "
...
public void testEscapeMapingColumnNames() throws Exception {
	...
	String[] args = {
		"--" + org.apache.sqoop.tool.BaseSqoopTool
...
public void testSqoopOptionsCloneIsEqual() throws Exception {
	...
	options.getConf().setAllowNullValueProperties(
		false); // always false in cloned conf
...
@Test
public void testSqoopOptionsCloneHasDistinctReferenceTypes()
throws Exception {
	SqoopOptions options = createSqoopOptionsFilledWithRandomData();
	SqoopOptions clonedOptions = (SqoopOptions)options.clone();
	...
	for (Field field : getEligibleFields(options.getClass())) {
		...
		.describedAs(
			String.format("%s : %s", field.getName(), field.getType()))
		.isNotSameAs(field.get(options));
	}
	softly.assertAll();
}

private Iterable<? extends Field>
getEligibleFields(Class<? extends SqoopOptions> clazz) {
	...
	for (Field field : allFields) {
		if (!field.getType().isPrimitive() &&
		    ...
		    !Modifier.isFinal(
			    field.getModifiers())) { // final and static fields are expected
			                             // to be the same
...
throws Exception {
	...
	options.getColumnNames(); // initializes the mapReplacedColumnJava field,
	                          // which is cloned but is otherwise inaccessible
...
private void setFieldToValueIfApplicable(Object target, Field field)
throws Exception {
	...
	for (Field field : clazz.getDeclaredFields()) {
		if (field.getType().equals(clazz) ||
		    field.getType().equals(
			    ClassLoader.class)) { // need to protect against circles.
			continue;
		}
