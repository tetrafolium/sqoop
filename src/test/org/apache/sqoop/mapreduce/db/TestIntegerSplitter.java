...
public class TestIntegerSplitter {
    ...
    for (int i = 0; i < expected.length; i++) {
      try {
        assertEquals("Failure at position " + i + "; got " + actual[i] +
                         ...
                         formatLongArray(actual),
                     expected[i], actual[i]);
      } catch (ArrayIndexOutOfBoundsException oob) {
        fail("Expected array with " + expected.length + " elements; got " +
             actual.length + ". Actual array is " + formatLongArray(actual));
      }
    }

    if (actual.length > expected.length) {
      fail("Actual array has " + actual.length + " elements; expected " +
           expected.length + ". Actual array is " + formatLongArray(actual));
    }
  }
  ...
  public void testOddSplits() throws SQLException {
    ...
    long[] expected = {
        0, 10, 20, 30, 40, 50, 59, 68, 77, 86, 95,
    };
    assertLongArrayEquals(expected, toLongArray(splits));
  }
  ...
  public void testBigIntSplits() throws SQLException {
    List<Long> splits =
        new IntegerSplitter().split(4, -1, 14, 7863696997872966707L);
