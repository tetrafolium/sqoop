...
private Verifier verifyThatLogsAreUploaded = new Verifier() {
	@Override
	public void verify() {
		File jobDir = tmpFolder.getRoot()
		              .toPath()
		              .resolve("job_job001_0001")
		...
		try {
			assertThat(FileUtils.readFileToString(jobDir.listFiles()[0]),
			           is(equalTo("test log")));
		} catch (IOException e) {
			e.printStackTrace();
...
public RuleChain rules =
	RuleChain.outerRule(tmpFolder).around(verifyThatLogsAreUploaded);
 ...
 * @return
 */
private NetezzaExternalTableExportMapper<LongWritable, Text>
basicMockingOfMapper() {
	NetezzaExternalTableExportMapper<LongWritable, Text> mapper =
		new NetezzaExternalTableExportMapper<LongWritable, Text>() {
		@Override
		public void map(LongWritable key, Text text, Context context) {
			// no-op. Don't read from context, mock won't be ready to handle
			// that.
		}
	};
