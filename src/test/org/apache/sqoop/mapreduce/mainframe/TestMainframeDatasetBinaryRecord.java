...
public class TestMainframeDatasetBinaryRecord {
  ...
  private static final Log LOG =
      LogFactory.getLog(TestMainframeDatasetBinaryRecord.class.getName());
  ...
  protected Answer returnSqoopRecord(final int byteLength) {
    return new Answer() {
      public Object answer(InvocationOnMock invocation) { return byteLength; }
    };
  }
  ...
  public void testGetNextBinaryRecordForFullRecord() {
    ...
    try {
      ...
      Assert.assertEquals(
          MainframeConfiguration
              .MAINFRAME_FTP_TRANSFER_BINARY_DEFAULT_BUFFER_SIZE.intValue(),
          ((byte[])record.getFieldMap().values().iterator().next()).length);
  ...
  public void testGetNextBinaryRecordFor2Records() {
    ...
    try {
      ...
      Assert.assertTrue(
          MainframeConfiguration
              .MAINFRAME_FTP_TRANSFER_BINARY_DEFAULT_BUFFER_SIZE.equals(
                  (((byte[])record.getFieldMap().values().iterator().next())
                       .length)));
      ...
      LOG.error(
          "Issue with reading 1 full binary buffer record followed by 1 partial binary buffer record",
          ioe);
      throw new RuntimeException(ioe);
    }
  }
  ...
  public void testGetNextBinaryRecordForMultipleReads() {
    ...
    try {
      ...
      LOG.error("Issue with verifying reading partial buffer binary records",
                ioe);
      throw new RuntimeException(ioe);
    }
  }
