...
public class TestMerge extends BaseSqoopTestCase {
  ...
  private static final List<List<Integer>> initRecords =
      Arrays.asList(Arrays.asList(new Integer(0), new Integer(0)),
                    Arrays.asList(new Integer(1), new Integer(42)));

  private static final List<List<Integer>> newRecords =
      Arrays.asList(Arrays.asList(new Integer(1), new Integer(43)),
                    Arrays.asList(new Integer(3), new Integer(313)));

  private static final List<List<Integer>> mergedRecords =
      Arrays.asList(Arrays.asList(new Integer(0), new Integer(0)),
  ...
  protected void createTable(List<List<Integer>> records) throws SQLException {
    PreparedStatement s =
        conn.prepareStatement("DROP TABLE \"" + TABLE_NAME + "\" IF EXISTS");
    try {
      s.executeUpdate();
    } finally {
      s.close();
  ...
  public void runMergeTest(SqoopOptions.FileLayout fileLayout)
      throws Exception {
  ...
  private boolean valueMatches(GenericRecord genericRecord,
                               List<Integer> recordVals) {
    return recordVals.get(0).equals(genericRecord.get(0)) &&
        recordVals.get(1).equals(genericRecord.get(1));
  }

  private void importData(String targetDir,
   ...
   */
  protected boolean checkTextFileForLine(FileSystem fs, Path p,
                                         List<Integer> record)
      throws IOException {
  ...
  private boolean checkAvroFileForLine(FileSystem fs, Path p,
                                       List<Integer> record)
      throws IOException {
  ...
  private boolean checkParquetFileForLine(Path path, List<Integer> record)
      throws IOException {
    List<GenericRecord> resultRecords =
        new ParquetReader(path.getParent()).readAll();
  ...
  protected boolean checkFileForLine(FileSystem fs, Path p,
                                     SqoopOptions.FileLayout fileLayout,
  ...
  protected void assertRecordStartsWith(List<Integer> record, String dirName,
    ...
    if (!recordStartsWith(record, dirName, fileLayout)) {
      fail("No record found that starts with [" +
           StringUtils.join(record, ", ") + "] in " + dirName);
    }
  }
}
