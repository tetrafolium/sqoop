...
public class TestSplittableBufferedWriter {
  ...
  public static final Log LOG =
      LogFactory.getLog(TestSplittableBufferedWriter.class.getName());

  private String getWriteDir() {
    return new File(BaseSqoopTestCase.getTempBaseDir(), "bufferedWriterTest")
        .toString();
  }
  ...
  public void testNonSplittingTextFile() throws IOException {
    SplittingOutputStream os = new SplittingOutputStream(
        getConf(), getWritePath(), "nonsplit-", 0, null);
    try {
    ...
    InputStream fis =
        new FileInputStream(new File(getWriteDir(), "nonsplit-00000"));
    try {
  ...
  public void testNonSplittingGzipFile() throws IOException {
    SplittingOutputStream os = new SplittingOutputStream(
        getConf(), getWritePath(), "nonsplit-", 0, new GzipCodec());
    SplittableBufferedWriter w = new SplittableBufferedWriter(os, true);
    try {
      w.allowSplit();
      w.write("This is a string!");
    ...
    verifyFileContents(new GZIPInputStream(new FileInputStream(
                           new File(getWriteDir(), "nonsplit-00000.gz"))),
                       expectedLines);
  }
  ...
  public void testSplittingTextFile() throws IOException {
    SplittingOutputStream os = new SplittingOutputStream(
        getConf(), getWritePath(), "split-", 10, null);
    try {
      SplittableBufferedWriter w = new SplittableBufferedWriter(os, true);
      try {
        w.allowSplit();
    ...
    InputStream fis =
        new FileInputStream(new File(getWriteDir(), "split-00000"));
    try {
  ...
  public void testSplittingGzipFile() throws IOException {
    SplittingOutputStream os = new SplittingOutputStream(
        getConf(), getWritePath(), "splitz-", 3, new GzipCodec());
    SplittableBufferedWriter w = new SplittableBufferedWriter(os, true);
    try {
      w.write("This is a string!");
      w.newLine();
    ...
    verifyFileContents(new GZIPInputStream(new FileInputStream(
                           new File(getWriteDir(), "splitz-00000.gz"))),
                       expectedLines0);
    ...
    verifyFileContents(new GZIPInputStream(new FileInputStream(
                           new File(getWriteDir(), "splitz-00001.gz"))),
                       expectedLines1);
  }
}
