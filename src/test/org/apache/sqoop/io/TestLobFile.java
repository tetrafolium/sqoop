...
private long[] writeClobFile(Path p, String codec, String... records)
throws Exception {
	if (fs.exists(p)) {
		fs.delete(p, false);
	}
...
throws Exception {
	...
	thrown.reportMissingExceptionWithMessage(
		"Expected IOException calling next after close");
	reader.next();
...
private void runClobFileTest(Path p, String codec, String... records)
throws Exception {
...
public void testSingleRecord() throws Exception {
	runClobFileTest(new Path(TEMP_BASE_DIR, "single.lob"), null,
...
public void testMultiRecords() throws Exception {
	runClobFileTest(
		new Path(TEMP_BASE_DIR, "multi.lob"), CodecMap.NONE,
...
public void testMultiIndexSegments() throws Exception {
	...
	runClobFileTest(
		new Path(TEMP_BASE_DIR, "multi-index.lob"), null,
		"this is the first record",
		"this is the second record. I assure you that this record is long.",
		"record number three", "last one in first index segment",
 ...
 */
private void runLineAndRecordTest(Path p, String firstLine, String... records)
throws Exception {
...
public void testVeryShortRead() throws Exception {
	...
	runLineAndRecordTest(p, FIRST_LINE, FIRST_LINE + "\n" + SECOND_LINE,
	                     RECORD2, RECORD3);
}
 ...
 *                            the record.
 * @param offset              the offset amount for each of the elements of
 *     the array.
 */
private void verifyBlobRecord(LobFile.Reader reader, long expectedDeclaredLen,
                              long expectedActualLen, int offset)
throws Exception {
 ...
 */
private void verifyBlobRecords(Path p, int numRecords, long declaredLen,
                               long actualLen) throws Exception {
...
public void testCompressedFile() throws Exception {
	...
	thrown.reportMissingExceptionWithMessage(
		"Expected UnsupportedCodecException for lzo");
	runCompressedTest(CodecMap.LZO);
}

@Test
public void
testCryptoOutputStreamClosingDoesNotThrowExceptionAndClosedProperly()
throws Exception {
	// Tests that closing CryptoOutputStream doesn't throw exception neither
	// with Java 7 nor with Java 8 For a detailed explanation see SQOOP-3243
	CryptoOutputStream cryptoOutputStream = createCryptoOutputStream();
	FSDataOutputStream wrappedCryptoOutputStream =
		new FSDataOutputStream(cryptoOutputStream, null);

	Path mockPath = spy(new Path("file://" + TEMP_BASE_DIR, "binary.lob"));
	FileSystem mockFileSystem =
		mock(FileSystem.class,
		     withSettings().defaultAnswer(CALLS_REAL_METHODS.get()));
...
public CryptoOutputStream createCryptoOutputStream() throws Exception {
	...
	CryptoOutputStream cryptoOutputStream = spy(new CryptoOutputStream(
							    fsDataOutputStream, new JceAesCtrCryptoCodec(), 512, BYTES, BYTES));
