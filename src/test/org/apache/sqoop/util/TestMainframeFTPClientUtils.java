...
public class TestMainframeFTPClientUtils {
  ...
  public void testListSequentialDatasets() {
    try {
      ...
      when(mockFTPClient.initiateListParsing(
               MainframeConfiguration.MAINFRAME_FTP_FILE_ENTRY_PARSER_CLASSNAME,
               ""))
          .thenReturn(mockFTPListParseEngine);
      when(mockFTPListParseEngine.hasNext()).thenReturn(true, false);
      when(mockFTPListParseEngine.getNext(25))
          .thenReturn(new FTPFile[] {file1, file2});
    } catch (IOException e) {
      fail("No IOException should be thrown!");
    }
    ...
    conf.set(MainframeConfiguration.MAINFRAME_INPUT_DATASET_NAME,
             "a.b.c.blah1");

    try {
      List<String> files =
          MainframeFTPClientUtils.listSequentialDatasets("a.b.c.blah1", conf);
  ...
  public void testListEmptySequentialDatasets() {
    ...
    try {
      ...
      List<String> files =
          MainframeFTPClientUtils.listSequentialDatasets(dsName, conf);
  ...
  public void testTrailingDotSequentialDatasets() {
    ...
    try {
      ...
      Assert.assertEquals(
          "Could not list datasets from a.b.c.blah1:java.io.IOException: Folder not found",
          ioeString);
    }
  }
  ...
  public void testPartitionedDatasetsShouldReturnAllFiles() {
    try {
      ...
      file2.setName("blah2");
      file2.setType(FTPFile.FILE_TYPE);
      // initiateListParsing should not be called here as it is a partitioned
      // dataset and default to listFiles()
  ...
  public void testFtpCommandsTwoCommandWithComma() {
    ...
    String[] expected =
        new String[] {"quote SITE RDW", "quote SITE RDW READTAPEFORMAT=V"};
    String[] cmds = MainframeFTPClientUtils.parseFtpCommands(inputString);
    assertArrayEquals(cmds, expected);
  }
