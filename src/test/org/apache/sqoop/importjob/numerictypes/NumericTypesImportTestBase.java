...
/**
 * This test covers the behavior of the Avro import for fixed point decimal
 * types, i.e. NUMBER, NUMERIC and DECIMAL.
 *
 * Oracle and Postgres store numbers without padding, while other DBs store them
 * padded with 0s.
 *
 * The features tested here affect two phases in Sqoop:
 * 1. Avro schema generation during avro and parquet import
 * Default precision and scale are used here to avoid issues with Oracle and
 * Postgres, as these don't return valid precision and scale if they weren't
 * specified in the table DDL.
 *
 * 2. Decimal padding during avro or parquet import
 * In case of Oracle and Postgres, Sqoop has to pad the values with 0s to avoid
 * errors.
 */
