...
public abstract class BaseSqoopTestCase {
  ...
  public static final Log LOG =
      LogFactory.getLog(BaseSqoopTestCase.class.getName());

  public static boolean isOnPhysicalCluster() { return onPhysicalCluster; }
   ...
   */
  protected String getTablePrefix() { return "SQOOP_TABLE_"; }
  ...
  protected String getWarehouseDir() { return getLocalWarehouseDir(); }

  private String[] colNames;
  protected String[] getColNames() { return colNames; }
  ...
  protected HsqldbTestServer getTestServer() { return testServer; }

  protected ConnManager getManager() { return manager; }

  protected void setManager(ConnManager manager) { this.manager = manager; }
  ...
  protected void incrementTableNum() { tableNum++; }

  /**
   * @return true if we need an in-memory database to run these tests.
   */
  protected boolean useHsqldbTestServer() { return true; }
   ...
   */
  protected String getConnectString() { return HsqldbTestServer.getUrl(); }

  /**
   * @return a Configuration object used to configure tests.
   */
  protected Configuration getConf() { return new Configuration(); }
  ...
  public void setUp() {
    ...
    setOnPhysicalCluster(!CommonArgs.LOCAL_FS.equals(
        System.getProperty(CommonArgs.FS_DEFAULT_NAME)));
    incrementTableNum();
  ...
  protected String getColName(int i) { return BASE_COL_NAME + i; }
  ...
  protected void dropTableIfExists(String table) throws SQLException {
    ...
    PreparedStatement statement = conn.prepareStatement(
        dropStatement, ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY);
    try {
  ...
  protected void createTableWithColTypesAndNames(String[] colNames,
                                                 String[] colTypes,
                                                 List<Object> record) {
    createTableWithColTypesAndNames(getTableName(), colNames, colTypes,
                                    toStringArray(record));
  }

  /**
   * Create a table with a set of columns with their names and add a row of
   * values.
    ...
    try {
      try {
        ...
        for (int i = 0; i < colTypes.length; i++) {
          columnDefStr += manager.escapeColName(colNames[i].toUpperCase()) +
                          " " + colTypes[i];
        ...
        createTableStr = "CREATE TABLE " +
                         manager.escapeTableName(newTableName) + "(" +
                         columnDefStr + ")";
        LOG.info("Creating table: " + createTableStr);
        statement =
            conn.prepareStatement(createTableStr, ResultSet.TYPE_FORWARD_ONLY,
      ...
      for (int count = 0; vals != null && count < vals.length / colTypes.length;
        ...
        try {
          String insertValsStr = "INSERT INTO " +
                                 manager.escapeTableName(newTableName) + "(" +
                                 columnListStr + ")"
                                 + " VALUES(" + valueListStr + ")";
          LOG.info("Inserting values: " + insertValsStr);
          statement =
              conn.prepareStatement(insertValsStr, ResultSet.TYPE_FORWARD_ONLY,
                                    ResultSet.CONCUR_READ_ONLY);
          statement.executeUpdate();
        } catch (SQLException sqlException) {
  ...
  protected void insertRecordsIntoTable(String[] colTypes,
  ...
  protected void insertRecordsIntoTableWithColTypesAndNames(
      String[] columns, String[] colTypes, List<List<Object>> records) {
    for (List<Object> record : records) {
      insertIntoTable(columns, colTypes, record);
    }
  }

  protected void insertIntoTable(String[] columns, String[] colTypes,
                                 List<Object> record) {
  ...
  protected void insertIntoTable(String[] columns, String[] colTypes,
    ...
    try {
      ...
      for (int count = 0; vals != null && count < vals.length / colTypes.length;
        ...
        try {
          String insertValsStr = "INSERT INTO " +
                                 manager.escapeTableName(getTableName()) + "(" +
                                 columnListStr + ")"
                                 + " VALUES(" + valueListStr + ")";
          LOG.info("Inserting values: " + insertValsStr);
          statement =
    ...
    try {
      ...
      for (int count = 0; vals != null && count < vals.length / colNames.length;
        ...
        for (int i = 1; i < colNames.length; i++) {
          updateStr += manager.escapeColName(colNames[i].toUpperCase()) +
                       " = " + vals[count * colNames.length + i];
        ...
        try {
          String updateValsStr = "UPDATE " +
                                 manager.escapeTableName(getTableName()) +
                                 " SET " + updateStr;
          LOG.info("updating values: " + updateValsStr);
          statement =
              conn.prepareStatement(updateValsStr, ResultSet.TYPE_FORWARD_ONLY,
                                    ResultSet.CONCUR_READ_ONLY);
          statement.executeUpdate();
        } catch (SQLException sqlException) {
  ...
  protected void createTableWithColTypes(String[] colTypes,
                                         List<Object> record) {
    createTableWithColTypes(colTypes, toStringArray(record));
  }

  protected void createTableWithRecords(String[] colTypes,
                                        List<List<Object>> records) {
  ...
  protected void createTableWithRecordsWithColTypesAndNames(
      String[] columns, String[] colTypes, List<List<Object>> records) {
