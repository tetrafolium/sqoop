...
public abstract class ExportJobTestCase extends BaseSqoopTestCase {
  ...
  public static final Log LOG =
      LogFactory.getLog(ExportJobTestCase.class.getName());
   ...
   */
  protected int getMaxRowsPerStatement() { return 1; }
  ...
  protected boolean usesSQLtable() { return true; }

  /** When exporting text columns, what should the text contain? */
  protected String getMsgPrefix() { return "textfield"; }
  ...
  protected void verifyExport(int expectedNumRecords, Connection conn)
    ...
    PreparedStatement statement = conn.prepareStatement(
        "SELECT COUNT(*) FROM " + getTableName(), ResultSet.TYPE_FORWARD_ONLY,
    ...
    statement = conn.prepareStatement(
        "SELECT \"MSG\" FROM " + getTableName() + " WHERE \"ID\" = " + minVal,
        ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY);
    String minMsg = "";
    try {
      rs = statement.executeQuery();
    ...
    statement = conn.prepareStatement(
        "SELECT \"MSG\" FROM " + getTableName() + " WHERE \"ID\" = " + maxVal,
        ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY);
    String maxMsg = "";
    try {
      rs = statement.executeQuery();
  ...
  protected void verifyExport(String expectedValue)
    ...
    // Check that we got back the correct number of records.
    PreparedStatement statement = conn.prepareStatement(
        "SELECT MSG FROM " + getTableName(), ResultSet.TYPE_FORWARD_ONLY,
        ResultSet.CONCUR_READ_ONLY);
