...
public static final Log LOG =
	LogFactory.getLog(AvroTestUtils.class.getName());
...
public static ArgumentArrayBuilder
getBuilderForAvroPaddingTest(BaseSqoopTestCase testCase) {
...
public static void verify(String[] expectedResults, Configuration conf,
                          Path tablePath) {
...
private static void readAndVerify(String[] expectedResults,
                                  Configuration conf, Path outputFile) {
	try (DataFileReader<GenericRecord> reader = read(outputFile, conf)) {
		GenericRecord record;
		if (!reader.hasNext() && expectedResults != null &&
 ...
 * @throws IOException
 */
public static DataFileReader<GenericRecord> read(Path filename,
                                                 Configuration conf)
throws IOException {
