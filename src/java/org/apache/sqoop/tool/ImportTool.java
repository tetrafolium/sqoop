...
public class ImportTool extends BaseSqoopTool {
  ...
  public ImportTool() { this("import", false); }

  public ImportTool(String toolName, boolean allTables) {
    this(toolName, new CodeGenTool(), allTables, new HiveClientFactory());
  }

  public ImportTool(String toolName, CodeGenTool codeGenerator,
                    boolean allTables, HiveClientFactory hiveClientFactory) {
    super(toolName);
  ...
  private void loadJars(Configuration conf, String ormJarFile,
    ...
    if (ConfigurationHelper.isLocalJobTracker(conf)) {
      ...
      this.prevClassLoader =
          ClassLoaderStack.addJarFile(ormJarFile, tableClassName);
    }
  }
  ...
  private boolean initIncrementalConstraints(SqoopOptions options,
    ...
    case AppendRows:
      try {
        ...
        if (isDateTimeColumn(checkColumnType)) {
          nextIncrementalValue = (nextVal == null)
                                     ? null
                                     : manager.datetimeToQueryString(
    ...
    case DateLastModified:
      if (shouldCheckExistingOutputDirectory(options)) {
        ...
        if (fs.exists(outputPath)) {
          throw new ImportException(
              "--" + MERGE_KEY_ARG + " or "
              + "--" + APPEND_ARG + " is required when using --" +
              this.INCREMENT_TYPE_ARG +
              " lastmodified and the output directory exists.");
        }
      }
      checkColumnType =
          manager.getColumnTypes(options.getTableName(), options.getSqlQuery())
              .get(options.getIncrementalTestColumn());
      nextVal = manager.getCurrentDbTimestamp();
      if (null == nextVal) {
        throw new IOException("Could not get current time from database");
      }
      nextIncrementalValue =
          manager.datetimeToQueryString(nextVal.toString(), checkColumnType);
      break;
    default:
    ...
    if (isDateTimeColumn(checkColumnType) && null != prevEndpoint &&
      ...
      // around, here.
      prevEndpoint =
          manager.datetimeToQueryString(prevEndpoint, checkColumnType);
    }

    String checkColName =
        manager.escapeColName(options.getIncrementalTestColumn());
    ...
    if (options.getTableName() != null) {
      ...
      String newQuery =
          options.getSqlQuery().replace("$CONDITIONS", sb.toString());
   ...
   */
  protected void lastModifiedMerge(SqoopOptions options,
                                   ImportJobContext context)
      throws IOException {
    ...
    if (fs.exists(context.getDestination())) {
      ...
      if (fs.exists(userDestDir)) {
        ...
        if (!context.getConnManager().isORMFacilitySelfManaged()) {
          tableClassName = new TableClassName(options).getClassForTable(
        ...
        // Merge to temporary directory so that original directory remains
        // intact.
        ...
        if (options.getFileLayout() == SqoopOptions.FileLayout.ParquetFile) {
          loadJars(options.getConf(), context.getJarFile(),
                   ClassWriter.toJavaIdentifier("codegen_" +
                                                context.getTableName()));
        } else {
          loadJars(options.getConf(), context.getJarFile(),
                   context.getTableName());
        ...
        ParquetMergeJobConfigurator parquetMergeJobConfigurator =
            getParquetJobConfigurator(options)
                .createParquetMergeJobConfigurator();
        ...
        LOG.info("Moving data from temporary directory " +
                 context.getDestination() + " to final destination " +
                 userDestDir);
        if (!fs.rename(context.getDestination(), userDestDir)) {
          throw new RuntimeException(
              "Couldn't move data from temporary directory " +
              context.getDestination() + " to final destination " +
              userDestDir);
        }
      }
  ...
  protected boolean importTable(SqoopOptions options)
    ...
    ImportJobContext context = new ImportJobContext(
        options.getTableName(), jarFile, options, outputPath);
    ...
    if (options.isAppendMode()) {
      ...
      app.append();
               ...
               SqoopOptions.IncrementalMode.DateLastModified) {
  ...
  private Path getOutputPath(SqoopOptions options, String tableName) {
    return getOutputPath(
        ...
        options.isAppendMode() ||
            options.getIncrementalMode().equals(
                SqoopOptions.IncrementalMode.DateLastModified));
  }
  ...
  protected RelatedOptions getImportOptions() {
    ...
    if (!allTables) {
      ...
      importOpts.addOption(
          OptionBuilder.withArgName("column")
              .hasArg()
              .withDescription("Key column to use to join results")
    ...
    importOpts.addOption(
        OptionBuilder.withDescription("Imports data to Parquet files")
            ...
            .create());
    importOpts.addOption(
        OptionBuilder.withDescription("Imports data to Binary files")
    ...
    importOpts.addOption(
        OptionBuilder.withArgName("name")
            ...
            .create());
    importOpts.addOption(OptionBuilder.withDescription("Enable compression")
    ...
    importOpts.addOption(
        OptionBuilder.withArgName("reset-mappers")
            .withDescription(
                "Reset the number of mappers to one mapper if no split key available")
    ...
    if (in.hasOption(INCREMENT_TYPE_ARG)) {
      ...
      if ("append".equals(incrementalTypeStr)) {
        ...
        throw new InvalidOptionsException(
            "Unknown incremental import mode: " + incrementalTypeStr +
    ...
    try {
      ...
      if (!allTables) {
        ...
        if (in.hasOption(SPLIT_LIMIT_ARG)) {
          out.setSplitLimit(
              Integer.parseInt(in.getOptionValue(SPLIT_LIMIT_ARG)));
        }
      ...
      if (in.hasOption(DIRECT_SPLIT_SIZE_ARG)) {
        out.setDirectSplitSize(
            Long.parseLong(in.getOptionValue(DIRECT_SPLIT_SIZE_ARG)));
      }

      if (in.hasOption(INLINE_LOB_LIMIT_ARG)) {
        out.setInlineLobLimit(
            Long.parseLong(in.getOptionValue(INLINE_LOB_LIMIT_ARG)));
      }
  ...
  protected void validateImportOptions(SqoopOptions options)
    ...
    if (!allTables && options.getTableName() == null &&
      ...
      throw new InvalidOptionsException("Jar specified with --jar-file, but no "
                                        + "class specified with --class-name." +
                                        HELP_STR);
      ...
      throw new InvalidOptionsException(
          ...
          HELP_STR);
               ...
               options.getSqlQuery() != null) {
      throw new InvalidOptionsException("Cannot specify --" + SQL_QUERY_ARG +
                                        " and --table together." + HELP_STR);
  ...
  void validateDirectImportOptions(SqoopOptions options)
      throws InvalidOptionsException {
  ...
  void validateDirectDropHiveDelimOption(SqoopOptions options)
      throws InvalidOptionsException {
  ...
  void validateDirectMysqlOptions(SqoopOptions options)
      throws InvalidOptionsException {
    ...
    if (options.getNullStringValue() != null ||
        options.getNullNonStringValue() != null) {
      throw new InvalidOptionsException(
  ...
  private void validateIncrementalOptions(SqoopOptions options)
    ...
    if (options.getIncrementalMode() ==
            SqoopOptions.IncrementalMode.DateLastModified &&
        options.getFileLayout() == SqoopOptions.FileLayout.AvroDataFile) {
      throw new InvalidOptionsException(
          "--" + INCREMENT_TYPE_ARG +
          " lastmodified cannot be used in conjunction with --" +
  ...
  private boolean isIncrementalModeAppendOrLastmodified(SqoopOptions options) {
    return options.getIncrementalMode() ==
        SqoopOptions.IncrementalMode.AppendRows ||
        options.getIncrementalMode() ==
            SqoopOptions.IncrementalMode.DateLastModified;
  }
  ...
  boolean shouldCheckExistingOutputDirectory(SqoopOptions options) {
    return options.getMergeKeyCol() == null && !options.isAppendMode() &&
        !isHBaseImport(options);
  }
  ...
  private boolean isHiveImportNeeded(SqoopOptions options) {
    ...
    ParquetJobConfiguratorFactory parquetJobConfigurator =
        getParquetJobConfigurator(options);
    return parquetJobConfigurator.createParquetImportJobConfigurator()
        .isHiveImportNeeded();
  }
}
