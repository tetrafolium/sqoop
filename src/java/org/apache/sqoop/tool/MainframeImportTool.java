...
public class MainframeImportTool extends ImportTool {
  ...
  public MainframeImportTool() { super("import-mainframe", false); }
  ...
  protected RelatedOptions getImportOptions() {
    ...
    importOpts.addOption(OptionBuilder.withArgName("Dataset name")
                             .hasArg()
                             .withDescription("Datasets to import")
                             .withLongOpt(DS_ARG)
                             .create());
    ...
    importOpts.addOption(OptionBuilder.withArgName("dir")
                             .hasArg()
                             .withDescription("HDFS plain file destination")
                             .withLongOpt(TARGET_DIR_ARG)
                             .create());
    importOpts.addOption(
        OptionBuilder.withArgName("Dataset type")
            .hasArg()
            .withDescription(
                "Dataset type (p=partitioned data set|s=sequential data set|g=GDG)")
            .withLongOpt(DS_TYPE_ARG)
            .create());
    importOpts.addOption(OptionBuilder.withArgName("Dataset is on tape")
                             .hasArg()
                             .withDescription("Dataset is on tape (true|false)")
    ...
    importOpts.addOption(
        OptionBuilder.withArgName("dir")
            .hasArg()
            .withDescription("HDFS parent for file destination")
    ...
    importOpts.addOption(
        OptionBuilder.withDescription("Imports data as plain text (default)")
            ...
            .create());
    importOpts.addOption(OptionBuilder.withDescription("Imports data as binary")
                             .withLongOpt(FMT_BINARYFILE_ARG)
                             .create());
    importOpts.addOption(
        OptionBuilder.hasArg()
            .withDescription(
                "Sets buffer size for binary import in bytes (default=32kB)")
            .withLongOpt(BUFFERSIZE_ARG)
            .create());
    importOpts.addOption(
        OptionBuilder
            .withArgName(
                "Comma separated FTP commands issued before FTP transfer")
            .hasArg()
            .withDescription("Additional FTP commands issued before transfer")
  ...
  public void applyOptions(CommandLine in, SqoopOptions out)
    ...
    if (in.hasOption(DS_TYPE_ARG)) {
      ...
      out.setMainframeInputDatasetType(
    ...
    if (in.hasOption(FMT_BINARYFILE_ARG)) {
      out.setMainframeFtpTransferMode(
    ...
    if (in.hasOption(BUFFERSIZE_ARG)) {
      ...
      out.setBufferSize(MainframeConfiguration
                            .MAINFRAME_FTP_TRANSFER_BINARY_DEFAULT_BUFFER_SIZE);
    }
  ...
  protected void validateImportOptions(SqoopOptions options)
    ...
    if (!dsType.equals(
            MainframeConfiguration.MAINFRAME_INPUT_DATASET_TYPE_PARTITIONED) &&
        !dsType.equals(
            MainframeConfiguration.MAINFRAME_INPUT_DATASET_TYPE_SEQUENTIAL) &&
        !dsType.equals(
            MainframeConfiguration.MAINFRAME_INPUT_DATASET_TYPE_GDG)) {
      throw new InvalidOptionsException("--" + DS_TYPE_ARG +
                                        " specified is invalid. " + HELP_STR);
    }
    Boolean dsTape = options.getMainframeInputDatasetTape();
    if (dsTape == null && dsTape != true && dsTape != false) {
      throw new InvalidOptionsException("--" + DS_TAPE_ARG +
                                        " specified is invalid. " + HELP_STR);
    }
    ...
    if (SqoopOptions.FileLayout.BinaryFile.equals(options.getFileLayout()) &&
        StringUtils.isEmpty(options.getMainframeInputDatasetName())) {
      throw new InvalidOptionsException(
          "--as-binaryfile should only be used with import-mainframe module.");
    }

    // only allow buffer size to be set different to default when binary file is
    // selected in any case, if --as-binaryfile isn't selected, --buffersize
    // parameter is harmless
    if (!SqoopOptions.FileLayout.BinaryFile.equals(options.getFileLayout()) &&
        !MainframeConfiguration
             .MAINFRAME_FTP_TRANSFER_BINARY_DEFAULT_BUFFER_SIZE.equals(
                 options.getBufferSize())) {
      throw new InvalidOptionsException(
          "--buffersize should only be used with --as-binaryfile parameter.");
    }
