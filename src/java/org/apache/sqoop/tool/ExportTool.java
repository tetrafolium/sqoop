...
public static final String NOT_SUPPORTED_FILE_FORMAT_ERROR_MSG =
	"Please note that the export tool "
	+
...
private final List<String> unsupportedArguments =
	Collections.unmodifiableList(Arrays.asList(
					     BaseSqoopTool.FMT_PARQUETFILE_ARG, BaseSqoopTool.FMT_AVRODATAFILE_ARG,
...
throws ExportException, IOException {
	...
	ExportJobContext context =
		new ExportJobContext(tableName, jarFile, options);
...
protected RelatedOptions getExportOptions() {
	...
	exportOpts.addOption(
		...
		.hasArg()
		.withDescription("Use 'n' map tasks to export in parallel")
	...
	exportOpts.addOption(OptionBuilder.withArgName("table-name")
	                     .hasArg()
	                     .withDescription("Intermediate staging table")
	...
	exportOpts.addOption(
		...
		.hasArg()
		.withDescription(
			"Specifies how updates are performed when "
...
throws InvalidOptionsException {
	if (options.getTableName() == null && options.getCall() == null) {
		...
		throw new InvalidOptionsException(
			      "Staging table cannot be the same as "
		...
		throw new InvalidOptionsException("Option the use a staging table is "
		                                  +
		                                  "specified as well as a call option.");
	} else if (options.getCall() != null && options.getUpdateKeyCol() != null) {
		throw new InvalidOptionsException("Option to call a stored procedure"
...
void vaildateDirectExportOptions(SqoopOptions options)
throws InvalidOptionsException {
	if (options.isDirect()) {
...
public void validateDirectMysqlOptions(SqoopOptions options)
throws InvalidOptionsException {
	if (!MYSQL.isTheManagerTypeOf(options)) {
		return;
	}
	if (options.getInNullStringValue() != null ||
	    options.getInNullNonStringValue() != null) {
		throw new InvalidOptionsException(
...
throws InvalidOptionsException {
	if (in.hasOption(UPDATE_MODE_ARG)) {
		...
		if ("updateonly".equals(updateTypeStr)) {
			...
			throw new InvalidOptionsException(
				      "Unknown new update mode: " + updateTypeStr +
