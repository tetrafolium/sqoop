...
public class DirectNetezzaManager extends NetezzaManager {
  ...
  public static final Log LOG =
      LogFactory.getLog(DirectNetezzaManager.class.getName());
  ...
  private void checkTable() throws IOException, ExportException {
    ...
    try {
      try {
        ...
        if (!rs.next()) {
          String message =
              tableName + " is not a valid Netezza table.  "
      ...
      throw new IOException("SQL exception checking table " + sqle.getMessage(),
                            sqle);
    }
  }
  ...
  public void importTable(org.apache.sqoop.manager.ImportJobContext context)
    ...
    if (null == tableName) {
      LOG.error(
          "Netezza external table import does not support query imports.");
      LOG.error("Do not use --direct and --query together for Netezza.");
      throw new IOException(
          "Null tableName for Netezza external table import.");
    }
  ...
  protected RelatedOptions getNetezzaExtraOpts() {
    ...
    netezzaOpts.addOption(OptionBuilder.withArgName(NETEZZA_ERROR_THRESHOLD_OPT)
                              .hasArg()
                              .withDescription("Error threshold for the job")
                              .withLongOpt(NETEZZA_ERROR_THRESHOLD_LONG_ARG)
                              .create());
    netezzaOpts.addOption(OptionBuilder.withArgName(NETEZZA_LOG_DIR_OPT)
                              .hasArg()
                              .withDescription("Netezza log directory")
                              .withLongOpt(NETEZZA_LOG_DIR_LONG_ARG)
                              .create());
    netezzaOpts.addOption(OptionBuilder.withArgName(NETEZZA_CTRL_CHARS_OPT)
                              .withDescription("Allow control chars in data")
                              .withLongOpt(NETEZZA_CTRL_CHARS_LONG_ARG)
                              .create());
    netezzaOpts.addOption(
        OptionBuilder.withArgName(NETEZZA_TRUNC_STRING_OPT)
            .withDescription("Truncate string to declared storage size")
            .withLongOpt(NETEZZA_TRUNC_STRING_LONG_ARG)
            .create());
    netezzaOpts.addOption(
        OptionBuilder.withArgName(NETEZZA_CRIN_STRING_OPT)
            .withDescription("Truncate string to declared storage size")
            .withLongOpt(NETEZZA_CRIN_STRING_LONG_ARG)
            .create());
    netezzaOpts.addOption(
        OptionBuilder.withArgName(NETEZZA_IGNORE_ZERO_OPT)
            .withDescription("Truncate string to declared storage size")
            .withLongOpt(NETEZZA_IGNORE_ZERO_LONG_ARG)
            .create());
    netezzaOpts.addOption(OptionBuilder.withArgName(NETEZZA_TABLE_ENCODING_OPT)
                              .hasArg()
                              .withDescription("Table encoding")
                              .withLongOpt(NETEZZA_TABLE_ENCODING_LONG_ARG)
                              .create());
    return netezzaOpts;
  }
  ...
  private void handleNetezzaExtraArgs(SqoopOptions opts) throws ParseException {
    ...
    if (cmdLine.hasOption(NETEZZA_ERROR_THRESHOLD_LONG_ARG)) {
      int threshold = Integer.parseInt(
          cmdLine.getOptionValue(NETEZZA_ERROR_THRESHOLD_LONG_ARG));
