...
private static final OraOopLog ORAOOP_LOG =
	OraOopLogFactory.getLog("org.apache.sqoop.manager.oracle");
private static final OraOopLog LOG =
	OraOopLogFactory.getLog(OraOopManagerFactory.class.getName());
...
public ConnManager accept(JobData jobData) {
	...
	OraOopUtilities.enableDebugLoggingIfRequired(
		jobData.getSqoopOptions().getConf());
	...
	if (jobData != null) {
		...
		if (connectString != null &&
			...
			OraOopUtilities.rememberSqoopJobType(
				jobType, jobData.getSqoopOptions().getConf());
			...
			case IMPORT:
				if (isNumberOfImportMappersOkay(sqoopOptions) &&
					...
					try {
						...
						if (isSqoopTableAnOracleTable(
							    connection, sqoopOptions.getUsername(),
							    oraOopConnManager.getOracleTableContext())) {

							// OraOop will not accept responsibility for an Index
							// Organized Table (IOT)...
							if (!isSqoopTableAnIndexOrganizedTable(
								    connection,
								    oraOopConnManager.getOracleTableContext())) {
						...
						throw new RuntimeException(
							      String.format(
								      ...
								      + "Error:%s",
								      sqoopOptions.getConnectString(), ex.getMessage()),
							      ex);
					}
				}
				break;
			...
			if (result != null) {
				...
				sqoopOptions.getConf().set(OraOopConstants.ORACLE_SESSION_ACTION_NAME,
				...
				try {
					if (sqoopOptions.getConf().getBoolean(
						...
						long scn = sqoopOptions.getConf().getLong(
...
private void setMapperConnectionDetails(OraOopConnManager oraOopConnManager,
	...
	if (OraOopUtilities.oracleJdbcUrlGenerationDisabled(
		    jobData.getSqoopOptions().getConf())) {
		LOG.info(String.format(
	...
	if (activeInstances != null) {
		...
		if (activeInstances.size() <
		    OraOopUtilities
		    .getMinNumberOfOracleRacActiveInstancesForDynamicJdbcUrlUse(
			    jobData.getSqoopOptions().getConf())) {
			LOG.info(String.format(
	...
	if (generateRacBasedJdbcUrls) {
		...
		String oracleServiceName = OraOopUtilities.getOracleServiceName(
			jobData.getSqoopOptions().getConf());

		// Generate JDBC URLs for each of the mappers...
		if (!oracleServiceName.isEmpty()) {
			if (!generateRacJdbcConnectionUrlsByServiceName(
				    jdbcHost, jdbcPort, oracleServiceName, jobData)) {
				throw new RuntimeException(String.format(
								   "Unable to connect to the Oracle database at %s "
								   + "via the service name \"%s\".",
								   jobData.getSqoopOptions().getConnectString(), oracleServiceName));
			}
		} else {
		...
		generateJdbcConnectionUrlsByTnsnameSidOrService(
			jdbcHost, jdbcPort, jdbcSid, jdbcService, jdbcTnsName, jobData);
	}
}

private void generateJdbcConnectionUrlsByTnsnameSidOrService(
	String hostName, int port, String sid, String serviceName, String tnsName,
	JobData jobData) {
	...
	if (tnsName != null && !tnsName.isEmpty()) {
		...
		jdbcUrl = OraOopUtilities.generateOracleServiceNameJdbcUrl(hostName, port,
		                                                           serviceName);
	}
	...
	// which one to use...
	for (int idxMapper = 0;
	     idxMapper < jobData.getSqoopOptions().getNumMappers(); idxMapper++) {
...
private boolean generateRacJdbcConnectionUrlsByServiceName(String hostName,
	...
	String jdbcUrl = OraOopUtilities.generateOracleServiceNameJdbcUrl(
		hostName, port, serviceName);

	if (testDynamicallyGeneratedOracleRacInstanceConnection(
		    jdbcUrl, jobData.getSqoopOptions().getUsername(),
...
private void generateJdbcConnectionUrlsByActiveInstance(
	...
	for (OracleActiveInstance activeInstance : activeInstances) {
		...
		OraOopUtilities
		.JdbcOracleThinConnection jdbcActiveInstanceThinConnection =
			new OraOopUtilities.JdbcOracleThinConnection(
				activeInstance.getHostName(), jdbcPort,
		...
		if (testDynamicallyGeneratedOracleRacInstanceConnection(
			    ...
			    jobData.getSqoopOptions().getPassword(),
			    jobData.getSqoopOptions().getConnectionParams(), true,
			    activeInstance.getInstanceName())) {
	...
	// make use of them...
	if (jdbcOracleActiveThinConnections.size() <
	    OraOopUtilities
	    .getMinNumberOfOracleRacActiveInstancesForDynamicJdbcUrlUse(
		    jobData.getSqoopOptions().getConf())) {
		LOG.info(String.format(
	...
	try {
		Connection testConnection =
			OracleConnectionFactory.createOracleJdbcConnection(
				OraOopConstants.ORACLE_JDBC_DRIVER_CLASS, url, userName, password,
				additionalProps);

		// Show the system time on each instance...
		if (showInstanceSysTimestamp) {
			LOG.info(
				String.format("\tDatabase time on %s is %s", instanceDescription,
...
private boolean isOraOopEnabled(SqoopOptions sqoopOptions) {
	...
	String oraOopDisabled = sqoopOptions.getConf()
	                        .get(OraOopConstants.ORAOOP_DISABLED, "false")
	                        .toLowerCase();
...
private OraOopConstants.Sqoop.Tool getSqoopJobType(JobData jobData) {
	...
	try {
		...
		LOG.debug(
			String.format("The Sqoop tool name \"%s\" is not supported by OraOop",
			              sqoopToolName),
			ex);
	}
	return result;
...
private boolean isNumberOfImportMappersOkay(SqoopOptions sqoopOptions) {
	...
	boolean result =
		(sqoopOptions.getNumMappers() >=
		 OraOopUtilities.getMinNumberOfImportMappersAcceptedByOraOop(
			 sqoopOptions.getConf()));
...
private boolean isNumberOfExportMappersOkay(SqoopOptions sqoopOptions) {
	...
	boolean result =
		(sqoopOptions.getNumMappers() >=
		 OraOopUtilities.getMinNumberOfExportMappersAcceptedByOraOop(
			 sqoopOptions.getConf()));
...
private boolean isSqoopTableAnOracleTable(Connection connection,
	...
	try {
		...
		OracleTable oracleTable = OraOopOracleQueries.getTable(
			connection, tableContext.getSchema(), tableContext.getName());
		if (oracleTable != null) {
			return true;
		}
	...
	boolean result = oracleObjectType.equalsIgnoreCase(
		OraOopConstants.Oracle.OBJECT_TYPE_TABLE);

	if (!result) {
		LOG.info(String.format("%s will not process this sqoop connection, "
		                       + "as %s is not an Oracle table, it's a %s.",
		                       OraOopConstants.ORAOOP_PRODUCT_NAME,
		                       tableContext.toString(), oracleObjectType));
	}
...
private boolean isSqoopTableAnIndexOrganizedTable(Connection connection,
	...
	boolean result = false;
	try {
		result = OraOopOracleQueries.isTableAnIndexOrganizedTable(connection,
		                                                          tableContext);
		return result;
	} catch (SQLException ex) {
		LOG.warn(String.format(
				 ...
				 + " table.\nError:\n"
				 + "%s",
				 tableContext.toString(), ex.getMessage()));
	}
	return result;
}
...
private boolean isSqoopImportIncremental(JobData jobData) {
	...
	if (result) {
		LOG.info(
			String.format("%1$s will not process this sqoop connection, "
...
private void createAnyRequiredOracleObjects(
	SqoopOptions sqoopOptions, Connection connection,
...
throws SQLException {
	...
	String sysDateStr = OraOopOracleQueries.oraDATEToString(
		sysDateTime, "yyyy-mm-dd hh24:mi:ss");
	OraOopUtilities.rememberOracleDateTime(
		conf, OraOopConstants.ORAOOP_JOB_SYSDATE, sysDateStr);

	checkForOldOraOopTemporaryOracleTables(
		connection, sysDateTime,
		OraOopOracleQueries.getCurrentSchema(connection),
		messagesToDisplayAfterWelcome);

	// Store the actual partition value, so the N mappers know what value to
	// insert...
	String partitionValue = OraOopOracleQueries.oraDATEToString(
		sysDateTime, OraOopConstants.ORAOOP_EXPORT_PARTITION_DATE_FORMAT);
	conf.set(OraOopConstants.ORAOOP_EXPORT_PARTITION_DATE_VALUE,
	         partitionValue);

	// Generate the (22 character) partition name...
	String partitionName =
		OraOopUtilities.createExportTablePartitionNameFromOracleTimestamp(
			sysDateTime);
	...
	boolean noLoggingOnNewTable = conf.getBoolean(
		OraOopConstants.ORAOOP_EXPORT_CREATE_TABLE_NO_LOGGING, false);
	...
	if (OraOopUtilities.userWantsToCreatePartitionedExportTableFromTemplate(
		    conf) ||
	    OraOopUtilities.userWantsToCreateNonPartitionedExportTableFromTemplate(
		    conf)) {
		...
		// table...
		String templateTableObjectType = OraOopOracleQueries.getOracleObjectType(
			connection, templateTableContext);
		...
		String newTableObjectType = OraOopOracleQueries.getOracleObjectType(
			connection, oraOopConnManager.getOracleTableContext());
		...
		if (updateKeyCol != null && !updateKeyCol.isEmpty()) {
			...
			if (!OraOopOracleQueries.doesIndexOnColumnsExist(
				    connection, oraOopConnManager.getOracleTableContext(),
				    updateKeyColumns)) {
				String msg = String.format(
					"\n**************************************************************"
				...
				messagesToDisplayAfterWelcome.add(
					new OraOopLogMessage(OraOopConstants.Logging.Level.WARN, msg));
			}
		}
	}
	...
	if (OraOopUtilities.userWantsToCreatePartitionedExportTableFromTemplate(
		...
		String[] subPartitionNames =
			OraOopUtilities.generateExportTableSubPartitionNames(
				numMappers, sysDateTime, conf);
	...
	/* ===================================== */
	if (OraOopUtilities.userWantsToCreateNonPartitionedExportTableFromTemplate(
		    conf)) {

		String tableStorageClause =
			OraOopUtilities.getExportTableStorageClause(conf);

		OraOopOracleQueries.createExportTableFromTemplate(
			connection, oraOopConnManager.getOracleTableContext(),
			tableStorageClause, templateTableContext, noLoggingOnNewTable);
		return;
	}
	...
	OracleTablePartitions tablePartitions = OraOopOracleQueries.getPartitions(
		connection, oraOopConnManager.getOracleTableContext());
	// Find any partition name starting with "ORAOOP_"...
	OracleTablePartition oraOopPartition = tablePartitions.findPartitionByRegEx(
	...
	if (oraOopPartition != null) {
		...
		OraOopOracleQueries.createMoreExportTablePartitions(
			connection, oraOopConnManager.getOracleTableContext(), partitionName,
...
private void checkForOldOraOopTemporaryOracleTables(
	Connection connection, Object sysDateTime, String schema,
	List<OraOopLogMessage> messagesToDisplayAfterWelcome) {
	...
	try {
		...
		String generatedTableName =
			OraOopUtilities
			.generateExportTableMapperTableName(0, sysDateTime, schema)
			.getName();
		...
		List<OracleTable> tables = OraOopOracleQueries.getTablesWithTableNameLike(
			connection, schema, generatedTableName);
