...
public final class OraOopUtilities {
  ...
  protected static final OraOopLog LOG =
      OraOopLogFactory.getLog(OraOopUtilities.class.getName());
  ...
  public static OracleTable
  decodeOracleTableName(String oracleConnectionUserName, String tableStr) {
    ...
    case 1: // <- table or schema.table
      ...
      case 2: // <- 1 period
        ...
        break;
      default:
        LOG.debug(String.format("Unable to decode the table name (displayed in "
                                    + "double quotes): \"%s\"",
                                tableStr));
        throw new RuntimeException(
            String.format("Unable to decode the table name: %s", tableStr));
      }
      break;
    ...
    default:
      LOG.debug(
          String.format("Unable to decode the table name (displayed in double "
                            + "quotes): \"%s\"",
                        tableStr));
      throw new RuntimeException(
          String.format("Unable to decode the table name: %s", tableStr));
  ...
  decodeOracleTableName(String oracleConnectionUserName, String tableStr,
    ...
    if (result.getSchema() == null || result.getName() == null) {
      ...
      LOG.debug(
          String.format("The Oracle table context has been derived from:\n"
                            + "\toracleConnectionUserName = %s\n"
                            + "\ttableStr = %s\n"
                            + "\tas:\n"
                            + "\towner : %s\n"
                            + "\ttable : %s",
                        oracleConnectionUserName, tableStr, result.getSchema(),
                        result.getName()));
  ...
  oracleJdbcUrlGenerationDisabled(org.apache.hadoop.conf.Configuration conf) {
  ...
  public static boolean
  enableDebugLoggingIfRequired(org.apache.hadoop.conf.Configuration conf) {

    boolean result = false;

    try {

      Level desiredOraOopLoggingLevel = Level.toLevel(
          conf.get(OraOopConstants.ORAOOP_LOGGING_LEVEL), Level.INFO);
  ...
  public static int getMinNumberOfImportMappersAcceptedByOraOop(
    ...
    return conf.getInt(
        OraOopConstants.ORAOOP_MIN_IMPORT_MAPPERS,
        OraOopConstants.MIN_NUM_IMPORT_MAPPERS_ACCEPTED_BY_ORAOOP);
  }

  public static int
  getMinAppendValuesBatchSize(org.apache.hadoop.conf.Configuration conf) {

    return conf.getInt(
        OraOopConstants.ORAOOP_MIN_APPEND_VALUES_BATCH_SIZE,
  ...
  public static int getMinNumberOfExportMappersAcceptedByOraOop(
    ...
    return conf.getInt(
        OraOopConstants.ORAOOP_MIN_EXPORT_MAPPERS,
  ...
  public static int getMinNumberOfOracleRacActiveInstancesForDynamicJdbcUrlUse(
    ...
    return conf.getInt(
        OraOopConstants.ORAOOP_MIN_RAC_ACTIVE_INSTANCES,
  ...
  public static int getNumberOfDataChunksPerOracleDataFile(
    ...
    LOG.debug(String.format("%s:\n"
                                ...
                                + "\tresult=%d",
                            getCurrentMethodName(), MAPPER_MULTIPLIER,
                            numberToMultiplyMappersBy, RESULT_INCREMENT,
                            numberToIncrementResultBy, desiredNumberOfMappers,
  ...
  public static OraOopConstants.OraOopOracleBlockToSplitAllocationMethod
  getOraOopOracleBlockToSplitAllocationMethod(
      org.apache.hadoop.conf.Configuration conf,
      OraOopConstants.OraOopOracleBlockToSplitAllocationMethod defaultMethod) {

    if (conf == null) {
      throw new IllegalArgumentException("The conf argument cannot be null");
    }

    String strMethod = conf.get(
    ...
    try {
      strMethod = strMethod.toUpperCase().trim();
      result = OraOopConstants.OraOopOracleBlockToSplitAllocationMethod.valueOf(
          strMethod);
    } catch (IllegalArgumentException ex) {
      ...
      String errorMsg = String.format(
          ...
          strMethod,
          OraOopConstants.ORAOOP_ORACLE_BLOCK_TO_SPLIT_ALLOCATION_METHOD,
          getOraOopOracleBlockToSplitAllocationMethods(), defaultMethod.name());
      LOG.error(errorMsg);
    }
  ...
  getOraOopTableImportWhereClauseLocation(
    ...
    try {
      ...
      result = OraOopConstants.OraOopTableImportWhereClauseLocation.valueOf(
          strLocation);
    } catch (IllegalArgumentException ex) {
      ...
      String errorMsg = String.format(
          ...
          getOraOopTableImportWhereClauseLocations(), defaultLocation.name());
      LOG.error(errorMsg);
    }
  ...
  getOutputDirectory(org.apache.hadoop.conf.Configuration conf) {
  ...
  public static String
  replaceConfigurationExpression(String str,
                                 org.apache.hadoop.conf.Configuration conf) {
  ...
  public static String stringArrayToCSV(String[] list,
    ...
    for (int idx = 0; idx < list.length; idx++) {
      ...
      result.append(
          String.format("%1$s%2$s%1$s", encloseValuesWith, list[idx]));
    }
    return result.toString();
  }
  ...
  public static void writeOutputFile(org.apache.hadoop.conf.Configuration conf,
    ...
    try {
      ...
      LOG.error(String.format(
          "Error attempting to write the file %s\n"
              + "%s",
  ...
  public static class JdbcOracleThinConnection {
    ...
    public String toString() {
      ...
      if (this.sid != null && !this.sid.isEmpty()) {
        return String.format("jdbc:oracle:thin:@%s:%d:%s", this.host, this.port,
                             this.sid);
      }
    ...
    public String getTnsName() { return tnsName; }
  }
  ...
  public static String
  getOracleServiceName(org.apache.hadoop.conf.Configuration conf) {
  ...
  public static String
  getMapperJdbcUrlPropertyName(int mapperId,
                               org.apache.hadoop.conf.Configuration conf) {
  ...
  public static void
  rememberSqoopJobType(OraOopConstants.Sqoop.Tool jobType,
                       org.apache.hadoop.conf.Configuration conf) {

    conf.set(SQOOP_JOB_TYPE, jobType.name());
  }

  public static OraOopConstants.Sqoop.Tool
  recallSqoopJobType(org.apache.hadoop.conf.Configuration conf) {
  ...
  public static boolean
  omitLobAndLongColumnsDuringImport(org.apache.hadoop.conf.Configuration conf) {
  ...
  formatTimestampForOracleObjectName(Object oracleDateTime) {
    ...
    String jobTimeStr = OraOopOracleQueries.oraDATEToString(
        oracleDateTime,
  ...
  private static Object decodeTimestampFromOracleObjectName(
    ...
    try {
      ...
      dateString = String.format("%s/%s/%s %s:%s:%s", year, month, day, hour,
                                 minute, second);
      dateFormatString = "yyyy/mm/dd hh24:mi:ss";

      return OraOopOracleQueries.oraDATEFromString(dateString,
                                                   dateFormatString);
    } catch (Exception ex) {
  ...
  public static String
  createExportTablePartitionNameFromOracleTimestamp(Object oracleDateTime) {
  ...
  public static String
  createExportTableNamePrefixFromOracleTimestamp(Object oracleDateTime) {
  ...
  public static Object decodeExportTableNamePrefix(String tableNamePrefix) {
    ...
    if (!tableNamePrefix.startsWith(
            OraOopConstants.EXPORT_MAPPER_TABLE_NAME_PREFIX)) {
      return null;
    }

    String formattedTimestamp = tableNamePrefix.substring(
  ...
  public static boolean userWantsToCreatePartitionedExportTableFromTemplate(
    ...
    return userWantsToCreateExportTableFromTemplate(conf) &&
        conf.getBoolean(OraOopConstants.ORAOOP_EXPORT_CREATE_TABLE_PARTITIONED,
                        false);
  }
  ...
  public static String generateExportTableSubPartitionName(
      int mapperId, Object sysDateTime,
      org.apache.hadoop.conf.Configuration conf) {
    ...
    String subPartitionName =
        String.format("%s_MAP_%d" // <- Should allow for
    ...
    if (subPartitionName.length() >
      ...
      throw new RuntimeException(String.format(
          "The generated Oracle subpartition name \"%s\" is longer "
              + "than %d characters.",
          subPartitionName, OraOopConstants.Oracle.MAX_IDENTIFIER_LENGTH));
    }

    return subPartitionName;
  }

  public static String[] generateExportTableSubPartitionNames(
      int numMappers, Object sysDateTime,
      org.apache.hadoop.conf.Configuration conf) {
  ...
  public static OracleTable
  generateExportTableMapperTableName(int mapperId, Object sysDateTime,
                                     String schema) {
    // mapperId: should allow 10,000,000 mappers before it exceeds 30
    // characters.
  ...
  public static OracleTable
  generateExportTableMapperTableName(String mapperSuffix, Object sysDateTime,
                                     String schema) {
  ...
  public static class DecodedExportMapperTableName {
    ...
    public void setMapperId(String newMapperId) { this.mapperId = newMapperId; }

    public Object getTableDateTime() { return tableDateTime; }
  ...
  public static DecodedExportMapperTableName
  decodeExportTableMapperTableName(OracleTable oracleTable) {
    ...
    try {
      ...
      String mapperIdFragment = oracleTable.getName().substring(
          lastUnderScoreIndex + 1, oracleTable.getName().length());
  ...
  public static void
  rememberOracleDateTime(org.apache.hadoop.conf.Configuration conf,
                         String propertyName, String dateTime) {
    conf.set(propertyName, dateTime);
  }

  public static Object
  recallOracleDateTime(org.apache.hadoop.conf.Configuration conf,
                       String propertyName) {
  ...
  getExportUpdateMode(org.apache.hadoop.conf.Configuration conf) {
  ...
  public static InsertMode
  getExportInsertMode(org.apache.hadoop.conf.Configuration conf) {
  ...
  public static String
  getTemporaryTableStorageClause(org.apache.hadoop.conf.Configuration conf) {
  ...
  public static String
  getExportTableStorageClause(org.apache.hadoop.conf.Configuration conf) {
  ...
  private static String[] getExtraExportUpdateKeyColumnNames(
      String updateKey, org.apache.hadoop.conf.Configuration conf) {
    ...
    try {
      ...
      String errorMsg = String.format(
          "An invalid value of \"%s\" was specified for the \"%s\" "
              ...
              + "\tValid values are: %s\n"
              + "\tThe default value of %s will be used.",
          strUsage, OraOopConstants.ORAOOP_ORACLE_APPEND_VALUES_HINT_USAGE,
  ...
  public static String
  getImportHint(org.apache.hadoop.conf.Configuration conf) {
    String result = null;
