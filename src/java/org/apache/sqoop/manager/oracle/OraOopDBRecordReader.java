...
import org.apache.sqoop.manager.oracle.OraOopConstants.OraOopTableImportWhereClauseLocation;
...
class OraOopDBRecordReader<T extends SqoopRecord>
  ...
  private static final OraOopLog LOG =
      OraOopLogFactory.getLog(OraOopDBRecordReader.class);
  ...
  public OraOopDBRecordReader(DBInputFormat.DBInputSplit split,
    ...
    LOG.info(String.format(
        ...
        conn.toString(), thisOracleInstanceName));
  ...
  public static OraOopDBInputSplit castSplit(DBInputSplit split) {
    ...
    if (!(split.getClass() == desiredSplitClass)) {
      String errMsg = String.format(
          "The type of Split available within %s "
              + "should be an instance of class %s, "
              + "but is actually an instance of class %s",
          OraOopUtilities.getCurrentMethodName(), desiredSplitClass.getName(),
  ...
  protected String getSelectQuery() {
    ...
    boolean consistentRead = this.getDBConf().getConf().getBoolean(
        OraOopConstants.ORAOOP_IMPORT_CONSISTENT_READ, false);
    long consistentReadScn = this.getDBConf().getConf().getLong(
    ...
    try {
      ...
      tableColumns = OraOopOracleQueries.getTableColumns(
    ...
    for (int idx = 0; idx < numberOfDataChunks; idx++) {
      ...
      query
          .append(getColumnNamesClause(tableColumns, dataChunk.getId())) // <-
          // SELECT
          // clause
          .append("\n");
  ...
  protected ResultSet executeQuery(String query) throws SQLException {
    ...
    try {
      ...
      LOG.error(String.format("Error in %s while executing the SQL query:\n"
                                  + "%s\n\n"
                                  + "%s",
                              OraOopUtilities.getCurrentMethodName(), query,
                              ex.getMessage()));
      throw ex;
    }
  }
  ...
  public void close() throws IOException {
    ...
    if (OraOopUtilities.userWantsOracleSessionStatisticsReports(
            getDBConf().getConf())) {
  ...
  public OracleTable getOracleTableContext() {
    ...
    Configuration conf = this.getDBConf().getConf();
    OracleTable result =
        new OracleTable(conf.get(OraOopConstants.ORAOOP_TABLE_OWNER),
                        conf.get(OraOopConstants.ORAOOP_TABLE_NAME));
    return result;
  }
}
