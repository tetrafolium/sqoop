...
public final class OracleUtils {
  ...
  public static boolean isEscaped(final String identifier) {
    return !StringUtils.isBlank(identifier) && identifier.startsWith("\"") &&
        identifier.endsWith("\"");
  }
  ...
  public static String escapeIdentifier(final String identifier,
    ...
    if (isEscaped(identifier)) {
      ...
      // If format is a.b.c then each identifier has to be escaped separately to
      ...
      return StringUtils.isNotBlank(escapedIdentifier)
  ...
  public static LogicalType toAvroLogicalType(int sqlType, Integer precision,
                                              Integer scale,
                                              Configuration conf) {
    switch (sqlType) {
    case Types.NUMERIC:
    case Types.DECIMAL:
      // Negative scale means that there are a couple of zeros before the
      // decimal point. We need to add it to precision as an offset because
      // negative scales are not allowed in Avro.
      ...
      if (!isValidScale(scale) && configuredScale == null) {
        throw new RuntimeException(
            "Invalid scale for Avro Schema. Please specify a default scale with the -D" +
            ConfigurationConstants.PROP_AVRO_DECIMAL_SCALE +
