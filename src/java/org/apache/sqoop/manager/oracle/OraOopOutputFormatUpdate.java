...
import org.apache.sqoop.manager.oracle.OraOopOracleQueries.CreateExportChangesTableOptions;
...
private static final OraOopLog LOG =
	OraOopLogFactory.getLog(OraOopOutputFormatUpdate.class);
...
throws IOException {
	...
	useAppendValuesOracleHint =
		allowUserToOverrideUseOfTheOracleAppendValuesHint(
			context, useAppendValuesOracleHint);
	...
	try {
		result = new OraOopDBRecordWriterUpdate(context, mapperId, updateMode,
		                                        useAppendValuesOracleHint);
		...
		throw new IOException(
			      String.format(
				      ...
				      + "The classpath is:\n%s",
				      OraOopUtilities.getJavaClassPath()),
			      ex);
	} catch (Exception ex) {
...
throws ClassNotFoundException, SQLException {
	...
	this.tableHasOraOopPartitions = conf.getBoolean(
		OraOopConstants.EXPORT_TABLE_HAS_ORAOOP_PARTITIONS, false);
}
	...
	if (this.tableHasOraOopPartitions) {
		...
		default:
			throw new RuntimeException(
				      String.format("Update %s to cater for the updateMode \"%s\".",
				                    OraOopUtilities.getCurrentMethodName(),
				                    this.updateMode.toString()));
		}
	} else {
		switch (this.updateMode) {
...
private void
updateMainExportTableFromUniqueMapperTable(TaskAttemptContext context,
...
throws SQLException {
	...
	OracleTable changesTable =
		OraOopUtilities.generateExportTableMapperTableName(
			Integer.toString(this.mapperId) + "_CHG", sysDateTime, null);

	OraOopOracleQueries.CreateExportChangesTableOptions changesTableOptions;
	boolean parallelizationEnabled =
		OraOopUtilities.enableOracleParallelProcessingDuringExport(
	...
	String temporaryTableStorageClause =
		OraOopUtilities.getTemporaryTableStorageClause(
			context.getConfiguration());

	try {
		int changeTableRowCount = OraOopOracleQueries.createExportChangesTable(
			this.getConnection(), changesTable, temporaryTableStorageClause,
			this.oracleTable, targetTable, this.updateColumnNames,
			changesTableOptions, parallelizationEnabled,
			OracleUtils.isOracleEscapingDisabled(context.getConfiguration()));
		...
		case ReInsertUpdatedRowsAndNewRows:
			...
			OraOopOracleQueries.deleteRowsFromTable(
				this.getConnection(), targetTable, changesTable,
				this.updateColumnNames, parallelizationEnabled);

			OraOopOracleQueries.insertRowsIntoExportTable(
				this.getConnection(), targetTable, changesTable, sysDateTime,
				this.mapperId, parallelizationEnabled,
				OracleUtils.isOracleEscapingDisabled(context.getConfiguration()));
			break;
		...
		case UpdateSql:
			...
			OraOopOracleQueries.updateTable(
				this.getConnection(), targetTable, changesTable,
				this.updateColumnNames, this.getOracleTableColumns(), sysDateTime,
				this.mapperId, parallelizationEnabled,
				context.getConfiguration().getBoolean(
					SqoopOptions.ORACLE_ESCAPING_DISABLED, false));
		...
		case MergeSql:
			...
			OraOopOracleQueries.mergeTable(
				this.getConnection(), targetTable, changesTable,
				this.updateColumnNames, this.getOracleTableColumns(), sysDateTime,
				this.mapperId, parallelizationEnabled);

			double mergeTimeInSec =
				(System.nanoTime() - mergeStart) / Math.pow(10, 9);
...
protected String getBatchSqlStatement() {
	...
	if (sqlStatement == null) {
		this.sqlStatement = getBatchInsertSqlStatement(
