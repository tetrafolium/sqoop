...
private static final OraOopLog LOG =
	OraOopLogFactory.getLog(OraOopOutputFormatBase.class);
...
protected boolean canUseOracleAppendValuesHint(TaskAttemptContext context) {
	...
	OracleVersion oracleVersion = new OracleVersion(
		conf.getInt(OraOopConstants.ORAOOP_ORACLE_DATABASE_VERSION_MAJOR, 0),
		conf.getInt(OraOopConstants.ORAOOP_ORACLE_DATABASE_VERSION_MINOR, 0), 0,
		0, "");
	...
	if (result) {
		boolean binaryDoubleColumnExists = conf.getBoolean(
			OraOopConstants.TABLE_CONTAINS_BINARY_DOUBLE_COLUMN, false);
		boolean binaryFloatColumnExists = conf.getBoolean(
			OraOopConstants.TABLE_CONTAINS_BINARY_FLOAT_COLUMN, false);
...
abstract class OraOopDBRecordWriterBase
	extends ExportOutputFormat<K, V>.ExportRecordWriter {
...
throws ClassNotFoundException, SQLException {
	...
	LOG.info(String.format(
			 ...
			 connection.toString(), thisOracleInstanceName));
...
protected void
setOracleTableColumns(OracleTableColumns newOracleTableColumns) {
...
throws SQLException {
	...
	setOracleTableColumns(OraOopOracleQueries.getTableColumns(
				      this.getConnection(), this.oracleTable,
...
protected String getBatchInsertSqlStatement(String oracleHint) {
	...
	for (int idx = 0; idx < this.oracleTableColumns.size(); idx++) {
		...
		boolean escapingDisabled =
			OracleUtils.isOracleEscapingDisabled(getConf());
		sqlNames.append(
			OracleUtils.escapeIdentifier(columnName, escapingDisabled));
		...
		if (pseudoColumnValue != null) {
		...
		} else if (oracleTableColumn.getOracleType() ==
		           OraOopOracleQueries.getOracleType("STRUCT")) {
			if (oracleTableColumn.getDataType().equals(
				...
				bindVarName = String.format("urifactory.getUri(%s)",
				   ...
				   OraOopConstants
				   .ORAOOP_MAP_TIMESTAMP_AS_STRING_DEFAULT)) {
			if (oracleTableColumn.getOracleType() ==
			    OraOopOracleQueries.getOracleType("DATE")) {
				bindVarName = String.format("to_date(%s, 'yyyy-mm-dd hh24:mi:ss')",
				                            columnNameToBindVariable(columnName));
			} else if (oracleTableColumn.getOracleType() ==
			           OraOopOracleQueries.getOracleType("TIMESTAMP")) {
				bindVarName =
					String.format("to_timestamp(%s, 'yyyy-mm-dd hh24:mi:ss.ff')",
					              columnNameToBindVariable(columnName));
			} else if (oracleTableColumn.getOracleType() ==
			           OraOopOracleQueries.getOracleType("TIMESTAMPTZ")) {
				bindVarName = String.format(
					"to_timestamp_tz(%s, 'yyyy-mm-dd hh24:mi:ss.ff TZR')",
					columnNameToBindVariable(columnName));
			} else if (oracleTableColumn.getOracleType() ==
			           OraOopOracleQueries.getOracleType("TIMESTAMPLTZ")) {
				bindVarName = String.format(
					"to_timestamp_tz(%s, 'yyyy-mm-dd hh24:mi:ss.ff TZR')",
					columnNameToBindVariable(columnName));
			}
	...
	String sql = String.format("insert %s into %s\n"
	                           ...
	                           + "values\n"
	                           + "(%s)\n",
	                           oracleHint, this.oracleTable.toString(),
	                           sqlNames.toString(), sqlValues.toString());
...
abstract void
configurePreparedStatement(PreparedStatement preparedStatement,
                           List<SqoopRecord> userRecords)
throws SQLException;

private void setBindValueAtName(PreparedStatement statement,
                                String bindValueName, Object bindValue,
                                OracleTableColumn column)
throws SQLException {
	if (column.getOracleType() ==
	...
	} else if (column.getOracleType() ==
	           OraOopOracleQueries.getOracleType("VARCHAR")) {
		...
		if (objValue instanceof Timestamp) {
			...
			OraOopOracleQueries.setStringAtName(statement, bindValueName, value);
	           ...
	           OraOopOracleQueries.getOracleType("BINARY_DOUBLE")) {
		...
		if (value != null) {
			...
			OraOopOracleQueries.setObjectAtName(statement, bindValueName, null);
	           ...
	           OraOopOracleQueries.getOracleType("BINARY_FLOAT")) {
	...
	} else if (column.getOracleType() == OraOopOracleQueries.getOracleType(
			   "STRUCT")) {                     // <- E.g. URITYPE
		...
		OraOopOracleQueries.setObjectAtName(statement, bindValueName,
		                                    bindValue);
	}
}

protected void
configurePreparedStatementColumns(PreparedStatement statement,
...
throws SQLException {
	...
	if (this.tableHasMapperRowNumberColumn) {
		bindValueName = columnNameToBindVariable(
...
protected Object getJobSysDate(TaskAttemptContext context) {
	...
	Configuration conf = context.getConfiguration();
	return OraOopUtilities.recallOracleDateTime(
		conf, OraOopConstants.ORAOOP_JOB_SYSDATE);
}
...
throws SQLException {
	...
	OraOopOracleQueries.createExportTableForMapper(
		this.getConnection(), mapperTable, temporaryTableStorageClause,
		templateTable, false); // <- addOraOopPartitionColumns

	LOG.debug(String.format("Created temporary mapper table %s",
...
protected String generateInsertValueForPseudoColumn(String columnName) {
	...
	if (columnName.equalsIgnoreCase(
		...
		String partitionValueStr = this.getConf().get(
