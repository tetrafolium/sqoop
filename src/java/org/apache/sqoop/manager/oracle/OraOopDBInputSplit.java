...
class OraOopDBInputSplit extends DBInputFormat.DBInputSplit {
  ...
  public void readFields(DataInput input) throws IOException {
    ...
    if (dataChunkCount == 0) {
      ...
      for (int idx = 0; idx < dataChunkCount; idx++) {
        try {
          dataChunkClass =
              (Class<? extends OraOopOracleDataChunk>)Class.forName(
  ...
  protected int getSplitId() { return this.splitId; }

  protected void setSplitId(int newSplitId) { this.splitId = newSplitId; }
  ...
  protected void
  setTotalNumberOfBlocksInAllSplits(int newTotalNumberOfBlocksInAllSplits) {
