...
private static final OraOopLog LOG =
	OraOopLogFactory.getLog(OraOopOutputFormatInsert.class);
...
throws IOException {
	...
	try {
		result = new OraOopDBRecordWriterInsert(context, mapperId, insertMode,
		                                        useAppendValuesOracleHint);
...
throws SQLException {
	...
	default:
		throw new RuntimeException(
			      String.format("Update %s to cater for the insertMode \"%s\".",
			                    OraOopUtilities.getCurrentMethodName(),
			                    this.insertMode.toString()));
	}
}
...
private void exchangePartitionUniqueMapperTableDataIntoMainExportTable(
	...
	try {
		...
		OraOopOracleQueries.exchangeSubpartition(
			this.getConnection(), mainTable, this.subPartitionName,
			this.oracleTable);
