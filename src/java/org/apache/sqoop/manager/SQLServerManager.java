...
public static final Log LOG =
	LogFactory.getLog(SQLServerManager.class.getName());
...
public SQLServerManager(final String driver, final SqoopOptions opts,
                        final SqlServerManagerContextConfigurator
                        configurator) {
	super(driver, opts);
...
throws IOException, ImportException {
	...
	String splitColumn =
		getSplitColumn(context.getOptions(), context.getTableName());
...
throws IOException, ExportException {
	...
	JdbcExportJob exportJob = new JdbcExportJob(
		context, null, null, context.getOutputFormatClass(),
...
throws IOException, ExportException {
	boolean runAsExportJob =
		formatConfigurator.configureContextForUpdate(context, this);
	if (runAsExportJob) {
...
throws IOException, ExportException {
	...
	JdbcUpsertExportJob exportJob = new JdbcUpsertExportJob(
		context, SqlServerUpsertOutputFormat.class,
		getParquetJobConfigurator().createParquetExportJobConfigurator());
	exportJob.runExport();
}
...
protected String getPrimaryKeyQuery(String tableName) {
	return "SELECT kcu.COLUMN_NAME FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS tc, "
...
void parseExtraArgs(String[] args) throws ParseException {
	...
	if (cmdLine.hasOption(
		    SqlServerManagerContextConfigurator.RESILIENT_OPTION)) {
		LOG.warn(
			"Sqoop will use resilient operations! In case of import, "
			+
...
private RelatedOptions getExtraOptions() {
	...
	extraOptions.addOption(OptionBuilder.withArgName("string")
	                       .hasArg()
	...
	extraOptions.addOption(
		OptionBuilder
		.withLongOpt(SqlServerManagerContextConfigurator.RESILIENT_OPTION)
		.create());
