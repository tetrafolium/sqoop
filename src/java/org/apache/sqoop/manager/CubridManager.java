 ...
 */
public class CubridManager extends CatalogQueryManager {

public static final Log LOG =
	LogFactory.getLog(CubridManager.class.getName());
...
throws IOException, ExportException {
	...
	JdbcUpsertExportJob exportJob = new JdbcUpsertExportJob(
		context, CubridUpsertOutputFormat.class,
		getParquetJobConfigurator().createParquetExportJobConfigurator());
	exportJob.runExport();
}
...
protected String getListColumnsQuery(String tableName) {
	...
	return "SELECT ATTR_NAME FROM DB_ATTRIBUTE WHERE"
	       + " CLASS_NAME = '" + tableName + "'  ORDER BY def_order";
}
