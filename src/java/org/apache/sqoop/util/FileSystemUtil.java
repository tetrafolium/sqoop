...
public final class FileSystemUtil {
   ...
   * but with a fully-qualified URI. Returns {@code null} if provided path is
   * {@code null};
   */
  ...
  public static boolean isFile(Path path, Configuration conf)
      throws IOException {
    return path.getFileSystem(conf).isFile(path);
  }

  public static List<Path> listFiles(Path path, Configuration conf)
      throws IOException {
  ...
  findFilesWithPathContainingPattern(Path path, Configuration conf,
