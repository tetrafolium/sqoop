...
private static final Log LOG =
	LogFactory.getLog(MainframeFTPClientUtils.class.getName());
...
public static List<String> listSequentialDatasets(String pdsName,
                                                  Configuration conf)
throws IOException {
	...
	String dsType =
		conf.get(MainframeConfiguration.MAINFRAME_INPUT_DATASET_TYPE);
	if (dsType == null) {
		// default dataset type to partitioned dataset
		conf.set(MainframeConfiguration.MAINFRAME_INPUT_DATASET_TYPE,
	...
	if (dsType != null && p != null) {
		isSequentialDs = p.getMainframeDatasetType().toString().equals(
			MainframeConfiguration.MAINFRAME_INPUT_DATASET_TYPE_SEQUENTIAL);
		isGDG = p.getMainframeDatasetType().toString().equals(
	...
	try {
		...
		if (ftp != null) {
			...
			if (!MainframeConfiguration.MAINFRAME_INPUT_DATASET_TYPE_PARTITIONED
			    .equals(dsType)) {
				FTPListParseEngine parser = null;
				if (MainframeConfiguration.MAINFRAME_INPUT_DATASET_TYPE_GDG.equals(
					    dsType)) {
					// use GDG specific parser to filter out non GDG datasets
					parser = ftp.initiateListParsing(
						MainframeConfiguration
						.MAINFRAME_FTP_FILE_GDG_ENTRY_PARSER_CLASSNAME,
						"");
				} else {
					// excepting partitioned datasets, use the
					// MainframeFTPFileEntryParser, default doesn't match larger
					// datasets
					parser = ftp.initiateListParsing(
						MainframeConfiguration
						.MAINFRAME_FTP_FILE_ENTRY_PARSER_CLASSNAME,
						"");
				}
				...
				while (parser.hasNext()) {
					...
					for (FTPFile file : files) {
						if (file != null) {
							listing.add(file);
							LOG.info(String.format("Name: %s Type: %s", file.getName(),
							                       file.getType()));
						}
					...
					for (FTPFile f : ftpFiles) {
						LOG.info(String.format("Name: %s, Type: %s", f.getName(),
						                       f.getType()));
					}
				}
			} else {
				...
				LOG.info(
					"Dataset is a partitioned dataset, using default FTP list parsing");
			...
			if (!isGDG) {
				...
				if (ftpFiles.length > 0 &&
				    ftpFiles[ftpFiles.length - 1].getType() == FTPFile.FILE_TYPE) {
...
throws IOException {
	...
	try {
		...
		if (StringUtils.equalsIgnoreCase(
			...
			// ftp.setFileTransferMode(FTP.BINARY_FILE_TYPE) doesn't work for MVS,
			// it throws a syntax error
			ftp.sendCommand("TYPE I");
			// this is IMPORTANT - otherwise it will convert 0x0d0a to 0x0a =
			// dropping bytes
...
public static List<String> applyFtpCmds(FTPClient ftp, Configuration conf)
throws IOException {
	String ftpCmds =
		conf.get(MainframeConfiguration.MAINFRAME_FTP_CUSTOM_COMMANDS);
