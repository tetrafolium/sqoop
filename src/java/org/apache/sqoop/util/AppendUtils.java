...
public class AppendUtils {
  ...
  public static final String MAPREDUCE_OUTPUT_BASENAME_PROPERTY =
      "mapreduce.output.basename";

  private ImportJobContext context = null;

  public AppendUtils(ImportJobContext context) { this.context = context; }
  ...
  /**
   * Move selected files from source to target using a specified starting
   * partition.
   *
  ...
  private void moveFiles(FileSystem fs, Path sourceDir, Path targetDir,
    ...
    for (FileStatus fileStatus : sourceFiles) {
      ...
      if (fileStatus.isDir()) { // move all subdirectories
        // pass target dir as initial dest to prevent nesting inside preexisting
        // dir
        if (!fs.exists(targetDir) &&
            fs.rename(fileStatus.getPath(), targetDir)) {
          LOG.debug("Directory: " + sourceFilename +
                    " renamed to: " + sourceFilename);
        } else {
          int dirNumber = 0;
          Path destPath;
          do {
            ...
            // name-nnnnn?
            destFilename.append(sourceFilename)
                .append("-")
          ...
          LOG.debug("Directory: " + sourceFilename +
                    " renamed to: " + destPath.getName());
        }
      } else if (getDataFileNamePattern()
                     .matcher(sourceFilename)
                     .matches()) { // move only matching top-level files
        do {
          // clear the builder in case this isn't the first iteration
          destFilename.setLength(0);

          // name-nnnnn
          destFilename.append(getFilename(sourceFilename))
        ...
        } while (!fs.rename(fileStatus.getPath(),
                            new Path(targetDir, destFilename.toString())));

        LOG.debug("Filename: " + sourceFilename +
                  " repartitioned to: " + destFilename.toString());
      } else {
        // Generated Parquet files do not follow the pattern
        // "part-m-([0-9]{5}).ext", so that these files cannot be moved to
        // target directory expectedly. We simply check file extension.
   ...
   *
   * @param salt Salt that will be appended at the end of the generated
   *     directory.
   *             Can be arbitrary string, for example table name or query
   * checksum.
  ...
  public static Path getTempAppendDir(String salt, SqoopOptions options) {
    ...
    String tempDir =
        options.getTempRootDir() + Path.SEPARATOR + uuid + "_" + salt;
    return new Path(tempDir);
  }

  /**
   * Return the Pattern of the file name that will end up in the target
   * directory
   *
   * Take into account that user might pass mapreduce.output.basename, which
   * should override the default filename prefix of "part"
   *
   * @return Pattern
   */
  private Pattern getDataFileNamePattern() {
    String prefix =
        context.getOptions().getConf().get(MAPREDUCE_OUTPUT_BASENAME_PROPERTY);
