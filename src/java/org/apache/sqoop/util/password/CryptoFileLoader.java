 ...
 * encrypted file. Please note that this method is merely obfuscating the
 * password, as malicious user will be able to retrieve the password if he
 * intercepts the Sqoop commands. He won't be able to get it if he will just get
 * the password file though. Current implementation is limited to ECB and is not
 ...
 */
private static String PROPERTY_CRYPTO_ALG =
	"org.apache.sqoop.credentials.loader.crypto.alg";
 ...
 */
private static String PROPERTY_CRYPTO_SALT =
	"org.apache.sqoop.credentials.loader.crypto.salt";

/**
 * Iterations argument for creating key.
 */
private static String PROPERTY_CRYPTO_ITERATIONS =
	"org.apache.sqoop.credentials.loader.crypto.iterations";

/**
 * Length of the key (driven by the used algorithm).
 */
private static String PROPERTY_CRYPTO_KEY_LEN =
	"org.apache.sqoop.credentials.loader.crypto.salt.key.len";

/**
 * Passphrase (encryption password).
 */
private static String PROPERTY_CRYPTO_PASSPHRASE =
...
throws IOException {
	...
	if (passPhrase == null) {
		throw new IOException("Passphrase is missing in property " +
		                      PROPERTY_CRYPTO_PASSPHRASE);
	}
	...
	int iterations =
		configuration.getInt(PROPERTY_CRYPTO_ITERATIONS, DEFAULT_ITERATIONS);
	...
	try {
		key = new SecretKeySpec(factory
								...
								passPhrase.toCharArray(), salt.getBytes(),
								iterations, keyLen))
		                        .getEncoded(),
		                        algOnly);
	} catch (Exception e) {
