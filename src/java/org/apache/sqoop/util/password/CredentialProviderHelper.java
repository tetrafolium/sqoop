...
public class CredentialProviderHelper {
  ...
  static {
    try {
      ...
      LOG.debug(
          "Found org.apache.hadoop.security.alias.CredentialProviderFactory");

      methCreateCredEntry = clsCredProvider.getMethod(
          "createCredentialEntry", new Class[] {String.class, char[].class});
      LOG.debug("Found CredentialProvider#createCredentialEntry");

      methFlush = clsCredProvider.getMethod("flush", new Class[] {});
      LOG.debug("Found CredentialProvider#flush");

      methGetPassword = Configuration.class.getMethod(
          "getPassword", new Class[] {String.class});
      LOG.debug("Found Configuration#getPassword");

      methGetProviders = clsCredProviderFactory.getMethod(
          "getProviders", new Class[] {Configuration.class});
  ...
  public static boolean isProviderAvailable() {
    ...
    if (clsCredProvider == null || clsCredProviderFactory == null ||
        methCreateCredEntry == null || methGetPassword == null ||
        methFlush == null) {
      return false;
    }
    return true;
  ...
  public static String resolveAlias(Configuration conf, String alias)
    ...
    try {
      char[] cred = (char[])methGetPassword.invoke(conf, new Object[] {alias});
      if (cred == null) {
      ...
      throw new RuntimeException("Error resolving password "
                                     + " from the credential providers ",
                                 ite.getTargetException());
    } catch (IllegalAccessException iae) {
      throw new RuntimeException(
          "Error invoking the credential provider method", iae);
    }
  }
  /**
  ...
  public static void createCredentialEntry(Configuration conf,
                                           ...
                                           String alias, String credential)
      throws IOException {
