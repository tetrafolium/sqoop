...
public final class LobFile {
   ...
   */
  public static org.apache.sqoop.io.LobFile.Reader open(Path p,
                                                        Configuration conf)
      throws IOException {
  ...
  create(Path p, Configuration conf, boolean isCharData, String codec,
         int entriesPerSegment) throws IOException {
   ...
   */
  public static org.apache.sqoop.io.LobFile.Writer
  create(Path p, Configuration conf, boolean isCharData) throws IOException {
   ...
   * @param conf the configuration to use to interact with the filesystem.
   */
  public static org.apache.sqoop.io.LobFile.Writer create(Path p,
                                                          Configuration conf)
      throws IOException {
  ...
  public abstract static class Writer implements Closeable {
     ...
     */
    public abstract java.io.Writer writeClobRecord(long len) throws IOException;
  ...
  private static class LobFileHeader implements Writable {
     ...
     */
    public LobFileHeader(DataInput in) throws IOException { readFields(in); }
     ...
     */
    public RecordStartMark getStartMark() { return startMark; }

    /**
     * @return the MetaBlock for this LobFile.
     */
    public MetaBlock getMetaBlock() { return metaBlock; }
  }
  ...
  private static class RecordStartMark implements Writable {
    ...
    public RecordStartMark() { generateStartMark(); }

    public RecordStartMark(DataInput in) throws IOException { readFields(in); }
  ...
  private static class MetaBlock
      extends AbstractMap<String, BytesWritable> implements Writable {
    ...
    public MetaBlock() { entries = new TreeMap<String, BytesWritable>(); }
  ...
  private static class IndexTableEntry implements Writable {
    ...
    public IndexTableEntry(DataInput in) throws IOException { readFields(in); }

    private void setSegmentOffset(long offset) { this.segmentOffset = offset; }

    private void setFirstIndexId(long id) { this.firstIndexId = id; }
     ...
     */
    public long getFirstIndexId() { return this.firstIndexId; }

    /**
     * @return the offset of the first record indexed by this segment.
     */
    public long getFirstIndexOffset() { return this.firstIndexOffset; }

    /**
     * @return the offset of the last record indexed by this segment.
     */
    public long getLastIndexOffset() { return this.lastIndexOffset; }

    /**
     * @return the offset from the start of the file of the IndexSegment
     * data itself.
     */
    public long getSegmentOffset() { return this.segmentOffset; }
  ...
  private static class IndexSegment implements Writable {
     ...
     * IndexTable.
     */
    public IndexTableEntry getTableEntry() { return tableEntry; }
    ...
    public void addRecordLen(long recordLen) throws IOException {
      ...
      this.tableEntry.setLastIndexOffset(this.tableEntry.getLastIndexOffset() +
                                         this.prevLength);
    ...
    public void readFields(DataInput in) throws IOException {
      ...
      if (SEGMENT_HEADER_ID != segmentId) {
        throw new IOException("Expected segment header id " +
                              SEGMENT_HEADER_ID + "; got " + segmentId);
      }
     ...
     */
    public void reset() { this.dataInputBuf = null; }
    ...
    public boolean next() {
      ...
      if (null == dataInputBuf) {
        ...
        this.dataInputBuf.reset(recordLenBytes.getBytes(), 0,
                                recordLenBytes.getLength());
     ...
     */
    public long getCurRecordLen() { return curLen; }
     ...
     * current record.
     */
    public long getCurRecordStart() { return curOffset; }
  }
  ...
  private static class IndexTable
    ...
    public IndexTable() { tableEntries = new ArrayList<IndexTableEntry>(); }

    public IndexTable(DataInput in) throws IOException { readFields(in); }
    ...
    public void add(IndexTableEntry entry) { tableEntries.add(entry); }

    public IndexTableEntry get(int i) { return tableEntries.get(i); }

    public int size() { return tableEntries.size(); }
  ...
  private static class V0Reader extends org.apache.sqoop.io.LobFile.Reader {
    ...
    private void openCodec() throws IOException {
      String codecName =
          header.getMetaBlock().getString(MetaBlock.COMPRESSION_CODEC_KEY);
      if (null != codecName) {
    ...
    private void readPositionedIndexSegment() throws IOException {
      ...
      this.curIndexSegment =
          new IndexSegment(this.indexTable.get(curIndexSegmentId), this.dataIn);
    }
    ...
    private boolean matchesRsm(byte[] buf, int offset) {
      return matchesRsm(this.header.getStartMark().getBytes(), buf, offset);
    }

    private boolean matchesRsm(byte[] buf) { return matchesRsm(buf, 0); }
    ...
    public InputStream readBlobRecord() throws IOException {
      ...
      if (this.codec != null) {
        ...
        this.userInputStream =
            new DecompressorStream(this.userInputStream, decompressor);
      }
  ...
  private static class V0Writer extends org.apache.sqoop.io.LobFile.Writer {
     ...
     */
    V0Writer(Path p, Configuration conf, boolean isCharData, String codecName,
             int entriesPerSegment) throws IOException {
    ...
    private void init() throws IOException {
      ...
      this.countingOut =
          new CountingOutputStream(new BufferedOutputStream(fsOut));
    ...
    public OutputStream writeBlobRecord(long claimedLen) throws IOException {
      ...
      this.userCountingOutputStream =
          new CountingOutputStream(new CloseShieldOutputStream(out));
      if (null == this.codec) {
        ...
        this.userOutputStream =
            new CompressorStream(this.userCountingOutputStream, compressor);
      }
