...
private void instantiateFactories(Configuration conf) {
	...
	String[] classNameArray = conf.getStrings(FACTORY_CLASS_NAMES_KEY,
	...
	for (String className : classNameArray) {
		try {
			...
			ManagerFactory factory = ReflectionUtils.newInstance(
				(Class<? extends ManagerFactory>)conf.getClassByName(className),
				conf);
...
public ConnManager getManager(JobData data) throws IOException {
	...
	if (managerClassName != null) {
		...
		try {
			Class<ConnManager> cls =
				(Class<ConnManager>)Class.forName(managerClassName);
			...
			LOG.error(
				"Sqoop wasn't able to create connnection manager properly. "
