...
public static final Log LOG =
	LogFactory.getLog(MySQLDumpMapper.class.getName());
...
public static final Log LOG =
	LogFactory.getLog(CopyingStreamThread.class.getName());

private final MySQLDumpMapper.Context context;
private final InputStream stream;
private final PerfCounters counters;

CopyingStreamThread(final InputStream is, final Context c,
                    final PerfCounters ctrs) {
	this.context = c;
	this.stream = is;
	this.counters = ctrs;
	...
	try {
		...
		while (true) {
			...
			if (preambleLen == -1) {
				...
				preambleLen =
					inLine.indexOf(recordStartMark) + recordStartMark.length();
			}
...
public static final Log LOG =
	LogFactory.getLog(ReparsingStreamThread.class.getName());
...
public void run() {
	...
	try {
		...
		char outputFieldDelim = (char)conf.getInt(
			MySQLUtils.OUTPUT_FIELD_DELIM_KEY, DelimiterSet.NULL_CHAR);
		String outputFieldDelimStr = "" + outputFieldDelim;
		char outputRecordDelim = (char)conf.getInt(
			MySQLUtils.OUTPUT_RECORD_DELIM_KEY, DelimiterSet.NULL_CHAR);
		String outputRecordDelimStr = "" + outputRecordDelim;
		char outputEnclose = (char)conf.getInt(
			MySQLUtils.OUTPUT_ENCLOSED_BY_KEY, DelimiterSet.NULL_CHAR);
		char outputEscape = (char)conf.getInt(
			MySQLUtils.OUTPUT_ESCAPED_BY_KEY, DelimiterSet.NULL_CHAR);
		boolean outputEncloseRequired =
			conf.getBoolean(MySQLUtils.OUTPUT_ENCLOSE_REQUIRED_KEY, false);

		DelimiterSet delimiters = new DelimiterSet(
			outputFieldDelim, outputRecordDelim, outputEnclose, outputEscape,
		...
		while (true) {
			...
			// and trailing text.
			CharBuffer charbuf =
				CharBuffer.wrap(inLine, preambleLen, inLine.length() - 2);
			...
			try {
				...
				LOG.warn("ParseError reading from mysqldump: " + pe.toString() +
				         "; record skipped");
			...
			for (String field : fields) {
				...
				String fieldStr =
					FieldFormatter.escapeAndEnclose(field, delimiters);
				sb.append(fieldStr);
