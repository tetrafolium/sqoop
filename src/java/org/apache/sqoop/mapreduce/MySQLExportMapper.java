...
public static final Log LOG =
	LogFactory.getLog(MySQLExportMapper.class.getName());
...
private void initMySQLImportProcess() throws IOException {
	...
	int outputFieldDelim =
		conf.getInt(MySQLUtils.OUTPUT_FIELD_DELIM_KEY, (int)',');
	int outputRecordDelim =
		conf.getInt(MySQLUtils.OUTPUT_RECORD_DELIM_KEY, (int)'\n');
	...
	this.importStream =
		new BufferedOutputStream(new FileOutputStream(fifoFile));
...
protected void setup(Context context) {
	...
	this.checkpointDistInBytes =
		conf.getLong(MYSQL_CHECKPOINT_BYTES_KEY, DEFAULT_CHECKPOINT_BYTES);
	...
	this.checkpointSleepMs =
		conf.getLong(MYSQL_CHECKPOINT_SLEEP_KEY, DEFAULT_CHECKPOINT_SLEEP_MS);
	...
	if (this.checkpointSleepMs >= conf.getLong("mapred.task.timeout", 0)) {
		LOG.warn("Value for " + MYSQL_CHECKPOINT_SLEEP_KEY +
