...
public class SQLServerDBRecordReader<T extends SqoopRecord>
  ...
  T currentValue() { return super.getCurrentValue(); }

  void saveCurrentValue(T value) {
    lastRecordValue = getCurrentValueOfSplitByColumnFromORM(value, splitColumn);
  }

  private String getCurrentValueOfSplitByColumnFromORM(T generatedORMRecord,
                                                       String columnName) {
    Object result = generatedORMRecord.getFieldMap().get(columnName);
    if (result != null) {
      return result.toString();
    }
    return getCurrentValueOfSplitByColumnFromORMIfSplitByDoesNotMatch(
        generatedORMRecord, columnName);
  }

  /*
   * SQOOP-3139: It is a workaround if the database/table/column is used in case
   * insensitive mode and the user uses Sqoop import with --split-by but the
   * given parameter doesn't match with table name if it is case sensitive eg.:
   * tableName.equals(split-by) doesn't match only if
   * tableName.equalsIgnorecase(split-by)
   *
   */
  private String getCurrentValueOfSplitByColumnFromORMIfSplitByDoesNotMatch(
      T generatedORMRecord, String columnName) {
    for (Map.Entry<String, Object> fields :
         generatedORMRecord.getFieldMap().entrySet()) {
  ...
  String getLastRecordValue() { return lastRecordValue; }
  ...
  public void initialize(InputSplit inputSplit, TaskAttemptContext context)
    ...
    try {
      String className =
          conf.get(SQLServerDBInputFormat.IMPORT_FAILURE_HANDLER_CLASS);
    ...
    if (!SQLFailureHandler.class.isAssignableFrom(connHandlerClass)) {
      String error =
          "A subclass of " + SQLFailureHandler.class.getName() +
  ...
  public boolean nextKeyValue() throws IOException {
    ...
    if (retryCount < 0) {
      throw new IOException("Failed to read from database after " + RETRY_MAX +
                            " retries.");
    }
  ...
  protected String getSelectQuery() {
    ...
    if (lastRecordValue == null) {
      ...
      selectQuery =
          getSelectQuery(lowerClause.toString(), dataSplit.getUpperClause());
    }
