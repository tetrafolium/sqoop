...
public class TextSplitter extends BigDecimalSplitter {
  ...
  public static final String ALLOW_TEXT_SPLITTER_PROPERTY =
  ...
  public List<InputSplit> split(Configuration conf, ResultSet results,
    ...
    if (!conf.getBoolean(ALLOW_TEXT_SPLITTER_PROPERTY, false)) {
      throw new ValidationException(
          ...
          + "allowed only in case of \"-D" + ALLOW_TEXT_SPLITTER_PROPERTY +
          "=true\" property "
    ...
    List<String> splitStrings =
        split(numSplits, minString, maxString, commonPrefix);
    List<InputSplit> splits = new ArrayList<InputSplit>();
  ...
  public List<String> split(int numSplits, String minString, String maxString,
                            String commonPrefix)
      throws SQLException, ValidationException {
    ...
    for (BigDecimal bd : splitPoints) {
      splitStrings.add(
          escapeSingleQuotesInSql(commonPrefix + bigDecimalToString(bd)));
    }
    ...
    if (splitStrings.size() == 1 || !splitStrings.get(splitStrings.size() - 1)
                                         .equals(commonPrefix + maxString)) {
  ...
  public void setUseNCharStrings(boolean use) { useNCharStrings = use; }

  public boolean isUseNCharStrings() { return useNCharStrings; }
}
