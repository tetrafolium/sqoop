...
public class DataDrivenDBInputFormat<T extends DBWritable>
    ...
    case Types.DECIMAL:
      if (splitLimit >= 0) {
        throw new IllegalArgumentException(
            "split-limit is supported only with Integer and Date columns");
      }
  ...
  public List<InputSplit> getSplits(JobContext job) throws IOException {
    ...
    long splitLimit = org.apache.sqoop.config.ConfigurationHelper.getSplitLimit(
    ...
    // table with no index.
    if (1 == targetNumTasks &&
        (boundaryQuery == null || boundaryQuery.isEmpty()) && splitLimit <= 0) {
      List<InputSplit> singletonSplit = new ArrayList<InputSplit>();
      singletonSplit.add(
          new org.apache.sqoop.mapreduce.db.DataDrivenDBInputFormat
              .DataDrivenDBInputSplit("1=1", "1=1"));
    ...
    try {
      ...
      if (null == splitter) {
        throw new IOException(
            "Sqoop does not have the splitter for the given"
            +
  ...
  createDBRecordReader(DBInputSplit split, Configuration conf)
    ...
    try {
      return new DataDrivenDBRecordReader<T>(
  ...
  public static void setInput(Job job, Class<? extends DBWritable> inputClass,
    ...
    DBInputFormat.setInput(job, inputClass, tableName, conditions, splitBy,
                           fieldNames);
  ...
  public static class DataDrivenDBInputSplit
    ...
    public String getLowerClause() { return lowerBoundClause; }

    public String getUpperClause() { return upperBoundClause; }
