...
@VisibleForTesting DBConfiguration dbc;
@VisibleForTesting File fifoFile;
private Connection con;
private OutputStream recordWriter;
public static final Log LOG =
	LogFactory.getLog(NetezzaExternalTableImportMapper.class.getName());
...
@VisibleForTesting String logDir = null;
@VisibleForTesting File taskAttemptDir = null;
...
private String getSqlStatement(DelimiterSet delimiters) throws IOException {
	...
	int errorThreshold =
		conf.getInt(DirectNetezzaManager.NETEZZA_ERROR_THRESHOLD_OPT, 1);
	...
	if (logDirPath.canWrite() && logDirPath.isDirectory()) {
		...
		.append(logDirPath.getAbsolutePath())
		.append(' ');
	} else {
...
throws IOException {
	...
	try {
		...
		extTableThread = new NetezzaJDBCStatementRunner(jdbcFailed, con, sqlStmt);
