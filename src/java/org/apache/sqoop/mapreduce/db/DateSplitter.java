...
public class DateSplitter extends IntegerSplitter {
  ...
  // For Split limit we take input as seconds. So we need to convert to
  // milliseconds
  ...
  public List<InputSplit> split(Configuration conf, ResultSet results,
    ...
    // For split size we are using seconds. So we need to convert to
    // milliseconds.
    long splitLimit =
    ...
    if (sqlDataType == Types.TIMESTAMP) {
      ...
      try {
        ((java.sql.Timestamp)startDate)
            .setNanos(results.getTimestamp(1).getNanos());
        ...
        if (sqlDataType == Types.TIMESTAMP) {
          ...
          try {
            ((java.sql.Timestamp)endDate)
                .setNanos(results.getTimestamp(2).getNanos());
          } catch (NullPointerException npe) {
