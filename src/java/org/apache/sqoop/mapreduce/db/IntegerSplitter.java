...
public List<InputSplit> split(Configuration conf, ResultSet results,
	...
	long splitLimit =
		org.apache.sqoop.config.ConfigurationHelper.getSplitLimit(conf);

	// Get all the split points together.
	List<Long> splitPoints = split(numSplits, splitLimit, minVal, maxVal);
	if (LOG.isDebugEnabled()) {
		LOG.debug(String.format("Splits: [%,28d to %,28d] into %d parts", minVal,
		                        maxVal, numSplits));
 ...
 */
public List<Long> split(long numSplits, long splitLimit, long minVal,
                        long maxVal) throws SQLException {
	...
	double splitSizeDouble =
		((double)maxVal - (double)minVal) / (double)numSplits;
