...
public abstract class SQLServerAsyncDBExecThread extends Thread {
  ...
  private static final Log LOG =
      LogFactory.getLog(SQLServerAsyncDBExecThread.class);
  ...
  protected DBConfiguration getDBConfiguration() { return dbConf; }
  ...
  protected Connection getConnection() throws SQLException {
    if (conn == null || conn.isClosed()) {
      try {
        ...
        LOG.error("Cannot create connection. Driver class not found: " + cnfEx);
      }
    }
    return conn;
  }
  ...
  protected void write(List<SqoopRecord> records)
      ...
      try {
        ...
        if (failureHandler.canHandleFailure(sqlEx)) {
          ...
          throw new IOException("Registered handler cannot recover error "
                                    + "with SQL State: " + sqlEx.getSQLState() +
    ...
    if (retryCount < 0) {
      throw new IOException("Failed to write to database after " + RETRY_MAX +
                            " retries.");
    }
  }
  ...
  getPreparedStatement(List<SqoopRecord> records) throws SQLException;
