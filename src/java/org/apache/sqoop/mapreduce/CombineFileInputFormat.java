...
public abstract class CombineFileInputFormat<K, V>
  ...
  public List<InputSplit> getSplits(JobContext job) throws IOException {
    ...
    Path[] paths =
        FileUtil.stat2Paths(listStatus(job).toArray(new FileStatus[0]));
    ...
    getMoreSplits(job, newpaths.toArray(new Path[newpaths.size()]), maxSize,
                  minSizeNode, minSizeRack, splits);
   ...
   */
  private void getMoreSplits(JobContext job, Path[] paths, long maxSize,
                             long minSizeNode, long minSizeRack,
    ...
    for (Iterator<Map.Entry<String, List<OneBlockInfo>>> iter =
             nodeToBlocks.entrySet().iterator();
         iter.hasNext();) {
    ...
    while (blockToNodes.size() > 0) {
      ...
      for (Iterator<Map.Entry<String, List<OneBlockInfo>>> iter =
               rackToBlocks.entrySet().iterator();
           iter.hasNext();) {
  ...
  private void addCreatedSplit(List<InputSplit> splitList,
    ...
    CombineFileSplit thissplit = new CombineFileSplit(
        fl, offset, length, locations.toArray(new String[0]));
  ...
  private static class OneFileInfo {
    ...
    OneFileInfo(Path path, Configuration conf, boolean isSplitable,
      ...
      if (locations == null) {
        ...
        if (!isSplitable) {
          ...
          blocks[0] =
              new OneBlockInfo(path, 0, fileSize, locations[0].getHosts(),
                               locations[0].getTopologyPaths());
        } else {
          ArrayList<OneBlockInfo> blocksList =
              new ArrayList<OneBlockInfo>(locations.length);
          for (int i = 0; i < locations.length; i++) {
            ...
            do {
              ...
              OneBlockInfo oneblock = new OneBlockInfo(
                  path, myOffset, myLength, locations[i].getHosts(),
    ...
    long getLength() { return fileSize; }

    OneBlockInfo[] getBlocks() { return blocks; }
  }
  ...
  private static class MultiPathFilter implements PathFilter {
    ...
    public MultiPathFilter() { this.filters = new ArrayList<PathFilter>(); }

    public MultiPathFilter(List<PathFilter> filters) { this.filters = filters; }

    public void add(PathFilter one) { filters.add(one); }
