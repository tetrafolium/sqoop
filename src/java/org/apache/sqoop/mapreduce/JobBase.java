...
public class JobBase {
  ...
  public JobBase(final SqoopOptions opts) { this(opts, null, null, null); }
   ...
   */
  public void setOptions(SqoopOptions opts) { this.options = opts; }
  ...
  protected void cacheJars(Job job, ConnManager mgr) throws IOException {
    ...
    if (localUrls.isEmpty() &&
        (org.apache.commons.lang.StringUtils.isEmpty(tmpjars))) {
      return;
    }
  ...
  protected void setJob(Job job) { mrJob = job; }

  /**
   * @return the main MapReduce job that is being run, or null if no
   * job has started.
   */
  public Job getJob() { return mrJob; }
  ...
  public Job createJob(Configuration configuration) throws IOException {
    ...
    if (configuration.getBoolean(SERIALIZE_SQOOPOPTIONS,
                                 SERIALIZE_SQOOPOPTIONS_DEFAULT)) {
  ...
  /**
   * Iterates over serialized form of SqoopOptions and put them into
   * Configuration object.
   *
   * @param opts SqoopOptions that should be serialized
   * @param configuration Target configuration object
   */
  public void putSqoopOptionsToConfiguration(SqoopOptions opts,
  ...
  protected void doValidate(SqoopOptions options, Configuration conf,
    ...
    ValidationThreshold threshold =
        (ValidationThreshold)ReflectionUtils.newInstance(
            options.getValidationThresholdClass(), conf);
    ValidationFailureHandler failureHandler =
        (ValidationFailureHandler)ReflectionUtils.newInstance(
            options.getValidationFailureHandlerClass(), conf);

    StringBuilder sb = new StringBuilder();
    sb.append("Validating the integrity of the import using the "
              + "following configuration\n");
    sb.append("\tValidator : ")
        .append(validator.getClass().getName())
        .append('\n');
    sb.append("\tThreshold Specifier : ")
        .append(threshold.getClass().getName())
        .append('\n');
    sb.append("\tFailure Handler : ")
        .append(failureHandler.getClass().getName())
        .append('\n');
