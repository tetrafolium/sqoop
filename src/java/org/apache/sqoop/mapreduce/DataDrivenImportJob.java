...
public static final Log LOG =
	LogFactory.getLog(DataDrivenImportJob.class.getName());

private final ParquetImportJobConfigurator parquetImportJobConfigurator;

public DataDrivenImportJob(
...
protected void configureMapper(Job job, String tableName,
	...
	if (options.getFileLayout() == SqoopOptions.FileLayout.TextFile) {
		...
		parquetImportJobConfigurator.configureMapper(job, schema, options,
...
private Schema generateAvroSchema(String tableName, String schemaNameOverride)
throws IOException {
	ConnManager connManager = getContext().getConnManager();
	AvroSchemaGenerator generator =
		new AvroSchemaGenerator(options, connManager, tableName);
...
private void writeAvroSchema(final Schema schema) throws IOException {
	...
	final File schemaFile =
		new File(options.getJarOutputDir(), schema.getName() + ".avsc");
	...
	try {
		FileUtils.moveFileToDirectory(schemaFile,
		                              new File(options.getCodeOutputDir()), true);
	} catch (final IOException e) {
	...
	try {
		...
		if (null == username || username.length() == 0) {
			DBConfiguration.configureDB(
				...
				options.getConnectString(), options.getFetchSize(),
		...
		if (null != tableName) {
			...
			DataDrivenDBInputFormat.setInput(
				job, DBWritable.class, mgr.escapeTableName(tableName), whereClause,
			...
			DataDrivenDBInputFormat.setInput(job, DBWritable.class, inputQuery,
			                                 inputBoundingQuery);
			new DBConfiguration(job.getConfiguration()).setInputOrderBy(splitByCol);
		}
		if (options.getRelaxedIsolation()) {
			LOG.info("Enabling relaxed (read uncommitted) transaction "
			         + "isolation for imports");
			job.getConfiguration().setBoolean(
				DBConfiguration.PROP_RELAXED_ISOLATION, true);
		}
