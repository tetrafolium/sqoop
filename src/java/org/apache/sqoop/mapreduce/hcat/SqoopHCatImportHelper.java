...
public class SqoopHCatImportHelper {
  public static final Log LOG =
      LogFactory.getLog(SqoopHCatImportHelper.class.getName());
  ...
  public SqoopHCatImportHelper(Configuration conf)
    ...
    storerInfoStr.append("HCatalog Storer Info : ")
        ...
        .append("\n\tInput format class = ")
        .append(storerInfo.getIfClass())
        .append("\n\tOutput format class = ")
        .append(storerInfo.getOfClass())
        .append("\n\tSerde class = ")
        .append(storerInfo.getSerdeClass());
    ...
    lobLoader = new LargeObjectLoader(
        conf, new Path(jobInfo.getTableInfo().getTableLocation()));
    ...
    IntWritable[] delimChars = DefaultStringifier.loadArray(
        conf, SqoopHCatUtilities.HIVE_DELIMITERS_TO_REPLACE_PROP,
        IntWritable.class);
    hiveDelimiters =
        new DelimiterSet((char)delimChars[0].get(), (char)delimChars[1].get(),
    ...
    doHiveDelimsReplacement = Boolean.valueOf(
        conf.get(SqoopHCatUtilities.HIVE_DELIMITERS_REPLACEMENT_ENABLED_PROP));

    IntWritable[] fPos = DefaultStringifier.loadArray(
        conf, SqoopHCatUtilities.HCAT_FIELD_POSITIONS_PROP, IntWritable.class);
    ...
    staticPartitionKeys =
        conf.getStrings(SqoopHCatUtilities.HCAT_STATIC_PARTITION_KEY_PROP);
    String partKeysString =
        staticPartitionKeys == null ? "" : Arrays.toString(staticPartitionKeys);
  ...
  public HCatRecord convertToHCatRecord(SqoopRecord sqr)
      throws IOException, InterruptedException {
    try {
    ...
    for (Map.Entry<String, Object> entry : fieldMap.entrySet()) {
      ...
      try {
        ...
        throw new IOException("Unable to lookup " + hfn +
                              " in the hcat schema");
      }
  ...
  private Object toHCat(Object val, HCatFieldSchema hfs) {
    ...
    if (val instanceof Number) {
      ...
      if (hfsType == HCatFieldSchema.Type.BINARY) {
        ...
        byte[] bytes =
            br.isExternal() ? br.toString().getBytes() : br.getData();
        retVal = bytes;
      }
    ...
    if (retVal == null) {
      LOG.error("Unable to convert [" + val + "]  of type " +
                val.getClass().getName() + " to HCatalog type " +
                hfs.getTypeString());
    }
    return retVal;
  }
  ...
  private Object convertStringTypes(Object val, HCatFieldSchema hfs) {
    ...
    if (isStringType(hfsType)) {
      ...
      if (doHiveDelimsReplacement) {
        str = FieldFormatter.hiveStringReplaceDelims(str, hiveDelimsReplacement,
                                                     hiveDelimiters);
      }
      if (hfsType == HCatFieldSchema.Type.STRING) {
        return str;
  ...
  private Object convertNumberTypes(Object val, HCatFieldSchema hfs) {
    ...
    if (val instanceof BigDecimal && isStringType(hfsType)) {
  ...
  private Object convertNumberToAnyType(HCatFieldSchema hfs,
  ...
  private Object convertBigDecimalToTextTypes(HCatFieldSchema hfs,
                                              HCatFieldSchema.Type hfsType,
