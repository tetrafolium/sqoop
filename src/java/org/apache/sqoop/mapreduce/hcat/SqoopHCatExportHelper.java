...
public static final Log LOG =
	LogFactory.getLog(SqoopHCatExportHelper.class.getName());
...
throws IOException, InterruptedException {
	...
	colTypesJava = DefaultStringifier.load(
		conf, SqoopHCatUtilities.HCAT_DB_OUTPUT_COLTYPES_JAVA,
		MapWritable.class);
	colTypesSql = DefaultStringifier.load(
		conf, SqoopHCatUtilities.HCAT_DB_OUTPUT_COLTYPES_SQL,
		MapWritable.class);
...
public SqoopRecord convertToSqoopRecord(HCatRecord hcr) throws IOException {
	...
	for (Map.Entry<String, Object> e : sqoopRecord.getFieldMap().entrySet()) {
		...
		Object sqlVal =
			convertToSqoop(hCatVal, fieldType, javaColType, hCatTypeString);
		if (debugHCatExportMapper) {
			LOG.debug("hCatVal " + hCatVal + " of type " +
			          ...
			          ",java type " + javaColType +
			          ", sql type = " + SqoopHCatUtilities.sqlTypeString(sqlType));
		}
...
private Object convertToSqoop(Object val, HCatFieldSchema.Type fieldType,
                              String javaColType, String hCatTypeString)
throws IOException {
