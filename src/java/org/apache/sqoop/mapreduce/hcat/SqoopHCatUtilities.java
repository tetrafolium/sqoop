...
public final class SqoopHCatUtilities {
  ...
  public static boolean isHCatView(final SqoopOptions opts) {
    ...
    try {
      ...
      LOG.warn("We were not able to determine if " + hCatDatabaseName + ":" +
               hCatTableName + "is view or table.");
  ...
  private static String getHCatDbNameFromOptions(SqoopOptions opts) {
    String hCatDatabaseName = opts.getHCatDatabaseName() != null
                                  ? opts.getHCatDatabaseName()
                                  : DEFHCATDB;
  ...
  public static SqoopHCatUtilities instance() { return Holder.INSTANCE; }

  private SqoopHCatUtilities() { configured = false; }

  public static final Log LOG =
      LogFactory.getLog(SqoopHCatUtilities.class.getName());
  ...
  private void checkHomeDirs(SqoopOptions opts) {
    ...
    if (home == null || home.length() == 0) {
      LOG.warn(
          "Hive home is not set. job may fail if needed jar files "
  ...
  public void configureHCat(final SqoopOptions opts, final Job job,
    ...
    HCatInputFormat hif =
        HCatInputFormat.setInput(hCatJob, hCatDatabaseName, hCatTableName);
    // For serializing the schema to conf
    if (filterStr != null) {
    ...
    for (HCatFieldSchema hfs : hCatPartitionSchemaFields) {
      if (hfs.getType() != HCatFieldSchema.Type.STRING) {
        throw new IOException(
            "The table provided " + getQualifiedHCatTableName() +
    ...
    for (String col : dbColumnNames) {
      try {
        ...
        throw new IOException("Caught Exception checking database column " +
                                  col + " in "
                                  + " hcatalog table.",
                              e);
      }
  ...
  public void validateDynamicPartitionKeysMapping() throws IOException {
    ...
    if (missingKeys.length() > 0) {
      throw new IOException(
          "Dynamic partition keys are not "
  ...
  public void validateHCatTableFieldTypes() throws IOException {
    ...
    if (hasComplexFields) {
      ...
      throw new IOException("The HCatalog table provided " +
                            ...
                            " has complex field types (" + unsupportedFields +
  ...
  private void initDBColumnInfo() throws IOException {
    ...
    if (dbTableName != null) {
      ...
      // Read procedure arguments from metadata
      colInfo.putAll(
          connManager.getColumnInfoForProcedure(this.options.getCall()));
    } else {
  ...
  private void createHCatTable(boolean dropIfExists, boolean isExternal)
      throws IOException {
    StringBuilder sb = new StringBuilder();
    if (dropIfExists) {
      sb.append("drop table ")
          .append(escHCatObj(hCatDatabaseName))
          .append('.')
          .append(escHCatObj(hCatTableName))
          .append(";\n");
    }
    sb.append("create ");
    if (isExternal) {
    ...
    sb.append("table ").append(escHCatObj(hCatDatabaseName)).append('.');
  ...
  private void validateStaticPartitionKey() throws IOException {
    ...
    if (hCatStaticPartitionKeys != null && hCatStaticPartitionKeys.size() > 0) {
      for (String key : hCatStaticPartitionKeys) {
        ...
        if (!found) {
          throw new IOException("The provided hive partition key " + key +
                                " is not part of the partition "
                                + " keys for table " +
                                getQualifiedHCatTableName());
        }
      }
  ...
  public static void
  configureImportOutputFormat(SqoopOptions opts, Job job, ConnManager connMgr,
                              String dbTable, Configuration config)
      throws IOException {
  ...
  public static void
  configureExportInputFormat(SqoopOptions opts, Job job, ConnManager connMgr,
                             String dbTable, Configuration config)
      throws IOException {
    ...
    for (Map.Entry<String, List<Integer>> e : dbColInfo.entrySet()) {
      ...
      if (mapColumnJava.containsKey(e.getKey())) {
        ...
        columnText = new Text(
            connMgr.toJavaType(dbTable, e.getKey(), e.getValue().get(0)));
      }
  ...
  public static void addJars(Job job, SqoopOptions options) throws IOException {
    ...
    String hCatStorageHandlerDir =
        hCatHome + File.separator + "share/hcatalog/storage-handlers";
  ...
  public void launchHCatCli(String cmdLine) throws IOException {
    ...
    tmpFileName = new File(tmpDir, "hcat-script-" + System.currentTimeMillis())
                      .getAbsolutePath();
  ...
  public static void setTestMode(boolean mode) { testMode = mode; }
  ...
  public String getDatabaseTable() { return dbTableName; }

  public String getHCatTableName() { return hCatTableName; }

  public String getHCatDatabaseName() { return hCatDatabaseName; }

  public String getQualifiedHCatTableName() { return hCatQualifiedTableName; }
  ...
  public String[] getDBColumnNames() { return dbColumnNames; }

  public HCatSchema getHCatOutputSchema() { return hCatOutputSchema; }

  public void setHCatOutputSchema(HCatSchema schema) {
    hCatOutputSchema = schema;
  }

  public HCatSchema getHCatPartitionSchema() { return hCatPartitionSchema; }
  ...
  public void setDbColumnNames(String[] names) { dbColumnNames = names; }

  public Map<String, List<Integer>> getDbColumnInfo() { return dbColumnInfo; }

  public String gethCatTableName() { return hCatTableName; }

  public String gethCatDatabaseName() { return hCatDatabaseName; }

  public String gethCatQualifiedTableName() { return hCatQualifiedTableName; }
