...
public static final Log LOG =
	LogFactory.getLog(SqoopHCatRecordReader.class.getName());

public SqoopHCatRecordReader(final InputSplit split,
                             final TaskAttemptContext context,
                             final SqoopHCatExportFormat inputFormat)
throws IOException {
...
@Override
public void initialize(final InputSplit split, final TaskAttemptContext ctxt)
throws IOException, InterruptedException {
	this.hCatSplit = (SqoopHCatInputSplit)split;
	this.context = ctxt;

	if (null != this.curReader) {
		this.curReader.initialize(((SqoopHCatInputSplit)split).get(0), context);
	}
}
