...
public class MainframeDatasetFTPRecordReader<T extends SqoopRecord>
  ...
  private static final Log LOG =
      LogFactory.getLog(MainframeDatasetFTPRecordReader.class.getName());
  ...
  protected boolean getNextRecord(T sqoopRecord) throws IOException {
    String line = null;
    Configuration conf = getConfiguration();
    if (MainframeConfiguration.MAINFRAME_FTP_TRANSFER_MODE_BINARY.equals(
    ...
    try {
      do {
        if (datasetReader == null) {
          ...
          datasetReader = new BufferedReader(
              new InputStreamReader(ftp.retrieveFileStream(dsName)));
        }
  ...
  protected boolean getNextBinaryRecord(T sqoopRecord) throws IOException {
    ...
    int BUFFER_SIZE = MainframeConfiguration
    ...
    try {
      ...
      do {
        bytesRead = inputStream.read(buf, cumulativeBytesRead,
                                     BUFFER_SIZE - cumulativeBytesRead);
        if (bytesRead == -1) {
          // EOF
          closeFtpInputStream();
          LOG.info("Data transfer completed.");
  ...
  protected void closeFtpInputStream() throws IOException {
    ...
    inputStream = null;
    if (!ftp.completePendingCommand()) {
      throw new IOException("Failed to complete ftp command. FTP Response: " +
                            ftp.getReplyString());
    }
  }

  protected Boolean writeBytesToSqoopRecord(byte[] buf, int cumulativeBytesRead,
