...
public class MainframeFTPFileEntryParser
   ...
   * G20427 Tape UNLOAD.ABCDE.ZZ9UYT.FB.TAPE SEM352 3390   2018/01/23  1    1 FB
   * 150  1500  PS  USER.BRODCAST OVR346 3390   2018/01/23  3    3  FB      80
   * 6160  PO  USER.ISPPROF
   */
  ...
  public MainframeFTPFileEntryParser(String regex) { super(REGEX); }
    ...
    if (matches(entry)) {
      ...
      if (!unit.equals("Tape")) {
        ...
        if (m.matches()) {
          ...
          LOG.info(String.format("Non tape match: %s, %s, %s", file.getName(),
                                 file.getSize(), dsOrg));
          if (dsOrg.equals(dsOrgPDSString) ||
              dsOrg.equals(dsOrgPDSExtendedString)) {
  ...
  protected FTPClientConfig getDefaultConfiguration() {
    return new FTPClientConfig(FTPClientConfig.SYST_MVS, DEFAULT_DATE_FORMAT,
                               null, null, null, null);
  }
  ...
  private FTPFile parseMigratedEntry(String entry) {
    ...
    if (m.matches()) {
      ...
      if (MIGRATED_STRING.equals(volume)) {
        ...
        file.setName(dsName);
        LOG.info(
            String.format("Migrated Dataset found: %s, %s", volume, dsName));
        return file;
      }
    }
    return null;
