...
public MergeJob(final SqoopOptions opts, final ParquetMergeJobConfigurator
...
public boolean runMergeJob() throws IOException {
	...
	try {
		...
		case PARQUET_FILE:
			...
			parquetMergeJobConfigurator.configureParquetMergeJob(
				jobConf, job, oldPath, newPath, finalPath);
			break;
		case AVRO_DATA_FILE:
...
private void configueAvroMergeJob(Configuration conf, Job job, Path oldPath,
	...
	if (oldPathSchema == null || newPathSchema == null ||
	    !oldPathSchema.equals(newPathSchema)) {
		throw new IOException(
			      "Invalid schema for input directories. Schema for old data: [" +
