...
/**
 * An implementation of {@link ParquetImportJobConfigurator} which depends on
...
public class HadoopParquetImportJobConfigurator
  ...
  private static final Log LOG =
      LogFactory.getLog(HadoopParquetImportJobConfigurator.class.getName());

  @Override
  public void configureMapper(Job job, Schema schema, SqoopOptions options,
                              String tableName, Path destination)
      throws IOException {
  ...
  private void configureLogicalTypeSupport(Job job, SqoopOptions options) {
    if (options.getConf().getBoolean(
            ConfigurationConstants.PROP_ENABLE_PARQUET_LOGICAL_TYPE_DECIMAL,
            false)) {
      AvroParquetOutputFormat.setAvroDataSupplier(job,
  ...
  void configureOutputCodec(Job job) {
    String outputCodec =
        job.getConfiguration().get(SQOOP_PARQUET_OUTPUT_CODEC_KEY);
    if (outputCodec != null) {
      LOG.info("Using output codec: " + outputCodec);
      ParquetOutputFormat.setCompression(
          job, CompressionCodecName.fromConf(outputCodec));
    }
  }
