...
public class JdbcExportJob extends ExportJobBase {
  ...
  public static final Log LOG =
      LogFactory.getLog(JdbcExportJob.class.getName());
  ...
  protected void configureInputFormat(Job job, String tableName,
    ...
    if (isHCatJob) {
      SqoopHCatUtilities.configureExportInputFormat(
          options, job, context.getConnManager(), tableName,
  ...
  private void configureGenericRecordExportInputFormat(Job job,
                                                       String tableName)
      throws IOException {
    ...
    if (options.getCall() == null) {
      ...
      columnTypeInts =
          connManager.getColumnTypesForProcedure(options.getCall());
    }
    String[] specifiedColumns = options.getColumns();
    MapWritable columnTypes = new MapWritable();
    for (Map.Entry<String, Integer> e : columnTypeInts.entrySet()) {
      String column = e.getKey();
      column = (specifiedColumns == null)
                   ? column
                   : options.getColumnNameCaseInsensitive(column);
      if (column != null) {
        Text columnName = new Text(column);
        Text columnType =
            new Text(connManager.toJavaType(tableName, column, e.getValue()));
