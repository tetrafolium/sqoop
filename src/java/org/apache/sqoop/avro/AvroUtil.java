...
public final class AvroUtil {
  ...
  private static BigDecimal padBigDecimal(BigDecimal bd, Schema schema) {
    ...
    if (schemaContainingScale != null) {
      int scale = Integer.valueOf(
          schemaContainingScale.getObjectProp("scale").toString());
      if (bd.scale() != scale) {
   ...
   */
  public static Object toAvro(Object o, Schema.Field field,
                              boolean bigDecimalFormatString,
                              boolean bigDecimalPaddingEnabled) {
  ...
  toGenericRecord(Map<String, Object> fieldMap, Schema schema,
                  boolean bigDecimalFormatString,
                  boolean bigDecimalPaddingEnabled) {
    ...
    for (Map.Entry<String, Object> entry : fieldMap.entrySet()) {
      ...
      Object avroObject =
          toAvro(entry.getValue(), field, bigDecimalFormatString,
                 bigDecimalPaddingEnabled);
  ...
  /**
   * This method checks if the precision is an invalid value, i.e. smaller than
   * 0 and if so, tries to overwrite precision and scale with the configured
   * defaults from the configuration object. If a default precision is not
   * defined, then throws an Exception.
   *
   * @param precision precision
   * @param scale scale
   * @param conf Configuration that contains the default values if the user
   *     specified them
   * @return an avro decimal type, that can be added as a column type in the
   *     avro schema generation
   */
  public static LogicalType createDecimalType(Integer precision, Integer scale,
                                              Configuration conf) {
    if (precision == null || precision <= 0) {
      // we check if the user configured default precision and scale and use
      // these values instead of invalid ones.
      Integer configuredPrecision =
          ConfigurationHelper.getIntegerConfigIfExists(
              conf, ConfigurationConstants.PROP_AVRO_DECIMAL_PRECISION);
      if (configuredPrecision != null) {
        precision = configuredPrecision;
      } else {
        throw new RuntimeException(
            "Invalid precision for Avro Schema. Please specify a default precision with the -D" +
            ConfigurationConstants.PROP_AVRO_DECIMAL_PRECISION +
  ...
  private static Path getFileToTest(Path path, Configuration conf)
      throws IOException {
  ...
  public static Schema getAvroSchemaFromParquetFile(Path path,
                                                    Configuration conf)
      throws IOException {
    Path fileToTest = getFileToTest(path, conf);
    if (fileToTest == null) {
    ...
    ParquetMetadata parquetMetadata = ParquetFileReader.readFooter(
        conf, fileToTest, ParquetMetadataConverter.NO_FILTER);
