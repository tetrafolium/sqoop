...
public class AvroSchemaGenerator {
   ...
   * @see <a
   *     href="https://github.com/apache/hive/blob/release-1.1/ql/src/java/org/apache/hadoop/hive/ql/io/parquet/serde/ParquetHiveSerDe.java#L90">Apache
   *     Hive</a>.
   */
  ...
  static {
    for (int prec = 1; prec <= MAX_PRECISION; prec++) {
      ...
      PRECISION_TO_BYTE_COUNT[prec - 1] = (int)Math.ceil(
          (Math.log(Math.pow(10, prec) - 1) / Math.log(2) + 1) / 8);
    }
  }
  ...
  public Schema generate() throws IOException { return generate(null); }

  public Schema generate(String schemaNameOverride) throws IOException {
    ClassWriter classWriter =
        new ClassWriter(options, connManager, tableName, null);
    ...
    for (String columnName : columnNames) {
      ...
      Field field =
          new Field(cleanedCol, avroSchema, null, NullNode.getInstance());
    ...
    String shortClassName =
        (tableName == null && options.getClassName() == null)
            ? DEFAULT_SCHEMA_NAME
            : tableClassName.getShortClassForTable(tableName);
    String avroTableName =
        (tableName == null ? TableClassName.QUERY_RESULT : tableName);
    String avroName =
        schemaNameOverride != null
            ? schemaNameOverride
   ...
   * @return Schema
   */
  public Schema toAvroSchema(int sqlType, String columnName, Integer precision,
                             Integer scale) {
  ...
  /**
   * @return True if this is a parquet import and parquet logical types are
   *     enabled,
   * or if this is an avro import and avro logical types are enabled. False
   * otherwise.
   */
  private boolean isLogicalTypeConversionEnabled() {
    if (ParquetFile.equals(options.getFileLayout())) {
      return options.getConf().getBoolean(
          ConfigurationConstants.PROP_ENABLE_PARQUET_LOGICAL_TYPE_DECIMAL,
          false);
    } else if (AvroDataFile.equals(options.getFileLayout())) {
      return options.getConf().getBoolean(
          ConfigurationConstants.PROP_ENABLE_AVRO_LOGICAL_TYPE_DECIMAL, false);
    }
    return false;
  }
  ...
  private LogicalType toAvroLogicalType(String columnName, int sqlType,
                                        Integer precision, Integer scale) {
    return connManager.toAvroLogicalType(tableName, columnName, sqlType,
                                         precision, scale);
  }
