...
public class CompilationManager {
  ...
  public static final Log LOG =
      LogFactory.getLog(CompilationManager.class.getName());
  ...
  public void addSourceFile(String sourceName) { sources.add(sourceName); }
  ...
  private String findHadoopJars() {
    ...
    Iterator<File> filesIterator = FileUtils.iterateFiles(
        hadoopMapRedHomeFile, new String[] {"jar"}, true);
  ...
  public void compile() throws IOException {
    ...
    for (String srcFileName : sources) {
      ...
      try {
         ...
         *any reason.*/
        LOG.warn("Could not rename " + orig + " to " + dest +
                 ". Error: " + e.getMessage());
      }
    }
  }
    ...
    for (File entry : dirEntries) {
      if (!entry.isDirectory()) {
        ...
        if (includeFileInJar(classFileName)) {
          ...
          LOG.debug("Got classfile: " + entry.getPath() + " -> " +
                    classFileName);
  ...
  boolean isInnerClass(String sourceFileName) {
    ...
    String sourceFileOfPotentialOuterClass =
        potentialOuterClassName + JAVA_EXTENSION;
