...
public ClassWriter(final SqoopOptions opts, final ConnManager connMgr,
	...
	this.maxColumnsPerMethod = this.options.getConf().getInt(
		PROPERTY_CODEGEN_METHODS_MAXCOLS, MAX_COLUMNS_PER_METHOD_DEFAULT);
}
...
public static String toJavaIdentifier(String candidate) {
	...
	if (isReservedWord(output)) {
		 ...
		 * purpose to preserve backward compatibility as much as possible. For
		 * example column "9isLegalInSql" was translated into "_9isLegalInSql" in
		 * original code and will translate to same "_9isLegalInSql" now. However
		 * it would be translated to "__9isLegalInSql" (notice that there are two
		 * "_" at the beginning) if we would use output.startsWith instead.
		 */
...
private String rpcGetterForType(String javaType, String inputObj,
	...
	if (javaType.equals("Integer")) {
		...
		return "    this." + colName + " = " +
		       BigDecimalSerializer.class.getCanonicalName() + ".readFields(" +
		       inputObj + ");\n";
	} else if (javaType.equals(ClobRef.class.getName())) {
		return "    this." + colName + " = " +
		       LobSerializer.class.getCanonicalName() + ".readClobFields(" +
		       inputObj + ");\n";
	} else if (javaType.equals(BlobRef.class.getName())) {
		return "    this." + colName + " = " +
		       LobSerializer.class.getCanonicalName() + ".readBlobFields(" +
		       inputObj + ");\n";
...
private String rpcSetterForType(String javaType, String outputObj,
	...
	if (javaType.equals("Integer")) {
		...
		throw new IllegalArgumentException("No ResultSet method for Java type " +
		                                   javaType);
	}
}
...
private void generateDbRead(Map<String, Integer> columnTypes,
	...
	if (numberOfMethods > 1) {
		...
		myGenerateDbRead(columnTypes, colNames, sb, 0, maxColumnsPerMethod,
		                 false);
	}
	sb.append("  }\n");
...
private void myGenerateDbRead(Map<String, Integer> columnTypes,
	...
	for (int i = methodNumber * size;
		...
		String javaType = toJavaType(col, sqlType);
		if (null == javaType) {
			LOG.error("No Java type for SQL type " + sqlType + " for column " +
			          col);
			continue;
		}
...
private void generateDbWrite(Map<String, Integer> columnTypes,
	...
	if (numberOfMethods > 1) {
		...
		myGenerateDbWrite(columnTypes, colNames, sb, 0, maxColumnsPerMethod,
		                  false);
	}
	...
	for (int i = 0; i < numberOfMethods; ++i) {
		myGenerateDbWrite(columnTypes, colNames, sb, i, maxColumnsPerMethod,
		                  true);
	}
}
...
private void generateHadoopRead(Map<String, Integer> columnTypes,
	...
	for (int i = 0; i < numberOfMethods; ++i) {
		myGenerateHadoopRead(columnTypes, colNames, sb, i, maxColumnsPerMethod,
		                     true);
	}
}
...
private void generateCloneMethod(Map<String, Integer> columnTypes,
	...
	if (numberOfMethods > 1) {
		...
		myGenerateCloneMethod(columnTypes, colNames, sb, 0, maxColumnsPerMethod,
		                      false);
	}
	...
	for (int i = 0; i < numberOfMethods; ++i) {
		myGenerateCloneMethod(columnTypes, colNames, sb, i, maxColumnsPerMethod,
		                      true);
	}
}
...
private void myGenerateCloneMethod(Map<String, Integer> columnTypes,
	...
	for (int i = methodNumber * size;
		...
		if (null == javaType) {
			...
			sb.append("    o." + colName + " = (o." + colName + " != null) ? "
			          + "new BytesWritable(Arrays.copyOf(" + colName +
			          ".getBytes(), " + colName + ".getLength())) : null;\n");
		}
	}
...
private void generateSetField(Map<String, Integer> columnTypes,
                              String[] colNames, String[] rawColNames,
                              StringBuilder sb) {
...
private void generateConstructorAndInitMethods(Map<String, Integer> colTypes,
                                               String[] colNames,
                                               String[] rawColNames,
                                               String typeName,
                                               StringBuilder sb) {
	...
	for (int methodNumber = 0; methodNumber < numberOfMethods; ++methodNumber) {
		...
		for (int i = methodNumber * maxColumnsPerMethod;
		     i < topBoundary(colNames, methodNumber, maxColumnsPerMethod); ++i) {
			String colName = colNames[i];
			...
			if (null == javaType) {
				...
				sb.append("    setters.put(\"" + serializeRawColName(rawColName) +
				          "\", new FieldSetterCommand() {" + sep);
				sb.append("      @Override" + sep);
				sb.append("      public void setField(Object value) {" + sep);
				sb.append("        " + typeName + ".this." + colName + " = (" +
				          javaType + ")value;" + sep);
				sb.append("      }" + sep);
				sb.append("    });" + sep);
			}
...
/**
 * Raw column name is a column name as it was created on database and we need
 * to serialize it between double quotes into java class that will be further
 * complied with javac. Various databases supports various edge conditions for
 * column names, so we can't assume that raw column name can be just passed
 * between double quotes and everything will work correctly. That will break
 * for databases that supports double quotes in column name as that will
 * result in "column with "" which is invalid.
 *
...
private void myGenerateSetField(Map<String, Integer> columnTypes,
                                String[] colNames, String[] rawColNames,
                                StringBuilder sb, int methodNumber,
                                int size) {
...
private void myGenerateGetFieldMap(Map<String, Integer> columnTypes,
	...
	for (int i = methodNumber * size;
		...
		sb.append("    __sqoop$field_map.put(\"" +
		          serializeRawColName(rawColNames[i]) + "\", this." +
		          colNames[i] + ");\n");
	}
...
private void generateParser(Map<String, Integer> columnTypes,
	...
	if (numberOfMethods > 1) {
		...
		myGenerateParser(columnTypes, colNames, sb, 0, maxColumnsPerMethod,
		                 false);
	}
	sb.append("  }\n\n");
...
private void generateHadoopWrite(Map<String, Integer> columnTypes,
	...
	if (numberOfMethods > 1) {
		...
		myGenerateHadoopWrite(columnTypes, colNames, sb, 0, maxColumnsPerMethod,
		                      false);
	}
	...
	for (int i = 0; i < numberOfMethods; ++i) {
		myGenerateHadoopWrite(columnTypes, colNames, sb, i, maxColumnsPerMethod,
		                      true);
	}
}
...
public void generate() throws IOException {
	...
	if (colNames == null || colNames.length == 0) {
		throw new IllegalArgumentException(
			      "There is no column found in the "
	...
	for (int i = 0; i < colNames.length; i++) {
		...
		if (identifier.isEmpty()) { // Name can't be blank
			throw new IllegalArgumentException(
				      "We found column without column "
	...
	StringBuilder sb = generateClassForColumns(
		columnTypes, cleanedColNames, cleanedDbWriteColNames, colNames);
	...
	// /tmp/sqoop-<username>/compile Add shutdown hook so that all files under
	// the jar output dir can be cleared
	if (options.getDeleteJarOutputDir()) {
		Runtime.getRuntime().addShutdownHook(
			new DirCleanupHook(options.getJarOutputDir()));
	}
...
protected String[] getColumnNames(Map<String, Integer> columnTypes) {
	...
	if (null == colNames) {
		if (null != tableName) {
			...
			colNames =
				connManager.getColumnNamesForProcedure(this.options.getCall());
		} else {
			// Infer/assign column names for arbitrary query.
			colNames =
				connManager.getColumnNamesForQuery(this.options.getSqlQuery());
		}
	} else {
		...
		for (int i = 0; i < colNames.length; ++i) {
			...
			if (len > 2 && userColName.charAt(0) == '"' &&
			    userColName.charAt(len - 1) == '"') {
...
generateClassForColumns(Map<String, Integer> columnTypes, String[] colNames,
                        String[] dbWriteColNames, String[] rawColNames) {
	...
	generateConstructorAndInitMethods(columnTypes, colNames, rawColNames,
	                                  className, sb);
