  ...
  public static String toHiveType(Schema schema, SqoopOptions options) {
    ...
    default:
      throw new RuntimeException(String.format(
          "There is no Hive type mapping defined for the Avro type of: %s ",
          avroType.getName()));
    }
  }

  private static String mapToDecimalOrBinary(Schema schema,
                                             SqoopOptions options) {
    boolean logicalTypesEnabled = options.getConf().getBoolean(
        ConfigurationConstants.PROP_ENABLE_PARQUET_LOGICAL_TYPE_DECIMAL, false);
    if (logicalTypesEnabled && schema.getLogicalType() instanceof Decimal) {
      ...
      int precision =
          Math.min(HiveDecimal.MAX_PRECISION, decimal.getPrecision());
      if (precision < decimal.getPrecision()) {
        LOG.warn(
            "Warning! Precision in the Hive table definition will be smaller than the actual precision of the column on storage! Hive may not be able to read data from this column.");
      }
      int scale = Math.min(HiveDecimal.MAX_SCALE, decimal.getScale());
      if (scale < decimal.getScale()) {
        LOG.warn(
            "Warning! Scale in the Hive table definition will be smaller than the actual scale of the column on storage! Hive may not be able to read data from this column.");
      }
