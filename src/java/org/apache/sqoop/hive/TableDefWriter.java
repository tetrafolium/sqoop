...
public class TableDefWriter {
  ...
  public static final Log LOG =
      LogFactory.getLog(TableDefWriter.class.getName());
  ...
  public String getCreateTableStmt() throws IOException {
    ...
    Map<String, Schema> columnNameToAvroFieldSchema =
        getColumnNameToAvroTypeMapping();
  ...
  private String getHiveColumnTypeForTextTable(Properties userMapping,
                                               String columnName,
                                               Integer columnType)
      throws IOException {
    ...
    if (hiveColType == null) {
      hiveColType =
          connManager.toHiveType(inputTableName, columnName, columnType);
    }
    if (null == hiveColType) {
  ...
  public static String getHiveOctalCharCode(int charNum) {
    if (charNum > 0177) {
      throw new IllegalArgumentException("Character " + charNum +
                                         " is an out-of-range delimiter");
    }
  ...
  /**
   * The JDBC connection owned by the ConnManager has been most probably opened
   * when the import was started so it might have timed out by the time
   * TableDefWriter methods are invoked which happens at the end of import. The
   * task of this method is to discard the current connection held by
   * ConnManager to make sure that TableDefWriter will have a working one.
   */
  private void resetConnManager() { this.connManager.discardConnection(true); }
  ...
  String getInputTableName() { return inputTableName; }

  String getOutputTableName() { return outputTableName; }

  boolean isCommentsEnabled() { return commentsEnabled; }

  Schema getAvroSchema() {
    if (avroSchema == null) {
      String schemaString =
          options.getConf().get(SQOOP_PARQUET_AVRO_SCHEMA_KEY);
