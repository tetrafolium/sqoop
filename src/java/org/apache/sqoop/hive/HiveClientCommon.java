...
public static final Log LOG =
	LogFactory.getLog(HiveClientCommon.class.getName());
 ...
 */
public void removeTempLogs(Configuration configuration, Path tablePath)
throws IOException {
 ...
 * @throws IOException
 */
public void cleanUp(Configuration configuration, Path outputPath)
throws IOException {
		...
		if (outputPath != null && fs.exists(outputPath)) {
			...
			if (statuses.length == 0) {
				...
				fs.delete(outputPath, true);
			           ...
			           statuses[0].getPath().getName().equals(
					   FileOutputCommitter.SUCCEEDED_FILE_NAME)) {
				LOG.info(
					"Export directory is contains the _SUCCESS file only, removing the directory.");
				fs.delete(outputPath, true);
			} else {
...
public void indexLzoFiles(SqoopOptions sqoopOptions, Path finalPath)
throws IOException {
	String codec = sqoopOptions.getCompressionCodec();
	if (codec != null &&
	    (codec.equals(CodecMap.LZOP) ||
	     codec.equals(CodecMap.getCodecClassName(CodecMap.LZOP)))) {
		try {
			Tool tool = ReflectionUtils.newInstance(
				Class.forName("com.hadoop.compression.lzo.DistributedLzoIndexer")
				.asSubclass(Tool.class),
