...
public abstract class LobRef<DATATYPE, CONTAINERTYPE, ACCESSORTYPE>
  ...
  protected CONTAINERTYPE getDataObj() { return realData; }

  /** Internal API to set the data object. */
  protected void setDataObj(CONTAINERTYPE data) { this.realData = data; }
   ...
   */
  public boolean isExternal() { return fileName != null; }
  ...
  public ACCESSORTYPE getDataStream(Configuration conf, Path basePath)
    ...
    if (isExternal()) {
      ...
      Path pathToRead =
          LobReaderCache.qualify(new Path(basePath, fileName), conf);
      ...
      new ThreadLocal<Matcher>() {
        ...
        protected Matcher initialValue() {
          Pattern externalPattern =
              Pattern.compile("externalLob\\(lf,(.*),([0-9]+),([0-9]+)\\)");
