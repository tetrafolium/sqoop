...
private void testSequentialScan(boolean compress) throws Exception {
	...
	try {
		...
		for (int i = 0; i < numRecords; i++) {
			...
			if (id != i) {
				throw new Exception("record id mismatch; expected " + i + " got " +
				                    id);
			}
...
throws Exception {
	...
	if (r.getRecordId() != expectedId) {
		throw new Exception("Expected record id=" + expectedId + "; got " +
		                    r.getRecordId());
	}
	...
	if (expected > 0) {
		throw new Exception("Couldn't read all the data! expected " + expected +
		                    " more bytes");
	}
